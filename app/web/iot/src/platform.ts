...
export function saveToken(value: string | null): void {
    ...
    } catch { /* without storage the token lives until a reload — so be it */ }
}
...
export const api = {
    ...
    add: (deviceName: string, label: string) =>
        iot<{ devices: DeviceView[] }>('/devices', { method: 'POST', body: JSON.stringify({ deviceName, label }) }),
    remove: (deviceName: string) =>
        iot<{ devices: DeviceView[] }>(`/devices/${encodeURIComponent(deviceName)}`, { method: 'DELETE' }),
