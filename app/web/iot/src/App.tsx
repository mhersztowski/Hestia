        ...
        <Stack spacing={2}>
          {platformAvailable === false && (
            <Alert severity="warning">
              The platform (<code>app/backend</code>) is not responding — without it there are no
              files and no MQTT. Start it with <code>pnpm dev:platform</code>.
            </Alert>
          )}
          {error && <Alert severity="error">{error}</Alert>}
                ...
                {devices.map((d) => (
                  ...
                  >
                    ...
                    <IconButton
                      ...
                      aria-label="Delete"
                      onClick={async () => {
                        setDevices((await api.remove(d.deviceName)).devices);
                      }}
                    >
          ...
          <Card variant="outlined">
            <CardContent>
              ...
              <Typography variant="body2" color="text.secondary">
                ...
                readings — <code>app/iot</code>. This application has no users and no store of its
                own.
              </Typography>
            </CardContent>
          </Card>
    ...
    <Dialog open={open} onClose={onClose} maxWidth="xs" fullWidth>
      ...
      <DialogContent>
        <Stack spacing={2} sx={{ mt: 1 }}>
          <TextField
            label="Name (the key in MQTT)"
            value={deviceName}
            autoFocus
            onChange={(e) => setDeviceName(e.target.value)}
            helperText="e.g. sensor-living-room"
          />
