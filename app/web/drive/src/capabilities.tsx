...
export function buildEditor(opts: DriveCapabilityOptions): DriveEditor {
  ...
  return {
    ...
    viewOptions: [
      {
        key: 'minimalView',
        label: 'Widok minimalny',
        description: 'Bez marginesów i bez nagłówków/ramek osadzonych bloczków',
      },
      {
        key: 'fullWidth',
        label: 'Pełna szerokość',
        description: 'Treść na całą szerokość — bez pustych obszarów po bokach',
      },
    ...
    render(
      file: DriveFileRef,
      {
        onSaved,
        view,
      }: { onClose: () => void; onSaved: () => void; view: Readonly<Record<string, boolean>> }
    ) {
