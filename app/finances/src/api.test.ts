...
describe('finance API', () => {
    ...
    it('creates an account and computes its balance from the transactions', async () => {
        const account = (await send('/api/accounts', 'POST', { name: 'Current', kind: 'personal', openingBalance: toMinorUnits('100,00') })).body;
        await send('/api/transactions', 'POST', { date: '2026-01-05', amount: toMinorUnits('-30,00'), description: 'Shopping', accountId: account.id });
    ...
    it('refuses a transaction on an account that does not exist', async () => {
        ...
        // not there.
        const r = await send('/api/transactions', 'POST', { date: '2026-01-05', amount: -100, description: '', accountId: 'nowhere' });
        expect(r.status).toBe(400);
        expect(r.body.error).toMatch(/nowhere/);
    });

    it('rejects data that does not match the schema', async () => {
        const r = await send('/api/transactions', 'POST', { date: '5 January', amount: 'a lot', accountId: 'a' });
    ...
    it('filters transactions by month and sorts newest first', async () => {
        ...
        for (const date of ['2026-01-05', '2026-01-20', '2026-02-01']) {
            await send('/api/transactions', 'POST', { date, amount: -100, description: date, accountId: account.id });
        }
    ...
    it('changes and deletes a transaction', async () => {
        const account = (await send('/api/accounts', 'POST', { name: 'A', kind: 'personal' })).body;
        const t = (await send('/api/transactions', 'POST', { date: '2026-01-05', amount: -100, description: 'old', accountId: account.id })).body;

        expect((await send(`/api/transactions/${t.id}`, 'PUT', { description: 'new' })).body.description).toBe('new');
    ...
    it('the summary gives everything for one screen at once', async () => {
        const account = (await send('/api/accounts', 'POST', { name: 'A', kind: 'personal', openingBalance: 0 })).body;
        await send('/api/categories', 'POST', { name: 'Food' });
        const category = (await ask('/api/categories')).body[0];
        await send('/api/transactions', 'POST', { date: '2026-01-05', amount: toMinorUnits('-120,00'), description: 'Shopping', accountId: account.id, categoryId: category.id });
        await send('/api/transactions', 'POST', { date: '2026-01-06', amount: toMinorUnits('500,00'), description: 'Inflow', accountId: account.id });

        const s = (await ask('/api/summary?month=2026-01')).body;
        expect(s.summary).toEqual({ month: '2026-01', income: 50000, expenses: 12000, net: 38000, count: 2 });
        expect(s.categories).toEqual([{ categoryId: category.id, total: 12000, name: 'Food', color: undefined }]);
