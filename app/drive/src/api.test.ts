...
describe('Drive API', () => {
  ...
  it("forwards files to the platform with the caller's own token", async () => {
    const written = await ask('/platform/api/vfs/writeFile', {
      method: 'POST',
      token: 'good',
      body: JSON.stringify({ path: 'notes/a.md', content: '# A note' }),
    });
    expect(written.status).toBe(200);
    expect(files.get('notes/a.md')).toBe('# A note');

    const read = await ask('/platform/api/vfs/readFile?path=notes%2Fa.md', { token: 'good' });
    expect(read.body.content).toBe('# A note');
  });
