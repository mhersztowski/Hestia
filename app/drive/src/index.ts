/**
 * The Drive application's server.
 *
 * It serves the built frontend from `public/` (that is, `app/web/drive`) and
 * exposes `/api/*` of its own; accounts and files come from the platform
 * (`app/backend`), reached through `/platform/*` — see `api.ts`.
 *
 * There is no store here. The drive shows the platform's files, and this server
 * exists to serve one page and to be the single address that page talks to.
 */
...
const PORT = Number(process.env.DRIVE_PORT ?? 4997);
...
async function main(): Promise<void> {
    ...
    console.log(`[drive] server running at http://localhost:${port}`);
    console.log(`[drive] platform: ${PLATFORM_URL}`);
    ...
    console.log(info ? '[drive] platform responding' : '[drive] platform not responding (start app/backend)');

    for (const signal of ['SIGINT', 'SIGTERM'] as const) {
        process.on(signal, () => {
            console.log(`\n[drive] ${signal} — shutting down`);
...
main().catch((e) => {
    console.error('[drive] failed to start:', e);
    process.exit(1);
});
