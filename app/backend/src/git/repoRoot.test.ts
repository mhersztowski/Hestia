...
describe('znajdzKorzenRepo', () => {
  ...
  it('znajduje repozytorium, gdy start jest jego korzeniem', () => {
    expect(znajdzKorzenRepo(`${drive}/projekt`, drive, maGit([`${drive}/projekt/.git`]))).toBe(
      `${drive}/projekt`
    );
  });
  ...
  it('wspina się przez katalogi pośrednie', () => {
    expect(
      znajdzKorzenRepo(`${drive}/projekt/src/moduł`, drive, maGit([`${drive}/projekt/.git`]))
    ).toBe(`${drive}/projekt`);
  });
  ...
  it('zatrzymuje się na korzeniu drive i nie sięga wyżej', () => {
    ...
    expect(znajdzKorzenRepo(`${drive}/projekt`, drive, maGit(['/data/.git', '/.git']))).toBeNull();
  });
