...
describe('GitService — repozytorium spod dowolnego podkatalogu', () => {
  ...
  const gitW = (dir: string, ...args: string[]) =>
  ...
  it('przygotowuje wszystkie nieśledzone pliki naraz', async () => {
    ...
    const r = await svc.stage(
      'ala',
      'projekt',
      nieslecone.map((z) => z.path)
    );
  ...
  it('odmawia odczytu spoza repozytorium', async () => {
    ...
    await expect(svc.worktree('ala', 'projekt', '../sekret.txt')).rejects.toThrow(
      /poza repozytorium/
    );
  });
  ...
  it('znacznik `.repo.json` dalej wskazuje swój podkatalog', async () => {
    zalozRepo('klon');
    writeFileSync(
      join(drive, 'klon.repo.json'),
      JSON.stringify({ type: 'git-repo', version: 1, url: 'https://example.invalid/x.git' })
    );
  ...
  it('znacznik w korzeniu repozytorium też jest znajdowany', async () => {
    ...
    writeFileSync(
      join(repo, '.repo.json'),
      JSON.stringify({ type: 'git-repo', version: 1, url: 'https://example.invalid/y.git' })
    );
    expect((await svc.info('ala', 'projekt/plik.txt')).repo.url).toBe(
