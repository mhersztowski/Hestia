...
describe('IoT API', () => {
    ...
    beforeEach(async () => {
        ...
        platformServer.get('/api/platform/info', () => ({ name: 'stub', mqtt: '/mqtt', capabilities: ['files', 'mqtt', 'auth'] }));
        platformServer.get('/api/auth/me', (ctx) => {
            lastToken = (ctx.req.headers.authorization ?? '').replace('Bearer ', '');
            if (lastToken !== 'good') { ctx.res.writeHead(401, { 'Content-Type': 'application/json' }); ctx.res.end('{}'); return; }
            return { user: { userId: 'u1', userName: 'admin', isAdmin: true, roles: [] } };
        });
        platformServer.get('/api/vfs/readFile', (ctx) => {
            const p = ctx.query.get('path') ?? '';
            if (!files.has(p)) { ctx.res.writeHead(500, { 'Content-Type': 'application/json' }); ctx.res.end('{"error":"missing"}'); return; }
    ...
    it('the registry lives in the platform files, not locally', async () => {
        await ask('/api/devices', {
            method: 'POST', token: 'good', body: JSON.stringify({ deviceName: 'sensor-1', label: 'Living room' }),
        });
    ...
    it('a reading stores metrics and marks the device present', async () => {
        await ask('/api/devices/sensor-1/reading', {
            method: 'POST', token: 'good', body: JSON.stringify({ temperature: 21.5 }),
        });
        const list = (await ask('/api/devices', { token: 'good' })).body.devices;
        expect(list[0]).toMatchObject({ deviceName: 'sensor-1', online: true, metrics: { temperature: 21.5 } });
    });

    it('a reading with no numbers is an error, not an empty write', async () => {
        const r = await ask('/api/devices/x/reading', {
            method: 'POST', token: 'good', body: JSON.stringify({ state: 'ok' }),
        });
        expect(r.status).toBe(400);
    });

    it('deletes a device, and reports a missing one as 404', async () => {
        await ask('/api/devices', { method: 'POST', token: 'good', body: JSON.stringify({ deviceName: 'a' }) });
