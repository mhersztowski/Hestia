...
async function whoIs(platform: PlatformClient, ctx: Context): Promise<{ token: string; who: UserIdentity }> {
    const t = token(ctx);
...
async function saveRegistry(platform: PlatformClient, t: string, registry: DeviceRegistry): Promise<void> {
...
export function registerForwarding(server: HttpServer, platformUrl: string): void {
    server.prefix('/platform', async (ctx) => {
        ...
        // from an outage.
        ctx.res.writeHead(response.status, { 'Content-Type': response.headers.get('content-type') ?? 'application/json' });
        ctx.res.end(text);
    });
}
...
export function registerApi(server: HttpServer, platform: PlatformClient): void {
    ...
    server.post('/api/devices/:deviceName/reading', async (ctx) => {
        ...
        if (Object.keys(numbers).length === 0) throw new HttpError(400, 'A reading must carry at least one numeric value');
    ...
    server.delete('/api/devices/:deviceName', async (ctx) => {
        ...
        const without = { devices: registry.devices.filter((d) => d.deviceName !== ctx.params.deviceName) };
