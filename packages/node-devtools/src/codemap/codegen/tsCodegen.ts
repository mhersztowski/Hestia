...
const vis = (v: Visibility): string =>
  v === 'private' ? 'private ' : v === 'protected' ? 'protected ' : '';
...
function genClass(s: CodeSymbol): string {
  const heritage = [
    s.extends.length ? `extends ${s.extends[0]}` : '',
    s.implements.length ? `implements ${s.implements.join(', ')}` : '',
  ]
    .filter(Boolean)
    .join(' ');
  ...
  for (const m of s.members.filter((x) => x.kind === 'field')) {
    lines.push(
      `  ${vis(m.visibility)}${m.isStatic ? 'static ' : ''}${m.name}${m.type ? `: ${m.type}` : ''};`
    );
  }
  for (const m of s.members.filter((x) => x.kind === 'method')) {
    if (m.isAbstract) {
      lines.push(`  abstract ${m.name}(${paramList(m.params)})${m.type ? `: ${m.type}` : ''};`);
      continue;
    }
    lines.push(
      `  ${vis(m.visibility)}${m.isStatic ? 'static ' : ''}${m.isAsync ? 'async ' : ''}${m.name}(${paramList(m.params)})${m.type ? `: ${m.type}` : ''} {`
    );
...
const genEnum = (s: CodeSymbol): string =>
