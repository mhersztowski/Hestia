...
const parserFor = (lang: Language): LanguageParser | undefined =>
  PARSERS.find((p) => p.languages.includes(lang));
...
export async function buildModel(files: SourceFile[]): Promise<CodeModel> {
  ...
  for (const f of files) {
    ...
    try {
      ...
      /* skip files that fail to parse rather than abort the whole run */
    }
  }
