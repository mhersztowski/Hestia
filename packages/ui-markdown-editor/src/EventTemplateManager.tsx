...
import { useEditorFiles } from './capabilities';
...
const EventTemplateManager: React.FC<EventTemplateManagerProps> = ({
  ...
  }, [open, userName, seedTemplate, files]);
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await saveTemplates(files, templates);
      setDirty(false);
      onSaved?.(templates);
      onClose();
    } catch (err) {
      ...
      <DialogContent>
        {loading ? (
          ...
          <Stack direction={{ xs: 'column', md: 'row' }} spacing={2} sx={{ mt: 1, minHeight: 400 }}>
            ...
            <Paper
              variant="outlined"
              sx={{ width: { md: 240 }, flexShrink: 0, display: 'flex', flexDirection: 'column' }}
            >
              <Stack direction="row" alignItems="center" sx={{ p: 1 }}>
                ...
                <Tooltip title="Dodaj szablon">
                  <IconButton size="small" onClick={addTemplate}>
              ...
              {!active ? (
                ...
                <Stack spacing={2}>
                  ...
                  {active.items.map((item, idx) => {
                    ...
                    const liveTask = item.taskId
                      ? tasks.find((t) => t.id === item.taskId)
                      : undefined;
                    const selectedTask: TaskOption | null =
                      liveTask ??
                      (item.taskId
                        ? {
                            id: item.taskId,
                      ...
                      <Paper key={idx} variant="outlined" sx={{ p: 1.5 }}>
                        <Stack spacing={1}>
                          <Stack direction="row" spacing={1} alignItems="center">
                            <Typography
                              variant="caption"
                              sx={{ color: 'text.secondary', minWidth: 28 }}
                            >
                              #{idx + 1}
                            </Typography>
                            <TextField
                          ...
                          <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1}>
                            <TextField
                              ...
                              onChange={(e) =>
                                updateItem(idx, { dayOffset: parseInt(e.target.value, 10) || 0 })
                              }
                              size="small"
                              sx={{ width: 130 }}
