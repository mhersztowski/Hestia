...
/** Math, image, audio and video are registered by extensions this toolbar does
 *  not import, so TipTap's chain type does not carry their commands. */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const extraCommands = (editor: Editor): any => editor.chain().focus();
...
export function moveBlock(editor: Editor, dir: 'up' | 'down'): void {
  ...
  if (dir === 'up') {
    ...
    try {
      tr.setSelection(TextSelection.near(tr.doc.resolve(Math.max(1, np))));
    } catch {
      /* ok */
    }
...
export const FormatPanel: React.FC<{ editor: Editor; onClose: () => void }> = ({
  editor,
  onClose,
}) => {
  const run = (fn: () => void) => {
    ...
    <Box sx={{ p: 1, display: 'flex', flexWrap: 'wrap', gap: 0.5, alignItems: 'center' }}>
      ...
      <Tooltip title="Math / equation">
        <IconButton
          size="medium"
          sx={BTN}
          onClick={() => run(() => extraCommands(editor).insertMathBlock('').run())}
        >
          <FunctionsIcon />
        </IconButton>
      </Tooltip>
...
export const ColorPanel: React.FC<{ editor: Editor; onClose: () => void }> = ({
  editor,
  onClose,
}) => (
    ...
    {HIGHLIGHT_COLORS.map(({ label, value }) => (
      <Box
        ...
        sx={{
          ...
          background: value,
          border: '3px solid',
          borderColor: editor.isActive('highlight', { color: value })
            ? 'primary.main'
            : 'transparent',
          boxShadow: 1,
          cursor: 'pointer',
...
export const TurnIntoPanel: React.FC<{ editor: Editor; onClose: () => void }> = ({
  editor,
  onClose,
}) => (
...
export const InsertPanel: React.FC<{ editor: Editor; onClose: () => void }> = ({
  editor,
  onClose,
}) => (
  <Box sx={{ p: 1, display: 'flex', gap: 0.5 }}>
    <Tooltip title="Image">
      <IconButton
        ...
        sx={BTN}
        onClick={() => {
          extraCommands(editor).setImage({ src: '', alt: '' }).run();
          onClose();
        }}
      >
        <ImageIcon />
    ...
    <Tooltip title="Audio">
      <IconButton
        ...
        sx={BTN}
        onClick={() => {
          extraCommands(editor).setAudio({ src: '' }).run();
          onClose();
        }}
      >
        <AudiotrackIcon />
    ...
    <Tooltip title="Video">
      <IconButton
        ...
        sx={BTN}
        onClick={() => {
          extraCommands(editor).setVideo({ src: '' }).run();
          onClose();
        }}
      >
        <VideocamIcon />
...
export const MobileMdToolbar: React.FC<Props> = ({ editor, keyboardOffset }) => {
  ...
  const togglePanel = useCallback(
    (p: SubPanel) => setSubPanel((prev) => (prev === p ? null : p)),
    []
  );
  ...
  const toolbarBottomCss =
    keyboardOffset > 0 ? `${keyboardOffset}px` : 'env(safe-area-inset-bottom, 0px)';
  const subPanelBottomCss =
    keyboardOffset > 0
