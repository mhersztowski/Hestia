...
import { useEditorServices, type ScriptTemplate } from './capabilities';
...
import {
  InfoMark,
  INFO_MARK_EDIT_EVENT,
  type InfoMarkEditEventDetail,
} from './extensions/InfoMarkExtension';
...
import {
  MdEmbed,
  MD_EMBED_EDIT_EVENT,
  type MdEmbedEditEventDetail,
} from './extensions/EmbedExtension';
import {
  GalleryEmbed,
  GalleryDialog,
  GALLERY_EDIT_EVENT,
  type GalleryEditEventDetail,
  type GalleryProvider,
} from './extensions/GalleryEmbedExtension';
import {
  PhotoMap,
  PhotoMapDialog,
  PHOTOMAP_EDIT_EVENT,
  type PhotoMapEditEventDetail,
} from './extensions/PhotoMapExtension';
...
const MdEditor: React.FC<MdEditorProps> = ({
  ...
  useEffect(() => {
    setMdViewSettings({ minimalView, fullWidth });
  }, [minimalView, fullWidth]);
  ...
  const { commandBus, scriptTemplates } = useEditorServices();
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const insertEventRef = useRef<((editor: any, range: any) => void) | undefined>((editor, range) =>
    setEventDialog({ editor, range })
  );

  const scriptTemplatesRef = useRef<(() => ScriptTemplate[]) | undefined>(undefined);
  scriptTemplatesRef.current = scriptTemplates ? () => scriptTemplates.list() : undefined;
  const insertTaskCardRef = useRef<((editor: any, range: any) => void) | undefined>((editor) =>
    setTaskCardDialog({ editor })
  );

  // Block action menu — one button per block
  const [blockPositions, setBlockPositions] = useState<
    Array<{ el: HTMLElement; top: number; left: number; pos: number }>
  >([]);
  ...
  const onPenPointerEnd = useCallback(() => {
    penScrollRef.current = null;
  }, []);
  ...
  const [spellLanguage, setSpellLanguage] = useState<string>(() => {
    try {
      return localStorage.getItem('mdeditor_spell_lang') || 'pl';
    } catch {
      return 'pl';
    }
  });
  const [spellEnabled, setSpellEnabled] = useState<boolean>(() => {
    try {
      return localStorage.getItem('mdeditor_spell_enabled') !== '0';
    } catch {
      return true;
    }
  });
  ...
  useEffect(() => {
    try {
      localStorage.setItem('mdeditor_spell_lang', spellLanguage);
    } catch {
      /* full storage */
    }
  }, [spellLanguage]);
  useEffect(() => {
    try {
      localStorage.setItem('mdeditor_spell_enabled', spellEnabled ? '1' : '0');
    } catch {
      /* full storage */
    }
  }, [spellEnabled]);
  ...
  const editor = useEditor({
    extensions: [
      ...
      SlashCommands.configure({
        createPageRef: onCreatePageRef,
        insertEventRef,
        insertTaskCardRef,
        // The slash menu is built inside TipTap's suggestion config, which
        // is not a component — hence a ref rather than the hook.
        scriptTemplatesRef,
      }),
      EventBlock,
      TaskCard,
    ...
    onBlur: () => {
      setTimeout(() => {
        if (
          !document.activeElement?.closest('.md-editor-bubble-menu') &&
    ...
    (type: 'person' | 'task' | 'project') => {
      ...
      (
        editor.chain().focus() as unknown as {
          insertComponentEmbed: (t: string, id: string) => { run: () => void };
        }
      )
        .insertComponentEmbed(type, '')
        .run();
  ...
  useEffect(() => {
    ...
    const handler = (e: MouseEvent) => {
      ...
      try {
        replacements = JSON.parse(span.dataset.matchReplacements || '[]');
      } catch {
        replacements = [];
      }
  ...
  useEffect(() => {
    ...
    console.log(
      ...
      dom.spellcheck,
      'lang=',
      JSON.stringify(dom.lang),
      'attrSpellcheck=',
      JSON.stringify(dom.getAttribute('spellcheck')),
      'attrLang=',
      JSON.stringify(dom.getAttribute('lang')),
      'contenteditable=',
      JSON.stringify(dom.getAttribute('contenteditable')),
      'tagName=',
      dom.tagName
    );
    ...
    const handler = (e: MouseEvent) => {
      ...
      if (/^[a-z][a-z0-9+.-]*:/i.test(rawHref)) {
        try {
          ...
          if (onLinkClickRef.current)
            onLinkClickRef.current(url.pathname + url.hash, { fromFileRef });
        } catch {
          window.open(rawHref, '_blank', 'noopener,noreferrer');
        }
        return;
      ...
      const headingText = (h: Element): string =>
        Array.from(h.childNodes)
          .filter(
            (n) => !(n.nodeType === 1 && (n as HTMLElement).classList?.contains('md-fold-toggle'))
          )
          .map((n) => n.textContent || '')
          .join('')
          .trim();
      const findTarget: () => HTMLElement | null = rawHref.startsWith('#^')
        ? () =>
            (Array.from(
              editorDom.querySelectorAll(`[data-block-id="${CSS.escape(rawHref.slice(2).trim())}"]`)
            ).find((el) => !el.closest('.md-embed')) as HTMLElement | undefined) ?? null
        : () => {
            const anchorText = rawHref.slice(1).trim();
            return (
              (Array.from(editorDom.querySelectorAll('h1,h2,h3,h4,h5,h6'))
      ...
      if (rawHref.startsWith('#')) {
        ...
        const ensureVisible = () => {
          ...
          if (!tgt) return;
          if (!container || !container.contains(tgt)) {
  ...
  const toggleGroupMode = useCallback(() => {
    setGroupMode((m) => {
      if (m) setSelectedBlockIds(new Set());
      return !m;
    });
  }, []);
  ...
  const blockKey = (blockId: string | null | undefined, pos: number): string =>
    blockId ?? `@${pos}`;
  ...
  const groupSelectedNodes = useCallback((): { pos: number; node: PmNode }[] => {
    ...
    editor.state.doc.forEach((node, offset) => {
      if (selectedBlockIds.has(blockKey(node.attrs.blockId as string | undefined, offset)))
  ...
  const groupSelectAll = useCallback(() => {
    ...
    editor.state.doc.forEach((node, offset) => {
      keys.add(blockKey(node.attrs.blockId as string | undefined, offset));
    });
  ...
  const groupDelete = useCallback(() => {
    ...
    nodes
      ...
      .forEach(({ pos, node }) => tr.delete(pos, pos + node.nodeSize));
    ...
    if (editor) {
      editor.state.doc.descendants((node) => {
        if (node.type.name === 'heading')
          hs.push({ level: (node.attrs.level as number) ?? 1, text: node.textContent });
      });
    }
  ...
  const handleInsertInternalLink = useCallback(
    (target: { path?: string; anchor?: string; anchorType?: 'heading' | 'block' }) => {
      if (!editor) return;
    ...
    if (editor) {
      editor.state.doc.descendants((n) => {
        if (n.type.name === 'heading')
          hs.push({ level: (n.attrs.level as number) ?? 1, text: n.textContent });
      });
    }
    setCurrentHeadings(hs);
  ...
  const embedTargetFrom = useCallback(
    (target: { path?: string; anchor?: string; anchorType?: 'heading' | 'block' }) => {
      const { path, anchor, anchorType } = target;
      const anchorSuffix = anchor ? (anchorType === 'block' ? `#^${anchor}` : `#${anchor}`) : '';
    ...
    (target: { path?: string; anchor?: string; anchorType?: 'heading' | 'block' }) => {
      ...
      if (typeof pos === 'number') {
        editor
          ...
          .focus()
          .command(({ tr }) => {
            tr.setNodeAttribute(pos, 'target', inner);
            return true;
          })
          .run();
      } else {
        editor
          .chain()
          .focus()
          .insertContent({ type: 'mdEmbed', attrs: { target: inner } })
          .run();
      }
  ...
  const [fileDialog, setFileDialog] = useState<{ open: boolean; path: string }>({
    open: false,
    path: '',
  });
  const [envDialog, setEnvDialog] = useState<{ open: boolean; name: string }>({
    open: false,
    name: '',
  });
  ...
  const insertFileRef = useCallback(() => {
    ...
    if (editor && path) {
      editor
        ...
        .insertContent({
          type: 'fileRef',
          attrs: { path, format: /\.json$/i.test(path) ? 'json' : 'auto' },
        })
        .run();
    }
  ...
  const insertEnvValue = useCallback(() => {
    ...
    if (editor && name)
      editor.chain().focus().insertContent({ type: 'envValue', attrs: { name } }).run();
    setEnvDialog({ open: false, name: '' });
  }, [editor, envDialog.name]);

  const handleInsertGallery = useCallback(
    (v: { provider: GalleryProvider; source: string; selected: string }) => {
      if (!editor) {
      ...
      if (typeof pos === 'number') {
        ...
        editor
          ...
          .insertContent({
            type: 'galleryEmbed',
            attrs: { provider: v.provider, source: v.source, selected: v.selected },
          })
          .run();
      }
      setGalleryDialog({ open: false });
    ...
    const onEdit = (e: Event) => {
      ...
      if (d)
        setGalleryDialog({
          open: true,
          pos: d.pos,
          initial: { provider: d.provider, source: d.source, selected: d.selected },
        });
    };
  ...
  const [photoMapDialog, setPhotoMapDialog] = useState<{
    open: boolean;
    pos?: number;
  ...
  const handleInsertPhotoMap = useCallback(
    (config: string) => {
      if (!editor) {
      ...
      if (typeof pos === 'number') {
        editor
          ...
          .focus()
          .command(({ tr }) => {
            tr.setNodeAttribute(pos, 'config', config);
            return true;
          })
          .run();
      } else {
  ...
  const handleCopyExport = useCallback(async () => {
    ...
    try {
      ...
      /* fall through — permission denied / not focused */
    }
  ...
  const handleEditorMouseOver = useCallback(
    (e: React.MouseEvent) => {
      ...
      const linkElement =
        rawLink && !rawLink.closest('.leaflet-container') && !rawLink.closest('.md-cadview-embed')
          ? rawLink
          : null;
              ...
              if (linkMark) {
                ...
                while (from > 0) {
                  ...
                  if (
                    !$from
                      .marks()
                      .some((m) => m.type.name === 'link' && m.attrs.href === linkMark.attrs.href)
                  ) {
                    break;
                  }
                  from--;
  ...
  useEffect(() => {
    if (!editor) return;
    // MyCastle reached for a global event bus in `web-client` with a dynamic
    // import, to dodge a circular dependency. A package cannot have somebody
    // else's global, so the host hands in a subscription; with none, nothing
    // outside drives the editor, which is the normal case.
    if (!commandBus) return;
    let unsub: (() => void) | null = null;
    Promise.resolve().then(() => {
      unsub = commandBus.on('mde:command', ({ type }) => {
        switch (type) {
          case 'bold':
  ...
  return (
    <>
        ...
        >
          ...
          >
            <DialogTitle sx={{ display: 'flex', alignItems: 'center', gap: 1, py: 1 }}>
              ...
              <IconButton size="small" onClick={() => setExportMd(null)}>
            ...
            <DialogContent dividers>
              <TextField
                ...
                InputProps={{
                  readOnly: true,
                  sx: { fontFamily: 'monospace', fontSize: 13, alignItems: 'flex-start' },
                }}
                onFocus={(e) => e.currentTarget.querySelector('textarea')?.select?.()}
              />
              {exportManualHint && (
                <Typography variant="caption" color="warning.main" sx={{ display: 'block', mt: 1 }}>
                  Nie udało się skopiować automatycznie. Tekst został zaznaczony — przytrzymaj i
                  wybierz „Kopiuj".
                </Typography>
              )}
            </DialogContent>
          ...
          {showBubbleMenu &&
            ...
            ReactDOM.createPortal(
              ...
              top level because it's per-character / per-block frequent. */}
                ...
                <IconButton
                  size="small"
                  onClick={openInsertInfoMarkDialog}
                  title="Wyróżnienie z opisem"
                >
                ...
                <IconButton
                  ...
                  title="Wstaw emoji / znak specjalny"
                >
          ...
          >
            {editor && <FormatPanel editor={editor} onClose={() => setFormatPanelAnchor(null)} />}
          </Popover>
          <Popover
            ...
            transformOrigin={{ vertical: 'bottom', horizontal: 'center' }}
          >
            {editor && <ColorPanel editor={editor} onClose={() => setColorPanelAnchor(null)} />}
          </Popover>
          <Popover
          ...
          <Popover
            ...
            transformOrigin={{ vertical: 'bottom', horizontal: 'center' }}
          >
            {editor && <InsertPanel editor={editor} onClose={() => setInsertPanelAnchor(null)} />}
          </Popover>
          ...
          >
            ...
            <MenuItem
              onClick={() => {
                setExtraMenuAnchor(null);
                void handlePasteMarkdownFromClipboard();
              }}
            >
              <ListItemIcon>
            ...
            >
              ...
              <ListItemText
                primary="Dyktuj zaznaczony tekst"
                secondary="Lub cały dokument gdy nic nie zaznaczone"
              />
            </MenuItem>
          </Menu>
          ...
          >
            ...
            >
              ...
              >
                <IconButton size="small" onClick={handleOpenLink} disabled={!linkUrl}>
                  <OpenInNewIcon fontSize="small" />
                </IconButton>
              </Tooltip>
              <Tooltip title="Remove link">
                <IconButton size="small" onClick={handleRemoveLink} color="error">
          ...
          >
            ...
            {isTodayJournal && (
              <TodayNowMarker containerRef={contentWrapperRef} layoutTick={contentTick} />
            )}
          </Box>
        </Box>
      </MdEnvProvider>
      ...
      {spellPopover && editor && (
        ...
        >
          <Box sx={{ p: 1.5, pb: 0.5 }}>
            ...
            >
              {spellPopover.match.category === 'TYPOS'
                ? 'BŁĄD PISOWNI'
                : spellPopover.match.category === 'GRAMMAR'
                  ? 'GRAMATYKA'
                  : spellPopover.match.category === 'PUNCTUATION'
                    ? 'INTERPUNKCJA'
          ...
          {spellPopover.match.replacements.length > 0 && (
            <List
              dense
              sx={{ borderTop: 1, borderColor: 'divider', maxHeight: 200, overflow: 'auto' }}
            >
        ...
        ReactDOM.createPortal(
          <>
            {blockPositions.map(({ top, left, pos }, i) => {
              ...
              const key =
                (editor?.state.doc.nodeAt(pos)?.attrs.blockId as string | undefined) ?? `@${pos}`;
              return (
                <Checkbox
                  key={i}
                  size="small"
        ...
        ReactDOM.createPortal(
          ...
          >
            ...
            <Tooltip title="Wklej za blokiem z kursorem">
              <IconButton size="small" onClick={() => void groupPaste()}>
            ...
            <Tooltip title="Zaznacz wszystko">
              <IconButton size="small" onClick={groupSelectAll}>
                <SelectAllIcon fontSize="small" />
              </IconButton>
            </Tooltip>
            <Button size="small" onClick={toggleGroupMode}>
              Gotowe
            </Button>
          </Paper>,
          document.body
      ...
      <Dialog
        ...
        onClose={() => setFileDialog({ open: false, path: '' })}
        maxWidth="sm"
        fullWidth
      >
        ...
        <DialogActions>
          ...
          <Button variant="contained" onClick={insertFileRef} disabled={!fileDialog.path.trim()}>
            Wstaw
          </Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog
        open={envDialog.open}
        onClose={() => setEnvDialog({ open: false, name: '' })}
        maxWidth="xs"
        fullWidth
      >
        ...
        <DialogActions>
          ...
          <Button variant="contained" onClick={insertEnvValue} disabled={!envDialog.name.trim()}>
            Wstaw
          </Button>
        </DialogActions>
      </Dialog>
      ...
      {taskCardDialog && (
        <TaskCardDialog
          ...
          onPick={(attrs) => {
            taskCardDialog.editor.chain().focus().insertContent({ type: 'taskCard', attrs }).run();
          }}
        />
      )}
