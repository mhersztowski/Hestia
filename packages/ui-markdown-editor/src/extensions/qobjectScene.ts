...
export function normalizeScene(raw: unknown): QObjectScene {
  ...
  return {
    type: 'qobject-scene',
    version: 1,
    roots: Array.isArray(r?.roots) ? r!.roots!.map(fix) : [],
  };
}
...
export function removeNode(
  scene: QObjectScene,
  id: string
): { scene: QObjectScene; removed: QObjectSceneNode | null } {
  const next = cloneScene(scene);
...
export function updateNode(
  scene: QObjectScene,
  id: string,
  mutate: (n: QObjectSceneNode) => void
): QObjectScene {
  const next = cloneScene(scene);
