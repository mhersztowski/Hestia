...
export interface GalleryEditEventDetail {
  pos: number;
  provider: string;
  source: string;
  selected: string;
...
const GalleryNodeView: React.FC<NodeViewProps> = ({ node, editor, getPos, deleteNode }) => {
  ...
  useEffect(() => {
    ...
    loadGallery(provider, source)
      ...
      .catch((e) => {
        if (alive) setError((e as Error).message || 'Nie udało się wczytać galerii.');
      })
      .finally(() => {
  ...
  const handleEdit = useCallback(() => {
    ...
    window.dispatchEvent(
      new CustomEvent<GalleryEditEventDetail>(GALLERY_EDIT_EVENT, {
        detail: { pos, provider, source, selected },
      })
    );
          ...
          <Box className="md-gallery-body" contentEditable={false}>
            {loading && items === null ? (
              ...
              <Typography variant="body2" color="text.secondary" sx={{ p: 1 }}>
                <em>Brak zdjęć (sprawdź czy link jest publiczny).</em>
              </Typography>
            )}
          </Box>
        )}
...
export const GalleryEmbed = Node.create({
  ...
  addAttributes() {
    return {
      provider: {
        default: 'immich',
        parseHTML: (el) => el.getAttribute('data-provider') || 'immich',
        renderHTML: (a) => ({ 'data-provider': a.provider }),
      },
      source: {
        default: '',
      ...
      selected: {
        default: '',
        parseHTML: (el) => el.getAttribute('data-selected') || '',
        renderHTML: (a) => (a.selected ? { 'data-selected': a.selected } : {}),
      },
    };
  },

  parseHTML() {
    return [{ tag: 'div[data-type="gallery-embed"]' }];
  },

  renderHTML({ HTMLAttributes }) {
    return ['div', mergeAttributes(HTMLAttributes, { 'data-type': 'gallery-embed' })];
  },
