...
const TableViewNodeView: React.FC<NodeViewProps> = ({ node, editor, updateAttributes }) => {
  ...
  const rows = useMemo(
    () => resolveRows(env.get, cfg.source.env),
    [env, cfg.source.env, env.version]
  );
      ...
      {editor.isEditable && (
        <Box className="md-tableview-toolbar" contentEditable={false}>
          <Tooltip title="Źródło danych (env)">
            <IconButton
              size="small"
              className="md-tableview-btn"
              onClick={(e) => setDsAnchor(e.currentTarget)}
            >
          ...
          <Tooltip title="Wygląd (kolumny, wysokość wiersza)">
            <IconButton
              size="small"
              className="md-tableview-btn"
              onClick={(e) => setDispAnchor(e.currentTarget)}
            >
      ...
      <Box className="md-tableview-scroll" contentEditable={false}>
        {columns.length === 0 ? (
          ...
          <table className="md-tableview-table">
            <colgroup>
              {columns.map((c, i) => (
                <col key={i} style={{ width: widthStyle(c) }} />
              ))}
            </colgroup>
            <thead>
              <tr>
              ...
              {rows.length === 0 ? (
                <tr style={{ height: cfg.rowHeight }}>
                  <td colSpan={columns.length} className="md-tableview-norows">
                    {cfg.source.env
                      ? `Brak danych w env „${cfg.source.env}"`
                      : 'Nie ustawiono źródła danych'}
                  </td>
                </tr>
              ) : (
                rows.map((row, ri) => (
      ...
      >
        <Box sx={{ p: 1.5, width: 340 }} contentEditable={false}>
          ...
          <TextField
            fullWidth
            size="small"
            label="Zmienna env (tablica obiektów)"
        ...
        <Box sx={{ p: 1.5, width: 440 }} contentEditable={false}>
            ...
            {cfg.columns.map((col, i) => {
              ...
              return (
                <Box key={i} sx={{ display: 'flex', gap: 0.5, alignItems: 'center' }}>
                  ...
                  <IconButton
                    size="small"
                    onClick={() => save({ ...cfg, columns: cfg.columns.filter((_, j) => j !== i) })}
                  >
          ...
          <Button
            ...
            onClick={() =>
              save({
                ...cfg,
                columns: [...cfg.columns, { key: '', label: 'Kolumna', width: 0, widthUnit: '%' }],
              })
            }
          >
            Dodaj kolumnę
...
export const TableView = Node.create({
  ...
  parseHTML() {
    return [{ tag: 'div[data-type="table-view"]' }];
  },
  renderHTML({ HTMLAttributes }) {
    return ['div', mergeAttributes(HTMLAttributes, { 'data-type': 'table-view' })];
  },
  addNodeView() {
    return ReactNodeViewRenderer(TableViewNodeView);
  },
});
