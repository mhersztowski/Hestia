...
import { DEFAULT_TASK_STATUSES } from '@hestia/core';
...
function TaskCardNodeView({ node, updateAttributes, deleteNode }: NodeViewProps) {
  ...
  const openTask = userName
    ? (id: string) =>
        navigate(
          `/user/${encodeURIComponent(userName)}/pim/projects2` + `?task=${encodeURIComponent(id)}`
        )
    : undefined;

  const status = task?.status ? DEFAULT_TASK_STATUSES.find((s) => s.id === task.status) : undefined;
  ...
  const overdue =
    task?.dueDate && status?.kind !== 'done' && new Date(task.dueDate).getTime() < Date.now();
    ...
    <NodeViewWrapper data-task-card>
      ...
      >
        ...
        <Box sx={{ flex: 1, minWidth: 0 }}>
          ...
          {projectName && (
            <Typography sx={{ fontSize: 11, opacity: 0.65, mt: 0.25 }}>{projectName}</Typography>
          )}
          ...
          <Stack direction="row" sx={{ gap: 1.5, mt: 0.75, flexWrap: 'wrap' }}>
            {priority && (
              <Stack direction="row" sx={{ alignItems: 'center', gap: 0.35 }}>
                <FlagIcon sx={{ fontSize: 13, color: priority.color }} />
                <Typography sx={{ fontSize: 11, opacity: 0.8 }}>{priority.label}</Typography>
              </Stack>
            )}
            {task?.dueDate && (
              <Stack direction="row" sx={{ alignItems: 'center', gap: 0.35 }}>
                <EventIcon
                  sx={{ fontSize: 13, color: overdue ? '#e5484d' : undefined, opacity: 0.8 }}
                />
                <Typography
                  sx={{
                    fontSize: 11,
            ...
            {(trackedMinutes > 0 || estimateMinutes > 0) && (
              <Stack direction="row" sx={{ alignItems: 'center', gap: 0.35 }}>
                <ScheduleIcon sx={{ fontSize: 13, opacity: 0.8 }} />
          ...
          {(waitsFor.length > 0 || blocks.length > 0) && (
            <Stack sx={{ gap: 0.35, mt: 0.75 }}>
              <Dependencies
                label="Czeka na"
                icon={<ArrowBackIcon sx={{ fontSize: 12, opacity: 0.7 }} />}
                tasks={waitsFor}
                onOpen={openTask}
              />
              <Dependencies
                label="Blokuje"
                icon={<ArrowForwardIcon sx={{ fontSize: 12, opacity: 0.7 }} />}
                tasks={blocks}
                onOpen={openTask}
              />
            </Stack>
...
const Dependencies: React.FC<{
  ...
  return (
    <Stack direction="row" sx={{ alignItems: 'flex-start', gap: 0.5, flexWrap: 'wrap' }}>
      {icon}
      <Typography sx={{ fontSize: 11, opacity: 0.6, flexShrink: 0 }}>{label}:</Typography>
