...
export function KnowledgeCatalog({ index, bodies = {}, onOpen, active }: KnowledgeCatalogProps) {
  ...
  const hits = useMemo(
    () => (query.trim() ? search(index, query, bodies) : []),
    [index, query, bodies]
  );
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        <div style={label}>
          {query.trim()
            ...
            : tag
              ? `temat: ${tag}`
              : 'wszystkie dokumenty w kolejności nauki'}
        </div>
            ...
            >
              <div style={{ display: 'flex', gap: 8, alignItems: 'baseline', flexWrap: 'wrap' }}>
                <strong style={{ fontSize: 13, color: '#0f172a' }}>
                  {document.meta.title ?? document.path}
                </strong>
              ...
              {document.meta.requires.length > 0 && (
                <div style={{ ...label, marginTop: 2 }}>
                  wymaga: {document.meta.requires.join(', ')}
                </div>
              )}
                ...
                <div style={{ marginTop: 4, display: 'flex', flexDirection: 'column', gap: 2 }}>
                  {hit.matches.slice(0, 3).map((match, i) => (
                    <div key={i} style={{ fontSize: 11, color: '#475569' }}>
                      <span style={{ color: '#7c3aed' }}>{MATCH_LABEL[match.kind]}:</span>{' '}
                      {match.detail}
                    </div>
                  ))}
                </div>
