...
export function createGitPlugin(opcje: GitPluginOptions): IPlugin {
  ...
  function PanelKontroliZrodel() {
    ...
    const api = zbudujApi();
    const otworzRoznice = useCallback(
      async (path: string) => {
        if (!api) return;
        try {
    ...
    const otworzKonflikt = useCallback(
      async (path: string) => {
        if (!api) return;
    ...
    return (
      <GitPanel
        api={api}
        onOpenDiff={(p) => {
          void otworzRoznice(p);
        }}
        onOpenConflict={(p) => {
          void otworzKonflikt(p);
        }}
  ...
  let otwarcieRoznicy:
    ((o: { path: string; original: string; modified: string; api: GitApi }) => void) | null = null;
  let otwarcieKonfliktu: ((o: { path: string; content: string; api: GitApi }) => void) | null =
    null;
  ...
  return {
    ...
    async activate(api: IPluginAPI) {
          ...
          component: () => (
            <ConflictTab
              ...
              onResolve={async (tresc) => {
                ...
                if (wynik.ok === false)
                  throw new Error(wynik.stderr || 'nie udało się oznaczyć jako rozwiązany');
                odswiezPanel();
              }}
            />
          ),
      ...
      api.commands.register('revertHunk', async () => {
        ...
        if (
          !window.confirm(
            `Cofnąć zmianę w wierszach ${trafiony.newRange.from}–${trafiony.newRange.to}?`
          )
        )
          return;
      ...
      api.ui.commandpalette.register({
        command: 'mycastle.git.openPanel',
        title: 'Git: pokaż zmiany',
        category: 'Git',
      });
      api.ui.commandpalette.register({
        command: 'mycastle.git.refresh',
        title: 'Git: odśwież stan',
        category: 'Git',
      });
      api.ui.commandpalette.register({
        command: 'mycastle.git.history',
        title: 'Git: historia commitów',
        category: 'Git',
      });
      api.ui.commandpalette.register({
        command: 'mycastle.git.revertHunk',
        title: 'Git: cofnij zmianę pod kursorem',
        category: 'Git',
      });
      ...
      async function odswiezMargines(uri: string, tekst: string): Promise<void> {
        ...
        let oryginal: string;
        try {
          oryginal = await gitApi.show(wzgledna, 'HEAD');
        } catch {
          return;
        }

        const zmiany = policzZmiany(oryginal, tekst);
        dekoracje = edytor.createDecorationsCollection(
          zmiany.map((z) => ({
      ...
      api.editor.onDidOpenDocument((uri, tekst) => {
        void odswiezMargines(uri, tekst);
      });
      api.editor.onDidSaveDocument(() => {
        void odswiezStatus();
        odswiezPanel();
      });
