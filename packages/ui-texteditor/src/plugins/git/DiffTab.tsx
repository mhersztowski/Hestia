...
function DiffTabWnetrze({
  ...
  const hunki = useMemo(
    () => toHunks(diffLines(tresc.original, tresc.modified), 3),
   ...
   */
  const latkaDla = useCallback(
    (index: number): string =>
      formatPatch(path, [hunki[index]], {
  ...
  const nalozLatke = useCallback(
    async (index: number, opts: { cached?: boolean; reverse?: boolean }) => {
                ...
                <span>
                  <Button
                    ...
                    onClick={() => {
                      ...
                      if (
                        window.confirm('Cofnąć ten fragment? Tej operacji nie da się odwrócić.')
                      ) {
