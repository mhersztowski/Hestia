...
import {
    hasConflictMarkers, parseConflicts, resolveAll, resolveConflict,
    type Resolution,
} from './conflicts';
    ...
    return (
        <Stack sx={{ height: '100%', minHeight: 0 }}>
            <Stack direction="row" spacing={1} alignItems="center" sx={{ px: 1.5, py: 1, borderBottom: '1px solid', borderColor: 'divider' }}>
                ...
                <Chip size="small" label={`${konflikty.length} do rozstrzygnięcia`}
                    color={konflikty.length ? 'warning' : 'success'} />
                <Box sx={{ flex: 1 }} />
                {konflikty.length > 1 && OPISY.map(({ wybor, etykieta }) => (
                    <Button key={wybor} size="small" sx={{ fontSize: 11, textTransform: 'none' }}
                        onClick={() => setTresc((b) => resolveAll(b, wybor))}>
                        Wszystkie: {etykieta.toLowerCase()}
                    </Button>
                ))}
                <Button size="small" variant="contained" disabled={zostalyZnaczniki || zapisuje}
                    onClick={() => { void zapisz(); }} sx={{ textTransform: 'none' }}>
                    {zapisuje ? 'Zapisuję…' : 'Oznacz jako rozwiązany'}
                </Button>
            </Stack>

            {blad && <Alert severity="error" sx={{ borderRadius: 0 }}>{blad}</Alert>}
            {zostalyZnaczniki && konflikty.length === 0 && (
                ...
                <Alert severity="warning" sx={{ borderRadius: 0, fontSize: 12 }}>
                    W pliku zostały znaczniki konfliktu, ale nie tworzą pełnego bloku.
                    Popraw je w edytorze — zapis jest zablokowany, żeby nie trafiły do repozytorium.
                </Alert>
            )}
            {!zostalyZnaczniki && (
            ...
            <Box sx={{ flex: 1, minHeight: 0, overflowY: 'auto', p: 1.5 }}>
                {konflikty.map((k, i) => (
                    <Paper key={`${k.startLine}-${i}`} variant="outlined" sx={{ mb: 2, overflow: 'hidden' }}>
                        <Stack direction="row" spacing={1} alignItems="center" sx={{ px: 1, py: 0.5, bgcolor: 'action.hover' }}>
...
function StronaKonfliktu({ etykieta, wiersze, kolor }: { etykieta: string; wiersze: string[]; kolor: string }) {
    return (
        <Box sx={{ flex: 1, minWidth: 0, p: 1 }}>
            <Typography variant="caption" sx={{ color: kolor, fontWeight: 600 }}>{etykieta}</Typography>
            <Box component="pre" sx={{
                m: 0, mt: 0.5, fontSize: 11, fontFamily: 'monospace',
