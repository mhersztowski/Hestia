...
function GitPanelWnetrze({
  ...
  useEffect(() => {
    void odswiez();
  }, [odswiez, refreshToken]);

  /** Operacja z blokadą przycisków i odświeżeniem po zakończeniu. */
  const wykonaj = useCallback(
    async (etykieta: string, akcja: () => Promise<unknown>) => {
      setZajety(etykieta);
      setBlad(null);
      try {
        const wynik = (await akcja()) as
          { ok?: boolean; output?: string; stderr?: string } | undefined;
        ...
        <Alert severity="info">
          {znacznik ? (
            <>
              ...
              <code> {api.repoPath}</code> i użyj „Klonuj", albo zacznij historię tutaj przyciskiem
              „Utwórz repozytorium".
            </>
          ) : (
            <>
              <code>{api.repoPath === '.' ? '/' : api.repoPath}</code> nie leży w repozytorium git.
              Utwórz je tutaj, otwórz katalog wewnątrz istniejącego, albo dodaj plik{' '}
              <code>.repo.json</code> z adresem i sklonuj.
            </>
          )}
        </Alert>
  ...
  return (
    <Stack sx={{ height: '100%', minHeight: 0 }}>
      ...
      >
        ...
        <Tooltip
          title={`Pobierz i wyślij zmiany · ${syncLabel(status?.ahead ?? 0, status?.behind ?? 0)}`}
        >
          <span>
            <IconButton
              size="small"
        ...
        <IconButton size="small" onClick={(e) => setMenuWiecej(e.currentTarget)}>
      ...
      <Box sx={{ p: 1 }}>
        <TextField
          ...
          onKeyDown={(e) => {
            if ((e.ctrlKey || e.metaKey) && e.key === 'Enter' && !przeszkoda) {
              void commit();
            }
          }}
            ...
            <Box key={sekcja}>
              ...
              {pozycje.map((z) => (
                <WierszZmiany
                  ...
                  onStage={() => {
                    void wykonaj('stage', () => api.stage([z.path]));
                  }}
                  onUnstage={() => {
                    void wykonaj('unstage', () => api.unstage([z.path]));
                  }}
                  onDiscard={() => {
      ...
      <Menu open={!!menuGalezi} anchorEl={menuGalezi} onClose={() => setMenuGalezi(null)}>
        {info.branches.map((b) => (
          <MenuItem
            ...
            onClick={() => {
              setMenuGalezi(null);
              void wykonaj('checkout', () => api.checkout(b));
            }}
          >
            {b}
          </MenuItem>
      ...
      <Menu open={!!menuWiecej} anchorEl={menuWiecej} onClose={() => setMenuWiecej(null)}>
        ...
        <MenuItem
          sx={{ fontSize: 12 }}
          disabled={doPrzygotowania.length === 0}
          onClick={() => {
            setMenuWiecej(null);
            void wykonaj('stage', () => api.stage(doPrzygotowania));
          }}
        >
        ...
        <MenuItem
          sx={{ fontSize: 12 }}
          disabled={grupy.staged.length === 0}
          onClick={() => {
            setMenuWiecej(null);
            void wykonaj('unstage', () => api.unstage(grupy.staged.map((z) => z.path)));
          }}
        >
        ...
        {schowek.map((w) => (
          <MenuItem
            ...
            onClick={() => {
              setMenuWiecej(null);
              void wykonaj('stash-pop', () => api.stashPop(w.ref));
            }}
          >
    ...
    >
      ...
      {dir && (
        <Typography
          variant="caption"
          noWrap
          sx={{ fontSize: 10, color: 'text.secondary', flex: 1, ml: 0.5 }}
        >
      ...
      >
        {sekcja !== 'staged' && (
          <Tooltip title="Przygotuj do commita">
            <span>
              <IconButton size="small" disabled={zajety} onClick={onStage}>
        ...
        {sekcja === 'staged' && (
          <Tooltip title="Wycofaj z przygotowanych">
            <span>
              <IconButton size="small" disabled={zajety} onClick={onUnstage}>
