...
export function diffLines(przed: string, po: string, maxLines = 20_000): DiffLine[] {
    ...
    let j = 0;
    while (i < m && j < n) {
        if (a[i] === b[j]) { wynik.push({ op: ' ', text: a[i] }); i++; j++; }
        ...
        else if (dp[i + 1][j] >= dp[i][j + 1]) { wynik.push({ op: '-', text: a[i++] }); }
        else { wynik.push({ op: '+', text: b[j++] }); }
    }
...
export function toHunks(diff: readonly DiffLine[], context = 3): Hunk[] {
    ...
    for (const i of indeksyZmian.slice(1)) {
        ...
        if (i - doIndeksu - 1 <= context * 2) { doIndeksu = i; continue; }
...
export function formatPatch(
    path: string,
    hunki: readonly Hunk[],
    opts: { oldEndsWithNewline?: boolean; newEndsWithNewline?: boolean; totalOldLines?: number; totalNewLines?: number } = {},
): string {
    ...
    for (const h of hunki) {
        ...
        h.lines.forEach((l, i) => {
            ...
            const koniecStarego = l.op !== '+' && opts.totalOldLines !== undefined
                && h.oldStart + h.oldLines - 1 >= opts.totalOldLines && opts.oldEndsWithNewline === false;
            const koniecNowego = l.op !== '-' && opts.totalNewLines !== undefined
                && h.newStart + h.newLines - 1 >= opts.totalNewLines && opts.newEndsWithNewline === false;
