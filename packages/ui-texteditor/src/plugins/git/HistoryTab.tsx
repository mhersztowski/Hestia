...
import {
    Box, CircularProgress, Divider, List, ListItemButton, ListItemText, Stack, Typography,
} from '@mui/material';
...
function HistoryTabWnetrze({ api, onOpenDiff, file }: HistoryTabProps) {
    ...
    useEffect(() => {
        let anulowane = false;
        api.log(100, file)
            .then((l) => { if (!anulowane) setWpisy(l); })
            .catch((e) => { if (!anulowane) setBlad((e as Error).message); });
        return () => { anulowane = true; };
    }, [api, file]);

    /** Pliki zmienione w commicie — z różnicy względem rodzica. */
    const wybierz = useCallback(async (wpis: GitLogEntry) => {
        setWybrany(wpis);
        setPliki(null);
        try {
    ...
    if (blad) return <Box sx={{ p: 2 }}><Typography variant="caption" color="error">{blad}</Typography></Box>;
    if (!wpisy) return <Box sx={{ p: 2 }}><CircularProgress size={18} /></Box>;
        ...
        <Box sx={{ display: 'flex', height: '100%', minHeight: 0 }}>
            <Box sx={{ width: 340, overflowY: 'auto', borderRight: '1px solid', borderColor: 'divider' }}>
                <List dense disablePadding>
                    {wpisy.map((w) => (
                        <ListItemButton key={w.hash} selected={wybrany?.hash === w.hash}
                            onClick={() => { void wybierz(w); }} divider sx={{ py: 0.5 }}>
                            <ListItemText
                                primary={<Typography variant="body2" noWrap>{w.subject}</Typography>}
                                secondary={
                                    <Typography variant="caption" sx={{ color: 'text.secondary' }}>
                                        <code>{w.short}</code> · {w.authorName} · {new Date(w.date).toLocaleString('pl')}
                                    </Typography>
                                }
                            />
                        </ListItemButton>
            ...
            <Box sx={{ flex: 1, minWidth: 0, overflowY: 'auto' }}>
                {!wybrany ? (
                    ...
                    <Stack sx={{ p: 1.5 }} spacing={1}>
                        ...
                        <Typography variant="caption" sx={{ color: 'text.secondary' }}>
                            {wybrany.authorName} &lt;{wybrany.authorEmail}&gt; · {new Date(wybrany.date).toLocaleString('pl')}
                        </Typography>
                        <Typography variant="caption" sx={{ fontFamily: 'monospace' }}>{wybrany.hash}</Typography>
                        <Divider />
                        {!pliki ? <CircularProgress size={16} /> : pliki.length === 0 ? (
                            <Typography variant="caption" sx={{ color: 'text.secondary' }}>
                            ...
                            <List dense disablePadding>
                                {pliki.map((p) => {
                                    ...
                                    return (
                                        <ListItemButton key={p} onClick={() => onOpenDiff(p, wybrany.hash)} sx={{ py: 0.25 }}>
                                            <ListItemText
                                                primary={<Typography variant="caption">{name}</Typography>}
                                                secondary={dir ? <Typography variant="caption" sx={{ fontSize: 10, color: 'text.secondary' }}>{dir}</Typography> : null}
                                            />
                                        </ListItemButton>
                                    );
                                })}
