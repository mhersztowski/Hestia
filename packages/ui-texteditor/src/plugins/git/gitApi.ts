...
export class GitApi {
  ...
  private async wywolaj<T>(op: string, body: Record<string, unknown> = {}): Promise<T> {
    const res = await fetch(`/api/users/${encodeURIComponent(this.opcje.userName)}/git/${op}`, {
      method: 'POST',
      headers: {
  ...
  stage(paths: string[]): Promise<GitOpResult> {
    return this.wywolaj('stage', { paths });
  }
  unstage(paths: string[]): Promise<GitOpResult> {
    return this.wywolaj('unstage', { paths });
  }
  discard(paths: string[]): Promise<GitOpResult> {
    return this.wywolaj('discard', { paths });
  }

  commit(message: string): Promise<GitOpResult> {
    return this.wywolaj('commit', { message });
  }
  pull(): Promise<GitOpResult> {
    return this.wywolaj('pull');
  }
  push(): Promise<GitOpResult> {
    return this.wywolaj('push');
  }
  ...
  createBranch(name: string): Promise<GitOpResult> {
    return this.wywolaj('branch', { name });
  }
   ...
   */
  init(branch?: string): Promise<GitOpResult> {
    return this.wywolaj('init', { branch });
  }
  ...
  stashPop(ref?: string): Promise<GitOpResult> {
    return this.wywolaj('stash', { action: 'pop', ref });
  }
  stashApply(ref?: string): Promise<GitOpResult> {
    return this.wywolaj('stash', { action: 'apply', ref });
  }
  stashDrop(ref?: string): Promise<GitOpResult> {
    return this.wywolaj('stash', { action: 'drop', ref });
  }
  ...
  async conflictVersions(file: string): Promise<{ base: string; ours: string; theirs: string }> {
    return (
      await this.wywolaj<{ versions: { base: string; ours: string; theirs: string } }>('conflict', {
        file,
      })
    ).versions;
  }
  /** Oznacza pliki jako rozwiązane (dodaje do indeksu). */
  markResolved(paths: string[]): Promise<GitOpResult> {
    return this.wywolaj('resolve', { paths });
  }
  abortMerge(): Promise<GitOpResult> {
    return this.wywolaj('abort-merge');
  }
}
