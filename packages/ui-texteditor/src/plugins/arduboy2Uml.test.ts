...
describe('projekt UML arduboy2js', () => {
    it('wystawia cztery klasy biblioteki', () => {
        expect([...new Set(callables.map((c) => c.owner))].sort())
            .toEqual(['Arduboy2', 'ArduboyAudio', 'ArduboyTones', 'Sprites']);
    });

    it('ma komplet metod, na których stoi pętla gry', () => {
        for (const name of ['begin', 'nextFrame', 'clear', 'display', 'pressed', 'justPressed', 'print', 'setCursor']) {
    ...
    it('każde wywołanie zna plik źródłowy, z którego pochodzi', () => {
        ...
        for (const callable of callables) {
            expect(callable.file, callable.label).toMatch(/^packages\/core\/browser\/arduboy2js\/\w+\.ts$/);
        }
    });
