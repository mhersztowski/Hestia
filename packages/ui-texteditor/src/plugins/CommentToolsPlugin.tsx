...
import { FileType } from '@hestia/core';
...
function parseEmbeds(text: string): EmbedBlock[] {
  ...
  lines.forEach((ln, i) => {
    const ms = ln.match(
      new RegExp(`${EMBED_TAG}:START id=(\\S+) path=(.+?)(?:\\s*(?:-->|\\*/)\\s*)?$`)
    );
    if (ms) {
      open.set(ms[1], { path: ms[2].trim(), line: i });
      return;
    }
    ...
    if (me) {
      ...
      if (s) {
        out.push({ id: me[1], path: s.path, startLine: s.line, endLine: i });
        open.delete(me[1]);
      }
    }
  });
...
function scanComments(text: string, path: string, terms: string[]): Match[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (inBlock) {
      ...
      if (ci >= 0) {
        comment += line.slice(0, ci);
        inBlock = false;
        line = line.slice(ci + blockClose.length);
      } else {
        comment += line;
      }
    }
    if (!inBlock && style.block) {
      ...
      while (idx >= 0) {
        ...
        if (close >= 0) {
          comment += ' ' + line.slice(idx + bo.length, close);
          line = line.slice(close + bc.length);
          idx = line.indexOf(bo);
        } else {
          comment += ' ' + line.slice(idx + bo.length);
          inBlock = true;
          blockClose = bc;
          break;
        }
...
function CommentToolsPanel({ provider }: { provider: FileSystemProvider }) {
  ...
  const refreshFromEditor = useCallback(() => {
    ...
    const pick =
      eds.find(
        (e) =>
          e.hasTextFocus() &&
          !(e.getModel()?.uri.toString() ?? 'virtual://').startsWith('virtual://')
      ) ??
      eds.find((e) => {
        const u = e.getModel()?.uri.toString();
        return !!u && !u.startsWith('virtual://');
      });
  ...
  const matches = useMemo(
    () => (text ? scanComments(text, filePath, terms) : []),
    [text, filePath, terms]
  );
      ...
      >
        <Stack direction="row" alignItems="center" spacing={0.5}>
          ...
          <Tooltip title="Refresh from editor">
            <IconButton size="small" onClick={refreshFromEditor}>
        ...
        <Stack direction="row" alignItems="center" spacing={0.5} sx={{ mb: 0.5 }}>
          ...
          <Tooltip title="Refresh">
            <IconButton size="small" onClick={() => void loadDir(dir)}>
              <RefreshIcon fontSize="small" />
            </IconButton>
          </Tooltip>
        </Stack>
        ...
        <Typography variant="caption" color="text.secondary">
          Click a file to embed it at the cursor.
        </Typography>
        ...
        {embeds.length === 0 ? (
          <Typography variant="caption" color="text.secondary">
            No embedded files in this document.
          </Typography>
        ) : (
              ...
              <ListItemButton key={b.id} dense onClick={() => jumpToLine(uri, b.startLine)}>
                ...
                <Tooltip title="Remove embedded content">
                  <IconButton
                    ...
                    onClick={(ev) => {
                      ev.stopPropagation();
                      removeLineRange(uri, b.startLine, b.endLine);
                    }}
                  >
        ...
        <Stack direction="row" spacing={0.5} sx={{ mb: 1 }}>
          <TextField
            size="small"
            placeholder="Add marker (e.g. HACK)"
            value={newTerm}
          ...
          <Tooltip title="Add marker">
            <span>
              <IconButton size="small" onClick={addTerm} disabled={!newTerm.trim()}>
...
const ICON =
  '<svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor"><path d="M20 2H4c-1.1 0-2 .9-2 2v18l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zM8 11H6V9h2v2zm5 0h-2V9h2v2zm5 0h-2V9h2v2z"/></svg>';
  ...
  return defineEditorPlugin(
    {
      ...
      version: '1.0.0',
      description:
        'Embed VFS files into the active file and scan source comments for TODO/FIXME-style markers',
