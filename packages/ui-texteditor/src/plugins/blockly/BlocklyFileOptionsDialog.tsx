...
import { readFileOptions, writeFileOptions, type OptionsStorage } from './fileOptions';
        ...
        <DialogContent dividers>
          <Stack spacing={2}>
            ...
            {available?.length === 0 && !listError && props.umlSource && (
              <Alert severity="info">
                Nie znaleziono żadnego projektu UML. Utwórz go na stronie Programming/UML — pliki
                `*.umlproj.json` w katalogu `drive/uml`.
              </Alert>
            )}
            ...
            {/*
                        ...
                        */}
            {projects
              .filter((p) => available && !available.some((a) => a.id === p))
              .map((p) => (
                <FormControlLabel
                  key={p}
