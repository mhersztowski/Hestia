...
export function createBlocklyPlugin(options: BlocklyPluginOptions = {}): IPlugin {
  ...
  const targetOrExplain = (): { file: string; label: string } | undefined => {
    if (!activeFile) {
      api?.logger.warn(
        'Blockly: nie ma otwartego pliku, dla którego można otworzyć edytor bloczkowy.'
      );
      return undefined;
    }
  ...
  return {
    ...
    async activate(host: IPluginAPI) {
      ...
      disposables.push(
        host.editor.onDidChangeModel((uri) => {
      ...
      host.commands.register('open', () => {
        ...
        pendingTab = setTimeout(() => {
          ...
          openBlocklyTab(host, target.file, target.label, options.umlSource).catch((e: unknown) =>
            host.logger.error(`Blockly: nie udało się otworzyć edytora bloczkowego: ${String(e)}`)
          );
        }, 0);
      });
      ...
      host.commands.register('fileOptions', () => {
        ...
        showFileOptions(host, activeFile, options.umlSource).catch((e: unknown) =>
          host.logger.error(`Blockly: nie udało się otworzyć okna opcji: ${String(e)}`)
        );
      });

      disposables.push(
        host.ui.contextmenu.register({
          id: 'blockly.open',
          label: 'Blockly: edytor bloczkowy',
          group: 'blockly',
          order: 1,
      ...
      disposables.push(
        host.ui.contextmenu.register({
          id: 'blockly.fileOptions',
          label: 'Blockly: opcje pliku…',
          group: 'blockly',
          order: 2,
          command: command('fileOptions'),
        })
      );
      disposables.push(
        host.ui.commandpalette.register({
          command: command('open'),
          title: 'Otwórz edytor bloczkowy',
          category: 'Blockly',
        })
      );
      disposables.push(
        host.ui.commandpalette.register({
          command: command('fileOptions'),
          title: 'Opcje pliku (diagram UML)',
          category: 'Blockly',
        })
      );
      disposables.push(
        host.ui.toolbar.register({
          id: 'blockly.open',
          label: 'Edytor bloczkowy',
          icon: ICON,
          group: 'blockly',
          order: 1,
          command: command('open'),
