...
const cppDocs: Record<string, { signature: string; description: string; example?: string }> = {
  ...
  const: {
    signature: 'const',
    description:
      'Declares a variable as constant (immutable). Can also be used for const-correctness in member functions.',
  ...
  nullptr: {
    signature: 'nullptr',
    description:
      'Null pointer literal of type std::nullptr_t. Preferred over NULL or 0 for null pointers.',
  ...
  vector: {
    ...
    description:
      'Dynamic array that can grow and shrink. Provides random access and efficient insertion at the end.',
  ...
  weak_ptr: {
    ...
    description:
      'Non-owning reference to an object managed by shared_ptr. Prevents circular references.',
  ...
  make_shared: {
    ...
    description:
      'Creates a shared_ptr with a single memory allocation for both object and control block.',
  ...
  sort: {
    ...
    description:
      'Sorts elements in ascending order using introsort (quicksort + heapsort + insertion sort).',
  ...
  find_if: {
    signature:
      'template<class InputIt, class Pred> InputIt find_if(InputIt first, InputIt last, Pred pred)',
