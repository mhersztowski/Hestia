...
export const ProjectPanel = forwardRef<ProjectPanelHandle, ProjectPanelProps>(function ProjectPanel(
  ...
  const statusBorder =
    lastStatus === 'success' ? '#2d5a2d' : lastStatus === 'error' ? '#5a1a1a' : '#3c3c3c';
  const statusBg =
    lastStatus === 'success' ? '#1a2e1a' : lastStatus === 'error' ? '#2e1a1a' : '#1e1e1e';
  ...
  // ESC is what an ANSI colour sequence begins with — the thing being
  // stripped here.
    ...
    <>
        ...
        <div style={{ display: 'flex', alignItems: 'center', padding: '5px 8px 2px' }}>
          ...
          {outputLines.length > 0 && (
            <button
              ...
              style={iconBtnStyle(outputOpen ? '#ccc' : '#666')}
            >
      ...
      {outputOpen && outputLines.length > 0 && (
        ...
        >
          ...
          >
            <span style={{ fontSize: 10, color: '#555', flexGrow: 1, fontFamily: 'monospace' }}>
              OUTPUT
            </span>
            {running && (
