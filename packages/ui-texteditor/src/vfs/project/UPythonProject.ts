...
export class UPythonProject extends Project {
    ...
    return [
      {
        id: 'deploy',
        label: 'Deploy',
        description: 'Upload all project files to device via mpremote',
        hasOutput: true,
        shortcut: 'F5',
      },
      {
        id: 'run',
        label: 'Run',
        description: 'Deploy and execute main.py on device',
        hasOutput: true,
        shortcut: 'F8',
      },
  ...
  async execute(
    ...
    if (actionId === 'deploy' || actionId === 'run' || actionId === 'sync') {
      ...
      const done = await this.apiGetSSE<DoneEvent>(`${base}/deploy`, { port }, onOutput, signal);
