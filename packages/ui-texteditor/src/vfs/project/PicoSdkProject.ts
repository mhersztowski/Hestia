...
export class PicoSdkProject extends Project {
    ...
    return [
      {
        id: 'build',
        label: 'Build',
        description: 'Build with CMake/Ninja (SSE streaming)',
        hasOutput: true,
        shortcut: 'F7',
      },
      {
        id: 'flash',
        label: 'Flash',
        description: 'Flash UF2 to device via picotool or OpenOCD',
        hasOutput: true,
        shortcut: 'F8',
      },
  ...
  async execute(
    ...
    const boardKey =
      (this.context.boardProfileKey && BOARD_KEY[this.context.boardProfileKey]) ??
