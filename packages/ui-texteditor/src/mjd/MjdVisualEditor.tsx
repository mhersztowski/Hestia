...
type HistoryAction =
  { type: 'set'; data: Record<string, unknown> } | { type: 'undo' } | { type: 'redo' };

type CtxMenu = {
  anchor: { left: number; top: number };
  path: string[];
  submenu: 'new' | 'view' | null;
} | null;
...
function validateValue(value: unknown, schema: JSchema, path: string[] = []): ValidationError[] {
  ...
  if (schema.type) {
    ...
    const ok = types.some(
      (t) => t === vt || (t === 'integer' && typeof value === 'number' && Number.isInteger(value))
    );
    if (!ok)
      errs.push({ path, message: `Expected ${types.join('|')}, got ${vt}`, keyword: 'type' });
  }
  ...
  if (typeof value === 'string') {
    ...
    if (
      schema.format &&
      FORMAT_PATTERNS[schema.format] &&
      !FORMAT_PATTERNS[schema.format].test(value)
    )
  ...
  if (value !== null && typeof value === 'object' && !Array.isArray(value)) {
    ...
    if (schema.required)
      for (const r of schema.required)
        if (!(r in obj))
          errs.push({ path: [...path, r], message: 'Required field missing', keyword: 'required' });
    ...
    if (schema.minProperties != null && Object.keys(obj).length < schema.minProperties)
      errs.push({
        path,
        message: `Min ${schema.minProperties} properties required`,
        keyword: 'minProperties',
      });
    if (schema.maxProperties != null && Object.keys(obj).length > schema.maxProperties)
      errs.push({
        path,
        message: `Max ${schema.maxProperties} properties allowed`,
  ...
  if (Array.isArray(value)) {
    ...
    if (schema.items)
      value.forEach((item, i) =>
        errs.push(...validateValue(item, schema.items!, [...path, String(i)]))
      );
  }
  return errs;
}
...
function autoLayout(
  ...
  keys.forEach((k, i) => {
    r[k] = { x: (i % cols) * (colW + 24), y: Math.floor(i / cols) * (rowH + 24) };
  });
  return r;
}
function defaultForType(t: string): unknown {
  return (
    { string: '', number: 0, integer: 0, boolean: false, object: {}, array: [], null: null }[t] ??
    ''
  );
}
...
const TYPE_COLORS_DARK: ColorMap = {
  string: '#ce9178',
  number: '#b5cea8',
  integer: '#b5cea8',
  boolean: '#569cd6',
...
const TYPE_COLORS_LIGHT: ColorMap = {
  string: '#a31515',
  number: '#098658',
  integer: '#098658',
  boolean: '#0070c1',
...
function typeColors(dark: boolean): ColorMap {
  return dark ? TYPE_COLORS_DARK : TYPE_COLORS_LIGHT;
}
...
function NodeShell({
  ...
  return (
    ...
    >
      ...
      {/* Body */}
      <Box sx={{ px: 1.5, py: 0.5, maxHeight: 280, overflowY: 'auto' }}>{children}</Box>
      {/* Validation errors */}
      {errors.length > 0 && (
        <Box
          sx={{
            ...
            borderTop: `1px solid ${alpha('#f44336', 0.3)}`,
          }}
        >
...
function DataNodeComponent({ data, selected }: NodeProps<DataFlowNode>) {
  ...
  const renderVal = (v: unknown) => {
    ...
    if (t === 'object')
      return (
        <Box
          component="span"
          sx={{ color: tc.object, fontFamily: 'monospace', fontSize: 11 }}
        >{`{ ${Object.keys(v as object).length} props }`}</Box>
      );
    if (t === 'array')
      return (
        <Box
          component="span"
          sx={{ color: tc.array, fontFamily: 'monospace', fontSize: 11 }}
        >{`[ ${(v as unknown[]).length} items ]`}</Box>
      );
    if (t === 'string')
      return (
        <Box
          component="span"
          sx={{ color: c, fontFamily: 'monospace', fontSize: 11 }}
        >{`"${String(v).slice(0, 40)}"`}</Box>
      );
    return (
      <Box component="span" sx={{ color: c, fontFamily: 'monospace', fontSize: 11 }}>
        {String(v)}
      </Box>
    );
  };
  ...
  const body = rows ? (
    <>
        ...
        >
          ...
          {editField === k ? (
            <TextField
              ...
              sx={{
                flex: 1,
                '& .MuiInputBase-input': { fontSize: 11, fontFamily: 'monospace', py: 0.25 },
              }}
            />
          ) : (
            <Box
      ...
      {jtype === 'array' && (value as unknown[]).length > 8 && (
        <Typography sx={{ fontSize: 10, color: 'text.disabled', py: 0.5 }}>
          +{(value as unknown[]).length - 8} more…
        </Typography>
      )}
    </>
  ) : editField === '__prim' ? (
    <TextField
      ...
      autoFocus
      fullWidth
      multiline={jtype === 'string'}
      onChange={(e) => setEditVal(e.target.value)}
      onBlur={() => commitEdit('__prim')}
      onKeyDown={(e) => {
        if (e.key === 'Enter' && !e.shiftKey) commitEdit('__prim');
        if (e.key === 'Escape') setEditField(null);
      }}
      sx={{ mt: 0.5, '& .MuiInputBase-input': { fontSize: 12, fontFamily: 'monospace' } }}
    />
  ) : (
    <Box sx={{ py: 0.5, cursor: 'text' }} onDoubleClick={() => startEdit('__prim', value)}>
      {renderVal(value)}
    </Box>
  );

  return (
    <NodeShell selected={!!selected} accentColor={accent} header={header} errors={errors}>
      {body}
    </NodeShell>
  );
}
...
function SchemaNodeComponent({ data, selected }: NodeProps<SchemaFlowNode>) {
  ...
  if (schema.pattern)
    constraints.push(`/${schema.pattern.slice(0, 12)}${schema.pattern.length > 12 ? '…' : ''}/`);
    ...
    <>
          ...
          return (
            ...
            >
              ...
              >
                ...
                <Typography
                  sx={{
                    fontSize: 11,
                    fontFamily: 'monospace',
                    color: isDark ? '#9cdcfe' : theme.palette.primary.dark,
                    overflow: 'hidden',
                    textOverflow: 'ellipsis',
                    whiteSpace: 'nowrap',
                  }}
              ...
              <Box sx={{ display: 'flex', gap: 0.5, flexWrap: 'wrap', flex: 1 }}>
                <Box component="span" sx={{ fontSize: 10, color: pc, fontFamily: 'monospace' }}>
                  {pt}
                </Box>
                {s.format && (
                  <Chip
        ...
        <Box sx={{ py: '3px' }}>
          <Typography sx={{ fontSize: 10, color: 'text.secondary' }}>
            ...
            <Box
              component="span"
              sx={{
                color: tc[(schema.items.type as string) ?? 'string'] ?? tc.string,
                fontFamily: 'monospace',
              }}
            >
      ...
      {(schema.oneOf || schema.anyOf || schema.allOf) && (
        <Box sx={{ py: 0.5, display: 'flex', gap: 0.5, flexWrap: 'wrap' }}>
          {schema.oneOf && (
            <Chip
              label={`oneOf(${schema.oneOf.length})`}
              size="small"
              sx={{
                height: 16,
                fontSize: 10,
                bgcolor: alpha(tc.oneOf, 0.15),
                color: tc.oneOf,
                '& .MuiChip-label': { px: 0.5 },
              }}
            />
          ...
          {schema.anyOf && (
            <Chip
              label={`anyOf(${schema.anyOf.length})`}
              size="small"
              sx={{
                height: 16,
                fontSize: 10,
                bgcolor: alpha(tc.anyOf, 0.15),
                color: tc.anyOf,
                '& .MuiChip-label': { px: 0.5 },
              }}
            />
          ...
          {schema.allOf && (
            <Chip
              label={`allOf(${schema.allOf.length})`}
              size="small"
              sx={{
                height: 16,
                fontSize: 10,
                bgcolor: alpha(tc.allOf, 0.15),
                color: tc.allOf,
                '& .MuiChip-label': { px: 0.5 },
              }}
            />
      ...
      {schema.description && (
        <Typography sx={{ fontSize: 10, color: 'text.secondary', pt: 0.5, fontStyle: 'italic' }}>
  ...
  return (
    <NodeShell selected={!!selected} accentColor={accent} header={header} errors={data.errors}>
      {body}
    </NodeShell>
  );
}
...
function Inspector({
  ...
  if (mode === 'data' || mode === 'validate') {
    ...
    const validationErrors = schema
      ? validateValue(value, schema.properties?.[selectedId] ?? {}, [selectedId])
      : [];
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        <Box sx={{ px: 1.5, py: 1, borderBottom: 1, borderColor: 'divider', flexShrink: 0 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
            ...
            <Typography
              sx={{ fontSize: 13, fontFamily: 'monospace', fontWeight: 700, color: accent }}
            >
              {selectedId}
            </Typography>
          </Box>
        ...
        <Tabs
          ...
          sx={{
            ...
            borderColor: 'divider',
            flexShrink: 0,
            '& .MuiTab-root': { fontSize: 11, minWidth: 60, py: 0.5 },
          }}
        >
          <Tab label="Value" />
          <Tab label="Schema" />
        ...
        <Box sx={{ flex: 1, overflow: 'auto', p: 1.5 }}>
              ...
              {jtype === 'boolean' && (
                <FormControlLabel
                  control={
                    <Switch
                      checked={!!value}
                      onChange={(e) => onEditData([selectedId], e.target.checked)}
                    />
                  }
                  label="Value"
                />
                    ...
                    <Box key={k} sx={{ display: 'flex', gap: 1, mb: 1, alignItems: 'flex-start' }}>
                      ...
                      <TextField
                        ...
                        onChange={(e) => {
                          ...
                          try {
                            pv = JSON.parse(e.target.value);
                          } catch {
                            /* string */
                          }
                          onEditData([selectedId], {
                ...
                <Box>
                  {(value as unknown[]).map((item, i) => (
                    <Box key={i} sx={{ display: 'flex', gap: 1, mb: 1, alignItems: 'center' }}>
                      <Typography sx={{ fontSize: 11, color: 'text.disabled', minWidth: 24 }}>
                        [{i}]
                      </Typography>
                      <TextField
                        size="small"
                  ...
                  <Button
                    size="small"
                    startIcon={<AddIcon />}
                    onClick={() => onEditData([selectedId], [...(value as unknown[]), ''])}
                  >
                    Add item
                  </Button>
                </Box>
          ...
          {tab === 1 && (
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
              ...
              <Box
                sx={{
                  bgcolor: isDark ? '#1e1e2e' : theme.palette.grey[50],
                  p: 1,
                  borderRadius: 1,
                  fontFamily: 'monospace',
                  fontSize: 11,
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
              {validationErrors.length === 0 ? (
                ...
                validationErrors.map((e, i) => (
                  ...
                  >
                    <ErrorOutlineIcon
                      sx={{ fontSize: 16, color: '#f44336', flexShrink: 0, mt: 0.25 }}
                    />
                    <Box>
                      <Typography sx={{ fontSize: 11 }}>{e.message}</Typography>
                      <Typography
                        sx={{ fontSize: 10, color: 'text.secondary', fontFamily: 'monospace' }}
                      >
                        {e.path.join(' › ')} [{e.keyword}]
                      </Typography>
                    </Box>
                  </Box>
                ))
  ...
  if (mode === 'schema') {
    ...
    const mainType = Array.isArray(nodeSchema.type)
      ? nodeSchema.type[0]
        ...
        <Box sx={{ px: 1.5, py: 1, borderBottom: 1, borderColor: 'divider', flexShrink: 0 }}>
          <Typography
            sx={{
              ...
              fontFamily: 'monospace',
              fontWeight: 700,
              color: tc[mainType] ?? tc.object,
            }}
          >
            {selectedId.startsWith('prop__')
              ? selectedId.slice(6)
              : selectedId.startsWith('def__')
                ? `$def:${selectedId.slice(5)}`
                : 'Root Schema'}
          </Typography>
        </Box>
        <Box
        ...
        >
          ...
          <FormControl size="small" fullWidth>
            ...
            <Select
              label="Type"
              value={mainType ?? ''}
              onChange={(e) => patchSchema({ type: e.target.value as JsType })}
              sx={{ fontSize: 12 }}
            >
          ...
          {mainType === 'string' && (
            <>
              <Divider>
                <Typography sx={{ fontSize: 10 }}>String</Typography>
              </Divider>
              <FormControl size="small" fullWidth>
                <InputLabel sx={{ fontSize: 12 }}>Format</InputLabel>
                <Select
                  label="Format"
                  value={nodeSchema.format ?? ''}
                  onChange={(e) => patchSchema({ format: e.target.value || undefined })}
                  sx={{ fontSize: 12 }}
                >
              ...
              <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 1 }}>
                <NumField
                  ...
                  onChange={(v) => patchSchema({ minLength: v })}
                />
                <NumField
                  label="maxLength"
                  value={nodeSchema.maxLength}
                  onChange={(v) => patchSchema({ maxLength: v })}
                />
              </Box>
              <TextField
                size="small"
            ...
            <>
              ...
              <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 1 }}>
                <NumField
                  ...
                  onChange={(v) => patchSchema({ minimum: v })}
                />
                <NumField
                  label="maximum"
                  value={nodeSchema.maximum}
                  onChange={(v) => patchSchema({ maximum: v })}
                />
                <NumField
                  label="exclusiveMin"
                  value={nodeSchema.exclusiveMinimum}
                  onChange={(v) => patchSchema({ exclusiveMinimum: v })}
                />
                <NumField
                  label="exclusiveMax"
                  value={nodeSchema.exclusiveMaximum}
                  onChange={(v) => patchSchema({ exclusiveMaximum: v })}
                />
              </Box>
              <NumField
                label="multipleOf"
                value={nodeSchema.multipleOf}
                onChange={(v) => patchSchema({ multipleOf: v })}
              />
            </>
          )}
          ...
          {mainType === 'array' && (
            <>
              <Divider>
                <Typography sx={{ fontSize: 10 }}>Array</Typography>
              </Divider>
              <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 1 }}>
                <NumField
                  label="minItems"
                  value={nodeSchema.minItems}
                  onChange={(v) => patchSchema({ minItems: v })}
                />
                <NumField
                  label="maxItems"
                  value={nodeSchema.maxItems}
                  onChange={(v) => patchSchema({ maxItems: v })}
                />
              </Box>
              <FormControlLabel
                control={
                  <Switch
                    size="small"
                    checked={!!nodeSchema.uniqueItems}
                    onChange={(e) => patchSchema({ uniqueItems: e.target.checked || undefined })}
                  />
                }
                label={<Typography sx={{ fontSize: 12 }}>uniqueItems</Typography>}
              />
              <FormControl size="small" fullWidth>
                <InputLabel sx={{ fontSize: 12 }}>Items type</InputLabel>
                <Select
                  label="Items type"
                  value={(nodeSchema.items?.type as string) ?? ''}
                  onChange={(e) =>
                    patchSchema({
                      items: e.target.value ? { type: e.target.value as JsType } : undefined,
                    })
                  }
                  sx={{ fontSize: 12 }}
                >
          ...
          {mainType === 'object' && (
            <>
              <Divider>
                <Typography sx={{ fontSize: 10 }}>Object</Typography>
              </Divider>
              <FormControlLabel
                control={
                  <Switch
                    ...
                    onChange={(e) =>
                      patchSchema({ additionalProperties: e.target.checked ? false : undefined })
                    }
                  />
                }
              ...
              <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 1 }}>
                <NumField
                  ...
                  onChange={(v) => patchSchema({ minProperties: v })}
                />
                <NumField
                  label="maxProperties"
                  value={nodeSchema.maxProperties}
                  onChange={(v) => patchSchema({ maxProperties: v })}
                />
              </Box>
              {/* Required toggles */}
                        ...
                        control={
                          <Switch
                            ...
                            onChange={(e) => {
                              ...
                              if (e.target.checked) reqs.add(k);
                              else reqs.delete(k);
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  patchSchema({
                    properties: { ...nodeSchema.properties, [pName]: { type: 'string' } },
                  });
                }}
              >
                Add property
          ...
          <>
            <Divider>
              <Typography sx={{ fontSize: 10 }}>Enum / Const</Typography>
            </Divider>
            {nodeSchema.enum ? (
              <Box>
              ...
              <Button size="small" onClick={() => patchSchema({ enum: [] })}>
                Enable enum
              </Button>
            )}
          </>
          ...
          {/* Default */}
          <Divider>
            <Typography sx={{ fontSize: 10 }}>Default</Typography>
          </Divider>
          <TextField
            size="small"
            label="Default (JSON)"
            ...
            onChange={(e) => {
              try {
                patchSchema({ default: e.target.value ? JSON.parse(e.target.value) : undefined });
              } catch {
                /* invalid JSON */
              }
            }}
...
function getTypeColors(isDark: boolean): ColorMap {
...
function TreeNode({
  ...
  return (
    <Box>
      ...
      >
        <Box
          sx={{ width: 12, flexShrink: 0, display: 'flex', alignItems: 'center' }}
          onClick={(e) => {
            e.stopPropagation();
            if (canExpand) onToggle(ps);
          }}
        >
          {canExpand ? (
            isExp ? (
...
const PALETTE_ITEMS: Array<{ type: string; label: string; icon: ReactNode; color: string }> = [
  {
    ...
    color: TYPE_COLORS_LIGHT.object,
  },
  {
    type: 'array',
    label: '[ ]',
    icon: <DataArrayIcon sx={{ fontSize: 14 }} />,
    color: TYPE_COLORS_LIGHT.array,
  },
  {
    type: 'string',
    label: '"a"',
    icon: <AbcIcon sx={{ fontSize: 14 }} />,
    color: TYPE_COLORS_LIGHT.string,
  },
  {
    type: 'number',
    label: '123',
    icon: <PinIcon sx={{ fontSize: 14 }} />,
    color: TYPE_COLORS_LIGHT.number,
  },
  {
    type: 'boolean',
    label: 't/f',
    icon: <ToggleOnIcon sx={{ fontSize: 14 }} />,
    color: TYPE_COLORS_LIGHT.boolean,
  },
  {
    type: 'null',
    label: 'null',
    icon: <RemoveIcon sx={{ fontSize: 14 }} />,
    color: TYPE_COLORS_LIGHT.null,
  },
];
            ...
            <Tooltip key={it.type} title={`Add ${it.type}`}>
              <Box
                sx={{
                  ...
                  cursor: 'pointer',
                  '&:hover': {
                    bgcolor: alpha(tc[it.type] ?? theme.palette.primary.main, 0.1),
                    borderColor: tc[it.type] ?? theme.palette.primary.main,
                  },
                }}
                onClick={() => onAddDataNode(it.type)}
              >
                <Box sx={{ color: tc[it.type] }}>{it.icon}</Box>
                <Typography sx={{ fontSize: 11, fontFamily: 'monospace', color: tc[it.type] }}>
                  {it.label}
                </Typography>
              </Box>
            </Tooltip>
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <Box sx={{ flex: 1, overflow: 'auto' }}>
            ...
            <Box key={v.id}>
              <Box
                sx={{
                  ...
                  py: '2px',
                  cursor: 'pointer',
                  bgcolor: isSel ? alpha(theme.palette.primary.main, 0.1) : 'transparent',
                  '&:hover': { bgcolor: alpha(theme.palette.primary.main, 0.06) },
                }}
                onClick={() => onSelect(v.id)}
              >
                <Box
                  sx={{ width: 14, flexShrink: 0, display: 'flex', alignItems: 'center' }}
                  onClick={(e) => {
                    e.stopPropagation();
                    setExp((p) => {
                      const n = new Set(p);
                ...
                topLevelKeys.map((k) => {
                  ...
                  return (
                    <Box
                      ...
                      onClick={() => onToggleKey(v.id, k, !vis)}
                    >
                      <Box
                        sx={{
                          width: 10,
                          height: 10,
                          border: '1px solid',
                          borderColor: vis ? 'primary.main' : 'text.disabled',
                          borderRadius: 0.25,
                          bgcolor: vis ? 'primary.main' : 'transparent',
                          flexShrink: 0,
                        }}
                      />
                      <Typography
                        sx={{
                          fontSize: 10,
                          fontFamily: 'monospace',
                          color: vis ? 'text.primary' : 'text.disabled',
                        }}
                      >
                        {k}
                      </Typography>
        ...
        {views.length === 0 && (
          <Typography
            variant="caption"
            sx={{ px: 1, py: 1, display: 'block', color: 'text.disabled' }}
          >
            No views — click + to create
          </Typography>
        )}
      </Box>
    </Box>
...
export function MjdVisualEditor({ value, onChange, height }: MjdVisualEditorProps) {
  ...
  const redo = useCallback(() => {
    ...
    dispatch({ type: 'redo' });
    onChange(hist.future[0]);
  }, [hist.future, onChange]);

  // Schema (when in schema mode, interpret data as JSchema)
  const schema: JSchema | null = useMemo(
    () => (mode === 'schema' || mode === 'validate' ? (data as JSchema) : null),
    [data, mode]
  );
  ...
  // Clipboard
  const [clipboard, setClipboard] = useState<{ path: string[]; val: unknown; cut: boolean } | null>(
    null
  );
  ...
  useEffect(() => {
    setPositions((prev) => {
      ...
      missing.forEach((k, i) => {
        extra[k] = { x: (i % 3) * 314, y: maxY + Math.floor(i / 3) * 234 };
      });
      return { ...prev, ...extra };
    });
  ...
  const visibleDataKeys = currentView
    ? topKeys.filter((k) => currentView.visibleKeys.includes(k))
    : topKeys;
  ...
  const { schemaFlowNodes, schemaEdges } = useMemo((): {
    schemaFlowNodes: SchemaFlowNode[];
    schemaEdges: Edge[];
  } => {
    ...
    nodes.push({
      ...
      data: {
        ...
        isRequired: false,
        errors: [],
        onEditSchema: (ns: JSchema) => commit(ns as Record<string, unknown>),
      },
    });
      ...
      for (const [pName, pSchema] of Object.entries(s.properties)) {
        ...
        nodes.push({
          ...
          data: {
            nodeId: id,
            label: pName,
            jsonPath: ['properties', pName],
            schema: pSchema,
            isRoot: false,
            isDef: false,
            isRequired: isReq,
            errors: [],
            onEditSchema: (ns: JSchema) =>
              commit({
                ...data,
                properties: { ...(data.properties as Record<string, unknown>), [pName]: ns },
              }),
          },
        });
        edges.push({
          ...
          style: {
            stroke: isReq ? '#f44336' : isDark ? '#4fc3f7' : theme.palette.primary.main,
            strokeDasharray: isReq ? undefined : '5,3',
          },
          label: isReq ? '●req' : undefined,
          labelStyle: { fontSize: 10, fill: '#f44336' },
        });
        i++;
      }
    }
        ...
        nodes.push({
          ...
          data: {
            ...
            onEditSchema: (ns: JSchema) =>
              commit({
                ...data,
                $defs: { ...(data.$defs as Record<string, unknown>), [defName]: ns },
              }),
          },
        });
        i++;
  ...
  useEffect(() => {
    setNodes(flowNodes as AnyFlowNode[]);
  }, [flowNodes, setNodes]);
  useEffect(() => {
    setEdges(schemaEdges);
  }, [schemaEdges, setEdges]);
  ...
  const handleTreeToggle = (ps: string) =>
    setTreeExp((p) => {
      const n = new Set(p);
      if (n.has(ps)) n.delete(ps);
      else n.add(ps);
      return n;
    });
  const handleTreeSelect = (path: string[]) => {
    setTreeSelected(path.join('\0'));
    if (path.length === 1) setSelectedId(path[0]);
  };
  ...
  const ctxAdd = (type: string) => {
    ...
    if (!path.length) commit({ ...data, [newKey]: v });
      ...
      if (t && typeof t === 'object' && !Array.isArray(t))
        commit(setAtPath(data, [...path, newKey], v));
  ...
  const ctxCut = () => {
    if (!ctxMenu) return;
    setClipboard({ path: ctxMenu.path, val: getAtPath(data, ctxMenu.path), cut: true });
    commit(deleteAtPath(data, ctxMenu.path));
    closeCtx();
  };
  const ctxCopy = () => {
    if (!ctxMenu) return;
    setClipboard({ path: ctxMenu.path, val: getAtPath(data, ctxMenu.path), cut: false });
    closeCtx();
  };
  const ctxPaste = () => {
    ...
    if (!path.length) commit({ ...data, [`pasted_${Date.now()}`]: clipboard.val });
      ...
      const t = getAtPath(data, path);
      if (t && typeof t === 'object' && !Array.isArray(t))
        commit(setAtPath(data, [...path, `pasted_${Date.now()}`], clipboard.val));
    }
    closeCtx();
  };
  const ctxDelete = () => {
    ...
    function search(obj: unknown, path: string[]) {
      ...
      if (obj && typeof obj === 'object' && !Array.isArray(obj))
        for (const [k, v] of Object.entries(obj as Record<string, unknown>)) {
          if (k.toLowerCase().includes(q)) res.push([...path, k].join(' › '));
          search(v, [...path, k]);
        }
  ...
  const handleEditSchema = (path: string[], val: unknown) => {
    if (!path.length) {
      commit(val as Record<string, unknown>);
      return;
    }
  ...
  const modeColor = {
    data: theme.palette.info.main,
    schema: theme.palette.primary.main,
    validate: validCount > 0 ? theme.palette.error.main : theme.palette.success.main,
  };
    ...
    >
      ...
      >
        ...
        <Tooltip title="Undo (Ctrl+Z)">
          <span>
            <IconButton size="small" onClick={undo} disabled={!hist.past.length}>
        ...
        <Tooltip title="Redo (Ctrl+Y)">
          <span>
            <IconButton size="small" onClick={redo} disabled={!hist.future.length}>
        ...
        <ToggleButtonGroup size="small" value={mode} exclusive onChange={(_, v) => v && setMode(v)}>
          ...
          <ToggleButton value="validate" sx={{ px: 1, gap: 0.5, fontSize: 11 }}>
            {validCount > 0 ? (
              <Badge badgeContent={validCount} color="error">
        ...
        <Tooltip title="Find (Ctrl+F)">
          <IconButton size="small" onClick={() => setFindOpen(true)}>
            <SearchIcon sx={{ fontSize: 16 }} />
          </IconButton>
        </Tooltip>
        <Tooltip title="Add root key">
          <IconButton size="small" onClick={() => commit({ ...data, [`key${Date.now()}`]: '' })}>
        ...
        {currentViewId && (
          <Chip
            size="small"
            label={views.find((v) => v.id === currentViewId)?.name}
            onDelete={() => setCurrentViewId(null)}
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
          ...
          <Typography variant="caption" color="text.secondary">
            ...
            {mode === 'validate' &&
              validCount > 0 &&
              ` · ${validCount} error${validCount > 1 ? 's' : ''}`}
          </Typography>
        </Box>
      </Box>
        ...
        >
          ...
          <Tabs
            ...
            sx={{
              ...
              flexShrink: 0,
              minHeight: 32,
              '& .MuiTab-root': { minHeight: 32, fontSize: 10, py: 0.5 },
            }}
          >
            <Tab label="Tree" />
            <Tab label="Palette" />
          ...
          {leftTab === 0 && (
            <Box sx={{ flex: 1, overflow: 'auto', py: 0.5 }}>
              ...
              {!topKeys.length && (
                <Typography
                  variant="caption"
                  sx={{ px: 1.5, py: 1, display: 'block', color: 'text.disabled' }}
                >
                  Empty — click + to add
                </Typography>
              )}
          ...
          {leftTab === 2 && (
            <Box sx={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
              <ViewsPanel
                ...
                onRename={(id, name) =>
                  setViews((p) => p.map((v) => (v.id === id ? { ...v, name } : v)))
                }
                onToggleKey={(vid, k, vis) =>
                  setViews((p) =>
                    p.map((v) => {
        ...
        <Box sx={{ flex: 1, minWidth: 0, position: 'relative' }}>
          <ReactFlowProvider>
            ...
            >
              ...
              <Controls />
              <MiniMap
                style={{ background: isDark ? '#181828' : theme.palette.grey[100] }}
      ...
      {/* ── Context menus ── */}
      <Menu
        open={!!ctxMenu && ctxMenu.submenu === null}
        onClose={closeCtx}
        anchorReference="anchorPosition"
        anchorPosition={ctxMenu?.anchor}
        PaperProps={{ sx: { minWidth: 180 } }}
      >
        <MenuItem onClick={() => setCtxMenu((m) => (m ? { ...m, submenu: 'new' } : m))}>
          <ListItemIcon>
            <AddIcon fontSize="small" />
          </ListItemIcon>
          <ListItemText>New</ListItemText>
          <ChevronRightIcon sx={{ ml: 1 }} fontSize="small" />
        </MenuItem>
        <MenuItem onClick={ctxCut}>
          <ListItemIcon>
            <ContentCutIcon fontSize="small" />
          </ListItemIcon>
          <ListItemText>Cut</ListItemText>
        </MenuItem>
        <MenuItem onClick={ctxCopy}>
          <ListItemIcon>
            <ContentCopyIcon fontSize="small" />
          </ListItemIcon>
          <ListItemText>Copy</ListItemText>
        </MenuItem>
        <MenuItem onClick={ctxPaste} disabled={!clipboard}>
          <ListItemIcon>
            <ContentPasteIcon fontSize="small" />
          </ListItemIcon>
          <ListItemText>Paste</ListItemText>
        </MenuItem>
        <MenuItem onClick={ctxDelete}>
          <ListItemIcon>
        ...
        <MenuItem
          ...
          onClick={() => setCtxMenu((m) => (m ? { ...m, submenu: 'view' } : m))}
        >
          <ListItemIcon>
      ...
      <Menu
        open={!!ctxMenu && ctxMenu.submenu === 'new'}
        onClose={closeCtx}
        anchorReference="anchorPosition"
        anchorPosition={
          ctxMenu ? { top: ctxMenu.anchor.top, left: ctxMenu.anchor.left + 185 } : undefined
        }
      >
          ...
          <MenuItem key={t} dense onClick={() => ctxAdd(t)}>
            <Typography
              sx={{
                fontSize: 12,
                fontFamily: 'monospace',
                color: getTypeColors(isDark)[t] ?? 'text.primary',
              }}
            >
              {t}
            </Typography>
      ...
      <Menu
        open={!!ctxMenu && ctxMenu.submenu === 'view'}
        onClose={closeCtx}
        anchorReference="anchorPosition"
        anchorPosition={
          ctxMenu ? { top: ctxMenu.anchor.top, left: ctxMenu.anchor.left + 185 } : undefined
      ...
      >
        <MenuItem dense onClick={() => ctxAddToView(false)}>
          <ListItemText>Add to view</ListItemText>
        </MenuItem>
        <MenuItem dense onClick={() => ctxAddToView(true)}>
          <ListItemText>Add all to view</ListItemText>
        </MenuItem>
      </Menu>
