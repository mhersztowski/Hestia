...
export function useDjvuNumPages(key: string, read: FileBytes): number | null {
  ...
  useEffect(() => {
    ...
    setN(null);
    if (!key) return;
    loadDjvuDocument(key, read)
      .then((doc) => {
        if (alive) setN(doc.getPagesQuantity());
      })
      .catch(() => {
        if (alive) setN(null);
      });
...
export const DjvuViewContent: React.FC<{
  ...
  const applyPan = useCallback(() => {
    ...
    const bw = el.clientWidth,
      bh = el.clientHeight;
    const maxX = Math.max(0, rw - bw),
      maxY = Math.max(0, rh - bh);
    let offX = (offRef.current.x || 0) * rw,
      offY = (offRef.current.y || 0) * rh;
    offX = Math.min(Math.max(0, offX), maxX);
    offY = Math.min(Math.max(0, offY), maxY);
    ...
    try {
      ...
      const fit = region
        ? Math.max(
            ...
            height > 0 ? height / imageData.height : width / imageData.width
          )
        : width / imageData.width;
  ...
  const persist = useCallback(() => {
    onViewChange?.({ x: offRef.current.x, y: offRef.current.y, zoom: vzoom });
  }, [onViewChange, vzoom]);
  ...
  const onPanPointerDown = useCallback(
    (e: React.PointerEvent<HTMLDivElement>) => {
      ...
      const startX = e.clientX,
        startY = e.clientY;
      const startOffX = offRef.current.x * rw,
        startOffY = offRef.current.y * rh;
      const maxX = Math.max(0, rw - bw),
        maxY = Math.max(0, rh - bh);
      (e.target as HTMLElement).setPointerCapture?.(e.pointerId);
      const onMove = (me: PointerEvent) => {
        let offX = startOffX - (me.clientX - startX),
          offY = startOffY - (me.clientY - startY);
        offX = Math.min(Math.max(0, offX), maxX);
        offY = Math.min(Math.max(0, offY), maxY);
        offRef.current = { x: rw > 0 ? offX / rw : 0, y: rh > 0 ? offY / rh : 0 };
        const canvas = canvasRef.current;
  ...
  return (
    <Box
      ...
      sx={{
        width: '100%',
        height: '100%',
        display: region ? 'block' : 'flex',
        flexDirection: 'column',
        alignItems: 'center',
        position: 'relative',
        bgcolor: '#525659',
        overflow: 'hidden',
        cursor: region && status === 'ok' ? 'grab' : 'default',
        touchAction: region ? 'none' : undefined,
      }}
    >
      {status === 'loading' && (
        <Box
          sx={{
            position: region ? 'absolute' : 'static',
            inset: 0,
            flex: 1,
            display: 'flex',
            alignItems: 'center',
      ...
      {status === 'error' && (
        <Box
          sx={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', p: 1 }}
        >
          <Typography
            sx={{ fontSize: 11, color: '#ff8a80', fontFamily: 'monospace', textAlign: 'center' }}
          >
      ...
      <canvas
        ref={canvasRef}
        style={{ display: status === 'ok' ? 'block' : 'none', maxWidth: region ? 'none' : '100%' }}
      />
          ...
          return (
            ...
            >
              <Tooltip title="First page" arrow>
                <span>
                  <IconButton size="small" sx={btn} disabled={cur <= 1} onClick={() => go(1)}>
              ...
              <Tooltip title="Previous" arrow>
                <span>
                  <IconButton size="small" sx={btn} disabled={cur <= 1} onClick={() => go(cur - 1)}>
