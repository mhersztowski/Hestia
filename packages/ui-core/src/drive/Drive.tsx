...
import {
    Alert, Box, Breadcrumbs, CircularProgress, Divider, IconButton, Link, List, ListItemButton,
    ListItemIcon, ListItemText, Menu, MenuItem, Snackbar, TextField, Tooltip, Typography,
} from '@mui/material';
...
import {
    baseName, breadcrumbs, dataUrl, formatSize, imageMimeType, kindOf, parentDir, pathIn, safeName,
    sortEntries, type DriveEntry, type DriveStore,
} from './store';
...
export function Drive({ store, onOpenFile, toolbarStart, listWidth = 280 }: DriveProps) {
    ...
    useEffect(() => { setDir(store.startDir ?? ''); setSelected(null); setPreview({ state: 'empty' }); }, [store]);

    const show = useCallback(async (entry: DriveEntry) => {
        const path = pathIn(dir, entry.name);
        setSelected(path);
        const kind = kindOf(entry.name);
        if (kind === 'other') {
            setPreview({ state: 'none', path, reason: 'The drive has no preview for this kind of file.' });
            return;
        }
        ...
        try {
            ...
            if (!store.readBytes) {
                setPreview({ state: 'none', path, reason: 'This store does not hand out file bytes, so there is nothing to draw.' });
                return;
            }
    ...
    const openEntry = (entry: DriveEntry) => {
        if (entry.directory) { setDir(pathIn(dir, entry.name)); setSelected(null); setPreview({ state: 'empty' }); return; }
        void show(entry);
    ...
    const newFolder = async () => {
        ...
        if (!name) return;
        try { await store.createDir(pathIn(dir, safeName(name))); await refresh(dir); }
        catch (e) { setToast(`Could not create the folder: ${message(e)}`); }
    };
    ...
    const newFile = async () => {
        ...
        if (!name) return;
        try { await store.write(pathIn(dir, safeName(name)), ''); await refresh(dir); }
        catch (e) { setToast(`Could not create the file: ${message(e)}`); }
    };
    ...
    const remove = async (entry: DriveEntry) => {
        ...
        try {
            ...
            if (selected === path) { setSelected(null); setPreview({ state: 'empty' }); }
            await refresh(dir);
        } catch (e) { setToast(`Could not delete: ${message(e)}`); }
    };
    ...
    const commitRename = async () => {
        ...
        try { await store.rename(pathIn(dir, entry.name), pathIn(dir, to)); await refresh(dir); }
        catch (e) { setToast(`Could not rename: ${message(e)}`); }
    };
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0 }}>
            ...
            <Box sx={{
                ...
                <Tooltip title="Refresh">
                    <IconButton size="small" onClick={() => void refresh(dir)}><RefreshIcon fontSize="small" /></IconButton>
                </Tooltip>
                ...
                {store.createDir && (
                    <Tooltip title="New folder">
                        <IconButton size="small" onClick={() => void newFolder()}><CreateNewFolderIcon fontSize="small" /></IconButton>
                    </Tooltip>
                )}
                {store.write && (
                    <Tooltip title="New file">
                        <IconButton size="small" onClick={() => void newFile()}><NoteAddIcon fontSize="small" /></IconButton>
                    </Tooltip>
                )}
                <Breadcrumbs sx={{ ml: 1, fontSize: 13, minWidth: 0, '& ol': { flexWrap: 'nowrap' } }}>
                    {trail.map((step, i) => (i === trail.length - 1
                        ? <Typography key={step.path} variant="body2" color="text.primary" noWrap>{step.label}</Typography>
                        : <Link key={step.path} component="button" underline="hover" variant="body2" onClick={() => setDir(step.path)}>{step.label}</Link>
                    ))}
                </Breadcrumbs>
            ...
            <Box sx={{ display: 'flex', flex: 1, minHeight: 0 }}>
                {/* The listing */}
                <Box sx={{ width: listWidth, flexShrink: 0, overflow: 'auto', borderRight: '1px solid', borderColor: 'divider' }}>
                    {error && <Alert severity="error" sx={{ m: 1 }}>{error}</Alert>}
                    {!loading && !error && entries.length === 0 && (
                        <Typography variant="body2" color="text.secondary" sx={{ p: 1.5 }}>This folder is empty.</Typography>
                    )}
                        ...
                        {entries.map((entry) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    {isRenaming ? (
                                        <TextField
                                            size="small" autoFocus fullWidth variant="standard"
                                    ...
                                    {(store.rename || store.remove) && !isRenaming && (
                                        <IconButton
                                            ...
                                            onClick={(e) => { e.stopPropagation(); setMenu({ anchor: e.currentTarget, entry }); }}
                                        >
            ...
            >
                {store.urlFor && menu && !menu.entry.directory && (
                    ...
                    >
                        <ListItemIcon><OpenInNewIcon fontSize="small" /></ListItemIcon>Open in a tab
                    </MenuItem>
                )}
                {store.rename && menu && (
                    <MenuItem onClick={() => { setRenaming({ entry: menu.entry, value: menu.entry.name }); setMenu(null); }}>
                        <ListItemIcon><DriveFileRenameOutlineIcon fontSize="small" /></ListItemIcon>Rename
                    </MenuItem>
                )}
                {store.remove && menu && [
                    <Divider key="sep" />,
                    <MenuItem key="delete" onClick={() => { const entry = menu.entry; setMenu(null); void remove(entry); }}>
            ...
            <Snackbar open={!!toast} autoHideDuration={4000} onClose={() => setToast(null)}
                ...
                {toast ? <Alert severity="error" variant="filled" onClose={() => setToast(null)}>{toast}</Alert> : undefined}
            </Snackbar>
        </Box>
    );
}
...
function PreviewPane({ preview, store }: { preview: Preview; store: DriveStore }) {
    ...
    if (preview.state === 'loading') {
        return <Centre><CircularProgress size={22} /></Centre>;
    }
    if (preview.state === 'error') {
        return <Alert severity="error" sx={{ m: 2 }}>Could not read {baseName(preview.path)}: {preview.reason}</Alert>;
    }
    if (preview.state === 'none') {
        const url = store.urlFor?.(preview.path);
        return (
            <Centre>
                <Typography variant="body2" color="text.secondary" align="center">{preview.reason}</Typography>
                {url && <Link href={url} target="_blank" rel="noreferrer" variant="body2" sx={{ mt: 1 }}>Open it in a tab</Link>}
            </Centre>
        );
    }
    if (preview.state === 'image') {
        return (
            <Box sx={{ p: 2, display: 'flex', justifyContent: 'center' }}>
                <Box component="img" src={preview.src} alt={baseName(preview.path)}
    ...
    if (preview.state === 'pdf') {
        ...
        return (
            <Box component="object" data={preview.src} type="application/pdf"
    ...
    return (
        <Box component="pre" sx={{
            m: 0, p: 1.5, fontFamily: 'monospace', fontSize: 12, lineHeight: 1.5,
