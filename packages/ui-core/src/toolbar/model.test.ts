...
import {
    applyToggle, custom, findNode, findPath, hasSubmenu, isActionable, item, separator, splitToggle, submenu,
    titleOf, toggle, visibleNodes, walk, type ToolbarNode,
} from './model';
...
describe('reading the tree', () => {
    ...
    it('walks every node once', () => {
        expect(walk(tree()).map((n) => n.id)).toEqual(
            ['file', 'new', 's1', 'export', 'png', 'svg', 'view', 'grid', 's2', 'light', 'dark'],
        );
    });
...
describe('a split button', () => {
    const palette = (): ToolbarNode[] => [
        ...
        splitToggle('circle', 'Circle', false, [
            item('circle-c', 'By centre'),
            item('circle-3p', 'By three points'),
        ], { group: 'tool' }),
    ];
