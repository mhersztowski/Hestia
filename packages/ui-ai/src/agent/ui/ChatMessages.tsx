  ...
  return (
    <Box sx={{ px: 1, py: 0.25 }}>
      ...
      {expanded && (
        ...
        >
          {parsedResult ? JSON.stringify(parsedResult, null, 2) : message.content}
        </Box>
      )}
    </Box>
  );
              ...
              >
                {msg.attachments?.map((att, i) =>
                  att.mimeType.startsWith('image/') ? (
                    ...
                    <Box
                      key={i}
                      sx={{ fontSize: 11, color: '#a0c4f8', fontFamily: 'monospace', mb: 0.25 }}
                    >
                      📎 {att.name}
                    </Box>
                  )
