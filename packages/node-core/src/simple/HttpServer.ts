...
interface PrefixRoute { prefix: string; handler: Handler }
...
export class HttpServer {
    ...
    private async handle(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
        ...
        try {
            for (const { prefix, handler } of this.prefixes) {
                ...
                const result = await handler({ params: { rest: url.pathname.slice(prefix.length) }, query: url.searchParams, body, req, res });
    ...
    private async staticFile(pathname: string, res: http.ServerResponse): Promise<void> {
        ...
        if (!dir) { this.sendJson(res, 404, { error: 'No built frontend' }); return; }
