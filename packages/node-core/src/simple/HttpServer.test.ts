...
describe('HttpServer', () => {
    ...
    it('returns the handler result as JSON', async () => {
        server.get('/api/echo/:what', ({ params, query }) => ({ what: params.what, howMany: query.get('howMany') }));
    ...
    it('HttpError from a handler sets the response code', async () => {
        server.get('/api/missing', () => { throw new HttpError(404, 'No such transaction'); });
        port = await server.start();
    ...
    it('a prefix catches a whole branch of URLs, on any method', async () => {
        ...
        expect((await (await get('/onwards', { method: 'POST', body: '{}' })).json()))
            .toEqual({ rest: '', method: 'POST' });
    });
