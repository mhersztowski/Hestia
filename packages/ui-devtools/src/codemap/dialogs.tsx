...
export function FilePickerDialog({
  ...
  const toggleFile = (path: string) =>
    setChecked((prev) => (prev.includes(path) ? prev.filter((f) => f !== path) : [...prev, path]));
  ...
  const load = useCallback(
    async (dir: string) => {
      ...
      setCwd(dir);
      try {
        setEntries(sortEntries(await store.list(dir)));
      } catch (e) {
        setEntries([]);
        setError(message(e));
      } finally {
  ...
  const switchRoot = (i: number) => {
    setRootIdx(i);
    setChecked([]);
    void load(roots[i].path);
  };
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      ...
      <DialogContent dividers sx={{ minHeight: 320 }}>
        {roots.length > 1 && (
          <Tabs
            value={Math.min(rootIdx, roots.length - 1)}
            onChange={(_, v: number) => switchRoot(v)}
            sx={{
              mb: 1,
              minHeight: 34,
          ...
          >
            {roots.map((r, i) => (
              <Tab key={r.path || i} value={i} label={r.label} />
            ))}
          </Tabs>
        )}
        <Breadcrumbs sx={{ mb: 1 }}>
          <Link component="button" underline="hover" onClick={() => void load(root.path)}>
            {root.label}
          </Link>
          {parts.map((p, i) => (
            <Link
            ...
            {entries.map((e) => {
              ...
              return (
                <ListItem
                  ...
                  secondaryAction={
                    !e.directory && mode === 'dir' ? (
                      <Checkbox
                        edge="end"
                        size="small"
                        checked={checked.includes(child)}
                        onChange={() => toggleFile(child)}
                      />
                    ) : undefined
                  }
                >
          ...
          <>
            ...
            <Button
              ...
              onClick={() => {
                ...
                onPick(
                  cwd,
                  checked.map((f) => (f.startsWith(base) ? f.slice(base.length) : f))
                );
                onClose();
              }}
            >
    ...
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
      ...
      <DialogContent dividers>
        {loading ? (
          ...
          <Typography color="error">Could not read the file: {error}</Typography>
        ) : (
          <Box
            component="pre"
            sx={{
...
export function HistoryDialog({
  ...
  open: boolean;
  history: CodemapHistory | null;
  uncommitted: boolean;
  onClose: () => void;
  onCheckoutBranch: (b: string) => void;
  onNewBranch: () => void;
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
      <DialogTitle>
        History — branch <b>{history.head}</b>
        {uncommitted && (
          <Chip size="small" color="warning" label="uncommitted changes" sx={{ ml: 1 }} />
        )}
      </DialogTitle>
      <DialogContent dividers>
        <Stack
        ...
        >
          ...
          {branchNames.map((b) => (
            <Chip
              ...
              label={b}
              size="small"
              color={b === history.head ? 'primary' : 'default'}
              variant={b === history.head ? 'filled' : 'outlined'}
              onClick={() => onCheckoutBranch(b)}
            />
          ))}
          <Button size="small" startIcon={<AddIcon />} onClick={onNewBranch}>
            New branch
          </Button>
        </Stack>
        <Divider sx={{ mb: 1 }} />
        ...
        <List dense>
          {log.map((c) => (
            <ListItem
              ...
              secondaryAction={
                <Tooltip title="Bring this state back as the working state">
                  <IconButton edge="end" size="small" onClick={() => onRestore(c.id)}>
            ...
            >
              ...
              <ListItemText
                primary={
                  ...
                  >
                    <span>{c.message}</span>
                    {(tagsByCommit[c.id] ?? []).map((t) => (
                      <Chip
                        key={t}
                        size="small"
                        label={t}
                        color={t === history.head ? 'primary' : 'default'}
                        sx={{ height: 18 }}
                      />
                    ))}
                  </Box>
...
export function CodemapDiffDialog({
  open,
  history,
  workingDiagrams,
  defaultDiagramId,
  ...
  const commitOptions = useMemo(() => {
    ...
    const list = Object.values(history.commits)
      .sort((a, b) => b.at - a.at)
  ...
  useEffect(() => {
    if (open) {
      setTargetId(WORKING);
      setBaseId(headTip ?? WORKING);
      setDiagramKey(defaultDiagramId ?? '');
    }
  }, [open, headTip, defaultDiagramId]);

  const snapOf = useCallback(
    (id: string): UmlDiagram[] =>
      id === WORKING ? workingDiagrams : (history?.commits[id]?.snapshot.diagrams ?? []),
    [history, workingDiagrams]
  );

  const diagramOptions = useMemo(() => {
    const map = new Map<string, string>();
    for (const d of [...snapOf(baseId), ...snapOf(targetId)])
      if (!map.has(d.id)) map.set(d.id, d.name);
    return [...map.entries()].map(([id, name]) => ({ id, name }));
  }, [snapOf, baseId, targetId]);

  const effDiagramKey = diagramOptions.some((d) => d.id === diagramKey)
    ? diagramKey
    : (diagramOptions[0]?.id ?? '');
  const { nodes, edges, counts } = useMemo(
    () =>
      buildDiff(
        snapOf(baseId).find((d) => d.id === effDiagramKey),
        snapOf(targetId).find((d) => d.id === effDiagramKey)
      ),
  ...
  const fmt = (
    label: string,
    [a, r, m]: [number, number, number] | [number, number],
    hasMod: boolean
  ) =>
    `${label}: ` +
    [`+${a}`, `-${r}`, hasMod ? `~${(m as number) ?? 0}` : ''].filter(Boolean).join(' ');
    ...
    >
      ...
      <Box sx={{ px: 3, pb: 1, display: 'flex', gap: 1, alignItems: 'center', flexWrap: 'wrap' }}>
        <Chip
          size="small"
          label="added"
          sx={{ bgcolor: `${DIFF_COLOR.added}22`, color: DIFF_COLOR.added }}
        />
        <Chip
          size="small"
          label="removed"
          sx={{ bgcolor: `${DIFF_COLOR.removed}22`, color: DIFF_COLOR.removed }}
        />
        <Chip
          size="small"
          label="modified"
          sx={{ bgcolor: `${DIFF_COLOR.modified}22`, color: DIFF_COLOR.modified }}
        />
        <Divider orientation="vertical" flexItem sx={{ mx: 0.5 }} />
        <Typography variant="caption" color="text.secondary">
          {fmt('Classes', counts.cls, true)} · {fmt('Members', counts.mem, true)} ·{' '}
          {fmt('Relations', counts.rel, false)}
        </Typography>
      </Box>
      <DialogContent sx={{ p: 0, borderTop: '1px solid', borderColor: 'divider' }}>
        ...
        <Box sx={{ width: '100%', height: '100%' }}>
          <ReactFlow
            nodes={nodes}
            edges={edges}
            nodeTypes={DIFF_NODE_TYPES}
            edgeTypes={DIFF_EDGE_TYPES}
            fitView
            minZoom={0.15}
            maxZoom={2.5}
          ...
          >
            ...
            <MiniMap
              ...
              pannable
              nodeColor={(n) => {
                const d = n.data as DiffNodeData;
                return d?.status && d.status !== 'unchanged'
                  ? DIFF_COLOR[d.status]
                  : (KIND_META[d?.kind ?? 'class']?.color ?? '#999');
              }}
            />
          </ReactFlow>
        </Box>
...
export function OpenCodemapDialog({
  open,
  files,
  current,
  codemapDir,
      ...
      <DialogContent dividers>
        {files.length === 0 ? (
          <Typography variant="body2" color="text.secondary">
            No codemaps in {codemapDir}/ yet. Create one with "New codemap" and save it, or build
            one "From code".
          </Typography>
        ) : (
          <List dense>
            {files.map((f) => (
                ...
                secondaryAction={
                  onRemove && (
                    <Tooltip title="Delete codemap">
                      <IconButton edge="end" size="small" onClick={() => onRemove(f)}>
              ...
              >
                ...
                >
                  ...
                  <ListItemText
                    primary={codemapDisplayName(f)}
                    secondary={f === current ? 'open' : undefined}
                  />
                </ListItemButton>
              </ListItem>
            ))}
...
export function OutputFileDialog({
  open,
  initialPath,
  placeholder,
  onAdd,
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      ...
      <DialogContent>
        <Stack direction="row" spacing={1} alignItems="flex-start" sx={{ mt: 1 }}>
          ...
          <Button
            ...
            onClick={() =>
              onBrowse((dir) => setPath(pathIn(dir, path.split('/').pop() || 'Model.d.ts')))
            }
          >
            Folder…
          </Button>
      ...
      <DialogActions>
        <Button onClick={onClose}>Cancel</Button>
        <Button variant="contained" disabled={!valid} onClick={add}>
          Add
        </Button>
      </DialogActions>
    </Dialog>
