...
import { DIFF_COLOR, type DiffFlowEdge, type DiffFlowNode, type DiffMember, type DiffStatus } from './diff';
import {
  KIND_META, REL_META, VIS_LABEL, VIS_ORDER, categoryColor, docTooltip, hasDoc, memberSigil,
          ...
          <Box key={sig}>
            ...
            {items.map((m) => (
              ...
              >
                ...
                {m.text}
                {hasDoc(m.doc) && <Box component="span" sx={{ ml: 0.5, opacity: 0.5, fontSize: 9 }}>ⓘ</Box>}
              </Typography>
            ))}
          </Box>
        ))}
  ...
  return (
    <Box sx={{ minWidth: 160, bgcolor: 'background.paper', border: '2px solid', borderColor: selected ? meta.color : 'divider', borderRadius: 1, boxShadow: selected ? `0 0 0 2px ${meta.color}55` : 1, fontSize: 12, overflow: 'hidden' }}>
      ...
      <Box sx={{ px: 1, py: 0.5, textAlign: 'center', position: 'relative', bgcolor: `${meta.color}14`, borderBottom: '1px solid', borderColor: 'divider' }}>
        {data.linkedFile && (
          <Tooltip title={`File: ${data.linkedFile}`}><LinkIcon sx={{ position: 'absolute', top: 3, right: 3, fontSize: 13, color: meta.color }} /></Tooltip>
        )}
        ...
        <Typography
          ...
          sx={{ fontWeight: 700, fontStyle: italic ? 'italic' : 'normal', color: meta.color, lineHeight: 1.3, cursor: hasDoc(data.doc) ? 'help' : 'default' }}
        >
          {data.name || 'Unnamed'}
...
function UmlEdgeView({ id, sourceX, sourceY, targetX, targetY, sourcePosition, targetPosition, data, selected }: EdgeProps<UmlFlowEdge>) {
  ...
  const renderMembers = (ms: DiffMember[]) => (
    <Box sx={{ px: 1, py: 0.5, minHeight: 18, borderBottom: '1px solid', borderColor: 'divider' }}>
      {ms.length === 0 ? <Typography sx={{ fontSize: 11, color: 'text.disabled' }}>&nbsp;</Typography> : ms.map((m) => (
        <Tooltip key={m.id} title={m.status === 'modified' && m.oldText ? `was: ${m.oldText}` : ''} disableHoverListener={m.status !== 'modified'}>
...
function DiffEdgeView({ id, sourceX, sourceY, targetX, targetY, sourcePosition, targetPosition, data }: EdgeProps<DiffFlowEdge>) {
