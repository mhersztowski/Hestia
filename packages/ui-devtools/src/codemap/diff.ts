...
export const DIFF_COLOR: Record<DiffStatus, string> = { added: '#2e7d32', removed: '#c62828', modified: '#ef6c00', unchanged: '#9e9e9e' };

export interface DiffMember extends UmlMember { status: DiffStatus; oldText?: string }
export type DiffNodeData = { kind: UmlKind; name: string; oldName?: string; status: DiffStatus; members: DiffMember[] };
...
export function buildDiff(baseDia: UmlDiagram | undefined, targetDia: UmlDiagram | undefined): DiffResult {
  ...
  const bumpMem = (ms: DiffMember[]) => ms.forEach((m) => {
    if (m.status === 'added') counts.mem[0]++; else if (m.status === 'removed') counts.mem[1]++; else if (m.status === 'modified') counts.mem[2]++;
  });
