...
import {
  Box, Button, Chip, CircularProgress, Collapse, IconButton, List, ListItem, ListItemButton, ListItemIcon,
  ListItemText, MenuItem, Popover, Stack, TextField, Tooltip, Typography,
} from '@mui/material';
...
import { VIS_COLOR, VIS_LABEL, VIS_ORDER, categoryColor, changeTextSigil, memberSigil } from './model';
...
export function usePersistentWidth(key: string, initial: number): [number, (w: number) => void] {
  ...
  const set = useCallback((w: number) => {
    setWidth(w);
    try { localStorage.setItem(key, String(Math.round(w))); } catch { /* storage blocked — the width lives until a reload */ }
  }, [key]);
  return [width, set];
}
 ...
 */
export function ResizeHandle({ side, width, setWidth, min = 160, max = 700 }: {
  ...
  return (
    <Box
      ...
      onPointerUp={(e) => { start.current = null; try { (e.target as HTMLElement).releasePointerCapture(e.pointerId); } catch { /* already released */ } }}
      sx={{
...
export function CodemapTreeItem({ label, sublabel, active, expanded, linkedPath, diagrams, activeDiagramId, onOpen, onSelectDiagram, onAddDiagram, onRenameDiagram, onDeleteDiagram, onLink, onUnlink, onRename, onDelete }: {
  ...
  return (
    <>
      <ListItem disablePadding secondaryAction={active ? (
        <Stack direction="row">
          <Tooltip title={linkedPath ? `Source directory: ${linkedPath} (click to unlink)` : 'Link to a source directory'}>
            <IconButton size="small" edge="end" onClick={linkedPath ? onUnlink : onLink}>{linkedPath ? <LinkOffIcon fontSize="small" /> : <LinkIcon fontSize="small" />}</IconButton>
          </Tooltip>
          {onRename && <Tooltip title="Rename / save"><IconButton size="small" edge="end" onClick={onRename}><DriveFileRenameOutlineIcon fontSize="small" /></IconButton></Tooltip>}
          {onDelete && <Tooltip title="Delete codemap"><IconButton size="small" edge="end" onClick={onDelete}><DeleteOutlineIcon fontSize="small" /></IconButton></Tooltip>}
        </Stack>
      ) : undefined}>
        ...
        <List dense disablePadding>
          {diagrams.map((d) => (
            <ListItem key={d.id} disablePadding sx={{ pl: 2 }} secondaryAction={
              <Stack direction="row">
                <Tooltip title="Rename"><IconButton size="small" edge="end" onClick={() => onRenameDiagram(d.id)}><DriveFileRenameOutlineIcon sx={{ fontSize: 15 }} /></IconButton></Tooltip>
                <Tooltip title="Delete diagram"><IconButton size="small" edge="end" onClick={() => onDeleteDiagram(d.id)}><DeleteOutlineIcon sx={{ fontSize: 15 }} /></IconButton></Tooltip>
              </Stack>}>
          ...
          <ListItem disablePadding sx={{ pl: 5 }}>
            <ListItemButton onClick={onAddDiagram}><ListItemIcon sx={{ minWidth: 26 }}><NoteAddIcon sx={{ fontSize: 16 }} /></ListItemIcon><ListItemText primary="New diagram" primaryTypographyProps={{ fontSize: 12, color: 'text.secondary' }} /></ListItemButton>
          </ListItem>
        </List>
      </Collapse>
    </>
...
function CategoryPicker({ value, categories, listId, onChange }: {
  ...
  const open = (e: MouseEvent) => { setLocal(value); setAnchor(e.currentTarget); };
...
export function MemberSection({ title, members, categories, onAdd, onChange, onCategory, onDelete, onReorder }: {
...
export interface Linker { className: string; diagramName: string; kind: UmlKind }
...
export function LinkedFilesPanel({ linkedPath, items, onPreview, onClose, width = 280, outputs = [], generating = null, onAddOutput, onRemoveOutput, onGenerate }: {
  linkedPath?: string;
  ...
  return (
    <Box sx={{ width, borderLeft: '1px solid', borderColor: 'divider', bgcolor: 'background.paper', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      <Box sx={{ px: 1.5, py: 1, display: 'flex', alignItems: 'center', gap: 1, borderBottom: '1px solid', borderColor: 'divider' }}>
        ...
        <Typography variant="subtitle2" sx={{ flex: 1 }}>Linked files</Typography>
        <Tooltip title="Hide panel"><IconButton size="small" onClick={onClose}><CloseIcon fontSize="small" /></IconButton></Tooltip>
      </Box>
      {linkedPath && (
        <Box sx={{ px: 1.5, py: 0.75, borderBottom: '1px solid', borderColor: 'divider' }}>
          <Typography variant="caption" color="text.secondary">Source directory</Typography>
          <Typography variant="body2" sx={{ fontFamily: 'monospace', fontSize: 11, wordBreak: 'break-all' }}>{linkedPath}</Typography>
        </Box>
      )}
      ...
      <Box sx={{ borderBottom: '1px solid', borderColor: 'divider' }}>
        <Box sx={{ px: 1.5, py: 0.5, display: 'flex', alignItems: 'center', gap: 1 }}>
          <Typography variant="caption" color="text.secondary" sx={{ flex: 1 }}>Output files</Typography>
          {onAddOutput && <Button size="small" onClick={onAddOutput} sx={{ minWidth: 0, px: 1 }}>+ Add</Button>}
        </Box>
        {outputs.length === 0 ? (
                ...
                <ListItem key={f} disablePadding sx={{ pr: 0.5 }}
                  secondaryAction={
                    <Box sx={{ display: 'flex', alignItems: 'center' }}>
                      ...
                      {onRemoveOutput && (
                        <Tooltip title="Remove from the list"><IconButton size="small" onClick={() => onRemoveOutput(f)}><CloseIcon sx={{ fontSize: 15 }} /></IconButton></Tooltip>
                      )}
                    </Box>
                  }>
                  <ListItemButton sx={{ py: 0.25, pr: 9 }} onClick={() => onPreview(f)}>
                    ...
                    <ListItemText primary={base} secondary={label}
      ...
      <Box sx={{ flex: 1, overflowY: 'auto' }}>
        {items.length === 0 ? (
          <Typography variant="body2" color="text.secondary" sx={{ p: 1.5 }}>No linked files. Select a class and use "Link to file".</Typography>
        ) : (
          <List dense disablePadding>
            {rows.map((r) => (
