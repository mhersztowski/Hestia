...
function CodemapEditorInner({ store, syncFromCode, toolbarStart }: CodemapEditorProps) {
  ...
  const [picker, setPicker] = useState<{
    mode: 'dir' | 'file';
    title: string;
    onPick: (path: string, files?: string[]) => void;
  } | null>(null);
  ...
  const refreshFiles = useCallback(async () => {
    try {
      setFiles(await listCodemaps(store));
    } catch (e) {
      setToast({ msg: `Could not list codemaps: ${message(e)}`, sev: 'error' });
    }
  }, [store]);
  ...
  const showDiagram = (d: UmlDiagram | undefined) => {
    setActiveDiagramId(d?.id ?? null);
    setNodes(d?.nodes ?? []);
    setEdges(d?.edges ?? []);
    clearSelection();
  };
    ...
    (async () => {
      ...
      if (list.length > 0) {
        try {
          ...
          setFile(list[0]);
          setDirty(false);
          showDiagram(c.diagrams[0]);
          return;
        } catch {
          /* fall through to a fresh example */
        }
      }
      if (!live) return;
  ...
  const withCanvas = useCallback(
    (c: Codemap): Codemap => ({
      ...
      diagrams: c.diagrams.map((d) =>
        d.id === activeDiagramId ? { ...d, nodes: cleanNodes(nodes), edges: cleanEdges(edges) } : d
      ),
    }),
  ...
  const onConnect = useCallback(
    (c: Connection) => {
      setEdges(
        (eds) =>
          addEdge({ ...c, id: nextId('e'), type: 'uml', data: { relType } }, eds) as UmlFlowEdge[]
      );
      setDirty(true);
    },
    [relType]
  ...
  const updateMembers = useCallback((id: string, fn: (m: UmlMember[]) => UmlMember[]) => {
    setNodes((nds) =>
      nds.map((n) => (n.id === id ? { ...n, data: { ...n.data, members: fn(n.data.members) } } : n))
    );
    setDirty(true);
  }, []);
  const patchEdgeData = useCallback((id: string, patch: Partial<UmlEdgeData>) => {
    setEdges((eds) =>
      eds.map((e) =>
        e.id === id ? { ...e, data: { ...(e.data ?? { relType: 'association' }), ...patch } } : e
      )
    );
    setDirty(true);
  }, []);
  ...
  const selectDiagram = useCallback(
    (id: string) => {
      ...
      const current = withCanvas(codemap);
      setCodemap(current);
      showDiagram(current.diagrams.find((d) => d.id === id));
    },
    [codemap, activeDiagramId, withCanvas]
  );
  const addDiagram = useCallback(() => {
    if (!codemap) return;
    const name = window.prompt('Name of the new diagram', `Diagram ${codemap.diagrams.length + 1}`);
    if (!name) return;
    const current = withCanvas(codemap);
    const d = emptyDiagram(name.trim());
    setCodemap({ ...current, diagrams: [...current.diagrams, d] });
    showDiagram(d);
    setDirty(true);
  }, [codemap, withCanvas]);
  const renameDiagram = useCallback(
    (id: string) => {
      if (!codemap) return;
      const cur = codemap.diagrams.find((d) => d.id === id);
      const name = window.prompt('New name of the diagram', cur?.name ?? '');
      if (!name) return;
      setCodemap(
        (c) =>
          c && {
            ...c,
            diagrams: c.diagrams.map((d) => (d.id === id ? { ...d, name: name.trim() } : d)),
          }
      );
      setDirty(true);
    },
  ...
  const deleteDiagram = useCallback(
    (id: string) => {
      if (!codemap) return;
      if (codemap.diagrams.length <= 1) {
        setToast({ msg: 'A codemap needs at least one diagram', sev: 'info' });
        return;
      }
      if (!window.confirm('Delete this diagram?')) return;
      const current = withCanvas(codemap);
      const remaining = current.diagrams.filter((d) => d.id !== id);
      setCodemap({ ...current, diagrams: remaining });
      if (id === activeDiagramId) showDiagram(remaining[0]);
      setDirty(true);
    },
    [codemap, activeDiagramId, withCanvas]
  );
  ...
  const linkedItems = useMemo(() => {
    ...
    for (const d of codemap.diagrams) {
      ...
      for (const n of dn) {
        const lf = n.data.linkedFile;
        if (lf)
          out.push({
            file: lf,
            linker: { className: n.data.name, diagramName: d.name, kind: n.data.kind },
          });
      }
    }
    return out;
  ...
  useEffect(() => {
    if (categoryFilter && !allCategories.includes(categoryFilter)) setCategoryFilter(null);
  ...
  const uncommitted = useMemo(
    () => (codemap ? hasUncommittedChanges(withCanvas(codemap)) : false),
    [codemap, withCanvas]
  );
  ...
  const replaceWorking = useCallback(
    (op: (c: Codemap) => Codemap, question: string, done: string) => {
      if (!codemap) return;
      if (uncommitted && !window.confirm(question)) return;
      try {
        const next = op(codemap);
        setCodemap(next);
        showDiagram(next.diagrams[0]);
        setDirty(true);
        setHistoryOpen(false);
        setToast({ msg: done, sev: 'info' });
      } catch (e) {
        setToast({ msg: message(e), sev: 'error' });
      }
    },
    [codemap, uncommitted]
  );
  ...
  const onCheckoutBranch = useCallback(
    (branch: string) => {
      if (!codemap || branch === codemap.history.head) {
        setHistoryOpen(false);
        return;
      }
      replaceWorking(
        (c) => checkoutBranch(c, branch),
        'Uncommitted changes will be lost. Switch branch?',
  ...
  const onRestore = useCallback(
    (id: string) => {
      replaceWorking(
        (c) => restoreCommit(c, id),
        'Uncommitted changes will be lost. Restore this commit?',
  ...
  const onNewBranch = useCallback(() => {
    if (!codemap) return;
    const name = window.prompt('Name of the new branch (it starts at the current commit)');
    if (!name) return;
    try {
  ...
  const save = useCallback(async () => {
    ...
    if (!target) {
      const name = window.prompt(
        `Codemap name (saved in ${store.codemapDir}/)`,
        codemap.name === 'Untitled' ? 'codemap' : codemap.name
      );
      if (!name) return;
    ...
    const next = {
      ...withCanvas(codemap),
      name: codemapDisplayName(target),
      updatedAt: Date.now(),
    };
    try {
  ...
  const createNew = useCallback(() => {
    ...
    const name = window.prompt('Name of the new codemap', 'codemap');
    if (!name) return;
    const c = newCodemap(name.trim(), false);
    setCodemap(c);
    setFile(null);
    showDiagram(c.diagrams[0]);
    setDirty(true);
  ...
  const rename = useCallback(
    async (from: string) => {
      const name = window.prompt('New name of the codemap', codemapDisplayName(from));
      if (!name) return;
      const to = codemapFileName(name);
      if (to === from) return;
      try {
        const source =
          from === file && codemap ? withCanvas(codemap) : await readCodemap(store, from);
        ...
        setToast({ msg: `Could not rename: ${message(e)}`, sev: 'error' });
      }
      await refreshFiles();
    },
  ...
  const remove = useCallback(
    async (target: string) => {
      ...
      try {
        await store.remove(pathIn(store.codemapDir, target));
      } catch (e) {
        setToast({ msg: `Could not delete: ${message(e)}`, sev: 'error' });
        return;
      }
      const list = await listCodemaps(store).catch(() => [] as string[]);
      setFiles(list);
      if (target === file) {
        ...
        if (list.length > 0) await load(list[0]);
          ...
          const c = newCodemap('Untitled', true);
          setCodemap(c);
          setFile(null);
          showDiagram(c.diagrams[0]);
          setDirty(true);
  ...
  const addOutput = useCallback((path: string) => {
    setCodemap((c) =>
      c && !(c.outputs ?? []).includes(path) ? { ...c, outputs: [...(c.outputs ?? []), path] } : c
    );
    setDirty(true);
  }, []);
    ...
    async (path: string) => {
      ...
      try {
        ...
        setToast({
          msg:
            plan.length === 1
              ? `Generated ${plan[0].path}`
              : `Generated ${plan.length} files in ${plan[0].path.slice(0, plan[0].path.lastIndexOf('/'))}/`,
          sev: 'success',
        });
      } catch (e) {
  ...
  // ── Links to code ─────────────────────────────────────────────────────────
  const pickSourceDir = useCallback(
    () =>
      setPicker({
        mode: 'dir',
        title: 'Link the codemap to a source directory',
        onPick: (path) => {
          setCodemap((c) => c && { ...c, linkedPath: path });
          setDirty(true);
        },
      }),
    []
  );
  const unlinkSourceDir = useCallback(() => {
    setCodemap((c) => c && { ...c, linkedPath: undefined });
    setDirty(true);
  }, []);
  const pickNodeFile = useCallback(
    (nodeId: string) =>
      setPicker({
        mode: 'file',
        title: 'Link the class to a source file',
        onPick: (path) => patchNodeData(nodeId, { linkedFile: path }),
      }),
    [patchNodeData]
  );
  ...
  const runCodeSync = useCallback(
    async (dir: string, chosen?: string[]) => {
      if (!codemap || !syncFromCode) return;
      const scope = chosen?.length
        ? `${chosen.length} chosen file(s) in ${dir}`
        : `the code in ${dir}`;
      const hasContent = codemap.diagrams.some((d) => d.nodes.length > 0);
      if (
        hasContent &&
        !window.confirm(
          `The source diagram will be rebuilt from ${scope} (node layout is kept). Continue?`
        )
      )
        return;
      setSyncing(true);
      try {
        const res = await syncFromCode({
          dir,
          files: chosen?.length ? chosen : undefined,
          codemap: withCanvas(codemap),
        });
        // `name?: type` fields from the parser get the "optional" category.
        const next = normalizeOptional(
          normalizeCodemap({ ...res.codemap, linkedPath: res.codemap.linkedPath ?? dir })
        );
        setCodemap(next);
        showDiagram(next.diagrams[0]);
        setDirty(true);
        setToast({
          msg: res.changes.length
            ? `From code: ${res.summary} (${res.changes.length} changes)`
            : 'From code: no structural changes',
          sev: 'success',
        });
      } catch (e) {
  ...
  const fromCode = useCallback(() => {
    ...
    if (codemap.linkedPath) void runCodeSync(codemap.linkedPath);
      ...
      setPicker({
        mode: 'dir',
        title: 'Choose a directory, or tick files in it',
        onPick: (path, chosen) => void runCodeSync(path, chosen),
      });
  }, [codemap, runCodeSync]);
  ...
  const selMethods = selectedNode
    ? selectedNode.data.members.filter((m) => m.kind === 'method')
    : [];
  const treeItemProps = {
    activeDiagramId,
    onSelectDiagram: (id: string) => {
      selectDiagram(id);
    ...
    onLink: pickSourceDir,
    onUnlink: unlinkSourceDir,
  };
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      ...
      >
        ...
        <Tooltip title="Panel: codemaps / diagrams">
          <IconButton
            size="small"
        ...
        <Tooltip title="Open codemap">
          <IconButton size="small" onClick={() => void showOpenDialog()}>
            <FolderOpenIcon />
          </IconButton>
        </Tooltip>
        <Tooltip title="Save codemap (Ctrl+S)">
          <span>
            <IconButton
              size="small"
        ...
        <TextField
          select
          size="small"
          label="Relation (for new connections)"
          value={relType}
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                  <Box
                    ...
                    sx={{
                      ...
                      height: 9,
                      borderRadius: '50%',
                      bgcolor: categoryColor(v as string),
                    }}
                  />
                  {v as string}
                </Box>
        ...
        >
          ...
          {allCategories.map((c) => (
            <MenuItem key={c} value={c}>
              <Box
                component="span"
                sx={{ width: 9, height: 9, borderRadius: '50%', bgcolor: categoryColor(c), mr: 1 }}
              />
              {c}
            </MenuItem>
          ))}
        ...
        {syncFromCode && (
          <>
            <Tooltip
              title={
                codemap?.linkedPath
                  ? `Build / update from the code in ${codemap.linkedPath}`
                  : 'Build from code — choose a directory'
              }
            >
              <span>
                <Button
                  size="small"
                  variant="outlined"
                  startIcon={syncing ? <CircularProgress size={14} /> : <AutoFixHighIcon />}
                  disabled={syncing}
                  onClick={fromCode}
        ...
        <Tooltip title="Current branch">
          <Chip
            size="small"
            icon={<CallSplitIcon />}
            label={codemap?.history.head ?? '—'}
            color={uncommitted ? 'warning' : 'default'}
            variant="outlined"
          />
        </Tooltip>
        <Tooltip title="Commit (record a version in the history)">
          <span>
            <IconButton size="small" disabled={!uncommitted} onClick={() => setCommitOpen(true)}>
              <CommitIcon />
            </IconButton>
          </span>
        </Tooltip>
        <Tooltip title="History / branches">
          <IconButton size="small" onClick={() => setHistoryOpen(true)}>
            <HistoryIcon />
          </IconButton>
        </Tooltip>
        <Tooltip title="Diff — compare two versions">
          <IconButton size="small" onClick={() => setDiffOpen(true)}>
            <CompareArrowsIcon />
          </IconButton>
        </Tooltip>
        <Tooltip title="Panel: properties">
          <IconButton
            size="small"
            color={propsOpen ? 'primary' : 'default'}
        ...
        <Tooltip
          title={`Panel: linked files${linkedItems.length ? ` (${linkedItems.length})` : ''}`}
        >
          <IconButton
            size="small"
            color={showFilesPanel ? 'primary' : 'default'}
        ...
        <Typography variant="body2" color="text.secondary" sx={{ ml: 0.5 }}>
          {codemap ? `${codemap.name}${file ? '' : ' (unsaved)'}` : '—'}
          {dirty ? ' •' : ''}
        </Typography>
      </Paper>
      ...
      <Box sx={{ display: 'flex', flex: 1, overflow: 'hidden', position: 'relative' }}>
        ...
        >
          <Box sx={{ display: 'flex', alignItems: 'center', px: 1, pt: 0.5 }}>
            ...
            <Tooltip title="Hide panel">
              <IconButton size="small" onClick={() => setTreeOpen(false)}>
            ...
            {files.map((f) => {
              ...
              return (
                <CodemapTreeItem
                  ...
                  expanded={isActive}
                  active={isActive}
                  label={codemapDisplayName(f)}
                  linkedPath={isActive ? codemap?.linkedPath : undefined}
                  diagrams={isActive ? (codemap?.diagrams ?? []) : []}
                  {...treeItemProps}
                  onOpen={() => {
                    void open(f);
                    if (isNarrow) setTreeOpen(false);
          ...
          <CategoryFilterContext.Provider value={categoryFilter}>
            ...
            >
              ...
              <MiniMap
                ...
                pannable
                nodeColor={(n) =>
                  KIND_META[(n.data as UmlNodeData)?.kind ?? 'class']?.color ?? '#999'
                }
              />
            </ReactFlow>
        ...
        >
          <Box sx={{ display: 'flex', alignItems: 'center', mb: 0.5 }}>
            ...
            <Tooltip title="Hide panel">
              <IconButton size="small" onClick={() => setPropsOpen(false)}>
                <CloseIcon fontSize="small" />
              </IconButton>
            </Tooltip>
          </Box>
          {!selectedNode && !selectedEdge && (
            <Typography variant="body2" color="text.secondary">
              Select a class or a relation to edit it. Drag from a class's edge to connect it to
              another.
            </Typography>
          )}
          ...
          {selectedNode && (
            <Stack spacing={1.5}>
              ...
              <TextField
                ...
                onChange={(e) =>
                  patchNodeData(selectedNode.id, { kind: e.target.value as UmlKind })
                }
              >
                {(Object.keys(KIND_META) as UmlKind[]).map((k) => (
              ...
              <TextField
                ...
                onChange={(e) => patchNodeData(selectedNode.id, { name: e.target.value })}
              />
              ...
              <MemberSection
                ...
                onAdd={() =>
                  updateMembers(selectedNode.id, (ms) => [...ms, member('field', '- field: type')])
                }
                onChange={(mid, text) =>
                  updateMembers(selectedNode.id, (ms) =>
                    ms.map((m) =>
                      m.id === mid
                        ? {
                            ...m,
                            text,
                            category: fieldNameOptional(text) ? 'optional' : m.category,
                          }
                        : m
                    )
                  )
                }
                onCategory={(mid, category) =>
                  updateMembers(selectedNode.id, (ms) =>
                    ms.map((m) => (m.id === mid ? { ...m, category: category || undefined } : m))
                  )
                }
                onDelete={(mid) =>
                  updateMembers(selectedNode.id, (ms) => ms.filter((m) => m.id !== mid))
                }
                onReorder={(from, to) =>
                  updateMembers(selectedNode.id, (ms) => reorderWithinKind(ms, from, to))
                }
              />
              ...
              <MemberSection
                ...
                onAdd={() =>
                  updateMembers(selectedNode.id, (ms) => [
                    ...ms,
                    member('method', '+ method(): void'),
                  ])
                }
                onChange={(mid, text) =>
                  updateMembers(selectedNode.id, (ms) =>
                    ms.map((m) => (m.id === mid ? { ...m, text } : m))
                  )
                }
                onCategory={(mid, category) =>
                  updateMembers(selectedNode.id, (ms) =>
              ...
              {selectedNode.data.linkedFile ? (
                <>
                  <Chip
                    icon={<LinkIcon />}
                    label={selectedNode.data.linkedFile}
                    size="small"
                    sx={{
                      maxWidth: '100%',
                      '& .MuiChip-label': { overflow: 'hidden', textOverflow: 'ellipsis' },
                    }}
                    onDelete={() => patchNodeData(selectedNode.id, { linkedFile: undefined })}
                  />
                  <Stack direction="row" spacing={1}>
                    <Button
                      size="small"
                      startIcon={<VisibilityIcon />}
                      onClick={() => setPreviewPath(selectedNode.data.linkedFile!)}
                    >
                      Preview
                    </Button>
                    <Button
                      size="small"
                      startIcon={<LinkIcon />}
                      onClick={() => pickNodeFile(selectedNode.id)}
                    >
                      Change
                    </Button>
                  </Stack>
          ...
          {selectedEdge && (
            <Stack spacing={1.5}>
              ...
              <TextField
                ...
                size="small"
                label="Relation type"
                value={selectedEdge.data?.relType ?? 'association'}
                onChange={(e) =>
                  patchEdgeData(selectedEdge.id, { relType: e.target.value as RelType })
                }
              >
                {REL_ORDER.map((r) => (
                  <MenuItem key={r} value={r}>
              ...
              <TextField
                ...
                onChange={(e) => patchEdgeData(selectedEdge.id, { label: e.target.value })}
              />
              <Button
                size="small"
                color="error"
        ...
        {showFilesPanel && (
          <ResizeHandle side="right" width={filesWidth} setWidth={setFilesWidth} />
        )}
        {showFilesPanel && (
          <Box
            sx={{
          ...
          >
            <LinkedFilesPanel
              ...
              linkedPath={codemap?.linkedPath}
              items={linkedItems}
              outputs={codemap?.outputs ?? []}
              generating={generating}
              onAddOutput={() => setOutputDialog(true)}
              onRemoveOutput={removeOutput}
              onGenerate={(f) => void generateOutput(f)}
      ...
      <CommitDialog
        open={commitOpen}
        canCommit={uncommitted}
        onCommit={doCommit}
        onClose={() => setCommitOpen(false)}
      />
      <HistoryDialog
        open={historyOpen}
        history={codemap?.history ?? null}
      ...
      <CodemapDiffDialog
        open={diffOpen}
        history={codemap?.history ?? null}
        workingDiagrams={codemap ? withCanvas(codemap).diagrams : []}
        defaultDiagramId={activeDiagramId}
      ...
      <OpenCodemapDialog
        ...
        files={files}
        current={file}
        codemapDir={store.codemapDir}
        onOpen={(f) => void open(f)}
        onRemove={store.remove ? (f) => void remove(f) : undefined}
