...
const PARTICIPANT =
  /^\s*(?:(create)\s+)?(participant|actor)\s+([A-Za-z0-9_]+)(?:\s+as\s+(.+?))?\s*$/i;
...
const arrowOf = (op: string): SequenceArrow =>
  ARROWS.find((a) => a.op === op)?.arrow ?? 'solidArrow';
...
export function parseSequenceDiagram(text: string): ParseResult {
  ...
  for (const line of front.body.split('\n')) {
    ...
    if (destroy) {
      push({ kind: 'destroy', participant: destroy[1] });
      continue;
    }
    ...
    if (BLOCK_END.test(trimmed) && stack.length) {
      stack.pop();
      continue;
    }
    ...
    if (activation) {
      push({
        kind: activation[1].toLowerCase() as 'activate' | 'deactivate',
...
export function serializeSequenceDiagram(doc: DiagramDocument): string {
  ...
  if (script.autonumber) {
    ...
    out.push(
      `    autonumber${start !== undefined ? ` ${start}` : ''}${step !== undefined ? ` ${step}` : ''}`
    );
  }
  ...
  for (const participant of script.participants) {
    ...
    const alias =
      participant.label && participant.label !== participant.id ? ` as ${participant.label}` : '';
  ...
  const writeSteps = (steps: SequenceStep[], indent: string) => {
    for (const step of steps) {
      ...
      if (step.kind === 'destroy') {
        out.push(`${indent}destroy ${step.participant}`);
        continue;
      }
      if (step.kind === 'create') {
