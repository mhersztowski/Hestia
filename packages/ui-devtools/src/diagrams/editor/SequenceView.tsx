...
function BlockLabel({ x, y, text }: { x: number; y: number; text: string }) {
  ...
  return (
    <g>
      ...
      <text x={x} y={y} fontSize={10} fill="#64748b">
        {label}
      </text>
    </g>
  );
    ...
    >
      {participant.isActor ? (
        ...
        <g stroke={stroke} fill="none" strokeWidth={1.4}>
          ...
          <path
            d={`M${participant.x} ${top + 14} v12 M${participant.x - 8} ${top + 19} h16 M${participant.x} ${top + 26} l-6 8 M${participant.x} ${top + 26} l6 8`}
          />
        </g>
      ) : (
        <rect
      ...
      <text
        x={participant.x}
        y={top + (participant.isActor ? 48 : 24)}
        textAnchor="middle"
        fill="#0f172a"
        fontWeight={600}
      >
  ...
  return (
    <div style={{ position: 'relative', width: layout.width }}>
      ...
      >
        ...
        {layout.blocks.map((block) => {
          ...
          return (
            ...
            >
              ...
              />
              {!fill && (
                <rect x={block.x} y={block.y} width={54} height={17} fill="#e2e8f0" rx={2} />
              )}
              {!fill && (
                <text x={block.x + 6} y={block.y + 12.5} fontSize={10} fill="#475569">
                  {block.block}
                </text>
              )}
              {!fill && block.title && (
                <BlockLabel x={block.x + 62} y={block.y + 12.5} text={block.title} />
              )}
        ...
        {layout.participants.map((participant) => {
          const top =
            participant.spawnY !== undefined ? participant.spawnY + 40 : layout.headHeight;
        ...
        {layout.activations.map((activation, i) => (
          <rect
            ...
            y={activation.top}
            width={10}
            height={Math.max(activation.bottom - activation.top, 8)}
            fill="#e2e8f0"
            stroke={LINE}
            strokeWidth={1}
          />
        ...
        {layout.notes.map((note) => (
          ...
          >
            ...
            />
            <text x={note.x + 8} y={note.y + 18} fontSize={11} fill="#713f12">
              {note.text}
            </text>
          </g>
        ))}
        ...
        {layout.messages.map((message) => {
          ...
          if (message.selfCall) {
            ...
            return (
              <g
                key={`m${message.path.join('.')}`}
                onClick={() => onSelect?.(message.path)}
                style={{ cursor: onSelect ? 'pointer' : 'default' }}
              >
                <path
                  d={d}
                ...
                />
                <text x={x + 42} y={message.y + 4} fontSize={11} fill="#334155">
                  {label}
                </text>
          ...
          return (
            ...
            >
              ...
              />
              <line
                x1={message.fromX + dir * 5}
                y1={message.y}
                x2={message.toX - dir * 5}
                y2={message.y}
                stroke={stroke}
                strokeWidth={active ? 2 : 1.4}
                strokeDasharray={isDotted(message.arrow) ? '5 4' : undefined}
