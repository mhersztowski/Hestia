...
export function GanttEditor({
  ...
  const task =
    selected?.task !== undefined
      ? chart.sections[selected.section]?.tasks[selected.task]
      : undefined;
  ...
  const setStartKind = (kind: 'none' | 'date' | 'after') => {
    ...
    if (kind === 'date')
      return patch({
        start: { kind: 'date', value: task?.start?.kind === 'date' ? task.start.value : '' },
      });
    return patch({
      start: { kind: 'after', ids: task?.start?.kind === 'after' ? task.start.ids : [] },
    });
  };
  const setEndKind = (kind: 'duration' | 'date' | 'until') => {
    if (kind === 'duration')
      return patch({
        end: { kind: 'duration', value: task?.end?.kind === 'duration' ? task.end.value : '1d' },
      });
    if (kind === 'date')
      return patch({
        end: { kind: 'date', value: task?.end?.kind === 'date' ? task.end.value : '' },
      });
    ...
    >
      {!readOnly && (
        ...
        >
          <button
            type="button"
            style={btn}
            onClick={() => emit(addSection(doc, 'Nowa sekcja', selected?.section))}
          >
            + Sekcja
          </button>
          <button
            ...
            onClick={() =>
              emit(addTask(doc, selected?.section ?? 0, 'Nowe zadanie', selected?.task))
            }
            title={
              chart.sections.length ? 'Dodaj zadanie w zaznaczonej sekcji' : 'Najpierw dodaj sekcję'
            }
          >
            + Zadanie
          </button>
          ...
          <span style={{ fontSize: 11, color: '#64748b' }}>
            ...
            {nieUlozone > 0 && (
              <span style={{ color: '#b91c1c' }}> · bez miejsca na osi: {nieUlozone}</span>
            )}
          </span>
        </div>
      )}
      ...
      <div ref={paneRef} style={{ flex: 1, minHeight: 0, overflow: 'auto', padding: 8 }}>
        <GanttView
          ...
          selected={
            selected?.task !== undefined
              ? { section: selected.section, task: selected.task }
              : undefined
          }
      ...
      {!readOnly && selected && selected.task === undefined && chart.sections[selected.section] && (
        ...
        >
          ...
          <button
            ...
            style={btn}
            title="Sekcja wyżej"
            disabled={selected.section === 0}
            onClick={() => {
              emit(moveSection(doc, selected.section, selected.section - 1));
              setSelected({ section: selected.section - 1 });
            }}
          >
            ↑
          </button>
          <button
            ...
            style={btn}
            title="Sekcja niżej"
            disabled={selected.section === chart.sections.length - 1}
            onClick={() => {
              emit(moveSection(doc, selected.section, selected.section + 1));
              setSelected({ section: selected.section + 1 });
            }}
          ...
          <button
            type="button"
            style={btn}
            title="Usuń sekcję razem z zadaniami"
            onClick={() => {
              emit(removeSection(doc, selected.section));
        ...
        >
          ...
          <div style={{ display: 'flex', gap: 3 }}>
            {GANTT_TAGS.map((tag) => (
              <button
                ...
                onClick={() =>
                  emit(toggleTag(doc, selected.section, selected.task!, tag as GanttTag))
                }
              >
                {tag}
              </button>
          ...
          {task.start?.kind === 'date' && (
            <label style={field('110px')}>
              ...
              <input
                ...
                onChange={(e) =>
                  patch({ start: { kind: 'date', value: e.target.value } as GanttStart })
                }
              />
            </label>
          )}
              ...
              <input
                ...
                onChange={(e) =>
                  patch({
                    start: {
                      kind: 'after',
                      ids: e.target.value.split(/\s+/).filter(Boolean),
                    } as GanttStart,
                  })
                }
              />
          ...
          {(task.end?.kind === 'duration' || task.end?.kind === 'date') && (
            <label style={field('100px')}>
              ...
              <input
                ...
                placeholder={
                  task.end.kind === 'duration' ? '5d' : (chart.dateFormat ?? 'YYYY-MM-DD')
                }
                onChange={(e) =>
                  patch({ end: { kind: task.end!.kind, value: e.target.value } as GanttEnd })
                }
              />
            </label>
          )}
          ...
          <div style={{ display: 'flex', gap: 3 }}>
            <button
              type="button"
              style={btn}
              title="Do sekcji wyżej"
              disabled={selected.section === 0}
              onClick={() => {
                emit(
                  moveTaskToSection(doc, selected.section, selected.task!, selected.section - 1)
                );
                setSelected({
                  section: selected.section - 1,
                  task: chart.sections[selected.section - 1].tasks.length,
                });
              }}
            >
              ↖ sekcja
            ...
            <button
              ...
              style={btn}
              title="Wyżej"
              disabled={selected.task === 0}
              onClick={() => {
                emit(moveTask(doc, selected.section, selected.task!, selected.task! - 1));
                setSelected({ section: selected.section, task: selected.task! - 1 });
              }}
            >
              ↑
            </button>
            ...
            <button
              ...
              title="Usuń zadanie"
              onClick={() => {
                emit(removeTask(doc, selected.section, selected.task!));
                setSelected({ section: selected.section });
              }}
            >
              ×
            </button>
          ...
          {/* Powód, dla którego zadanie nie trafiło na oś — przy samym zadaniu,
              nie w rogu ekranu. */}
          {schedule.tasks.find(
            (e) => e.sectionIndex === selected.section && e.taskIndex === selected.task
          )?.issue && (
