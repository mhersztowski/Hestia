...
export function NodeShapeBackground({
  ...
  switch (shape) {
      ...
      return svg(
        <>
          ...
          <circle
            cx={w / 2}
            cy={h / 2}
            r={Math.max(Math.min(w, h) / 2 - 5, 0.5)}
            fill="none"
            stroke={stroke}
    ...
    case 'hexagon':
      return svg(
        <polygon
          points={`${slant},1 ${w - slant},1 ${w - 1},${h / 2} ${w - slant},${h - 1} ${slant},${h - 1} 1,${h / 2}`}
          {...common}
        />
      );
    case 'parallelogram':
      return svg(
        <polygon points={`${slant},1 ${w - 1},1 ${w - slant},${h - 1} 1,${h - 1}`} {...common} />
      );
    case 'parallelogramAlt':
      return svg(
        <polygon points={`1,1 ${w - slant},1 ${w - 1},${h - 1} ${slant},${h - 1}`} {...common} />
      );
    case 'trapezoid':
      return svg(
        <polygon points={`${slant},1 ${w - slant},1 ${w - 1},${h - 1} 1,${h - 1}`} {...common} />
      );
    case 'trapezoidAlt':
      return svg(
        <polygon points={`1,1 ${w - 1},1 ${w - slant},${h - 1} ${slant},${h - 1}`} {...common} />
      );
    case 'asymmetric':
      // Chorągiewka: lewy bok wcięty w „strzałkę".
      return svg(
        <path
          d={`M1,1 L${w - 1},1 L${w - 1},${h - 1} L1,${h - 1} L${slant + 6},${h / 2} Z`}
          {...common}
        />
      );
    case 'choice':
