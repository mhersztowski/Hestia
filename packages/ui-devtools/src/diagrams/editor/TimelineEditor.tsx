      ...
      {!readOnly && (
        ...
        >
          <button
            type="button"
            style={btn}
            onClick={() => emit(addTimelineSection(doc, 'Nowa sekcja', selected?.section))}
          >
            + Sekcja
          </button>
          <button
            ...
            style={btn}
            onClick={() =>
              emit(addPeriod(doc, selected?.section ?? 0, 'Nowy okres', selected?.period))
            }
          ...
          <button
            ...
            title={
              selected?.period === undefined
                ? 'Zaznacz okres'
                : 'Dodaj wydarzenie w zaznaczonym okresie'
            }
          >
            + Wydarzenie
          </button>
          ...
          <span style={{ fontSize: 11, color: '#64748b' }}>
            sekcji: {timeline.sections.length} · okresów: {periodCount(timeline)} · wydarzeń:{' '}
            {eventCount(timeline)}
          </span>
        </div>
      )}
      ...
      <div style={{ flex: 1, minHeight: 0, overflow: 'auto', padding: '12px 8px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
          {timeline.sections.map((section, sectionIndex) => {
            ...
            const aktywnaSekcja =
              selected?.section === sectionIndex && selected.period === undefined;
            return (
              <div
                key={sectionIndex}
              ...
              >
                ...
                <div style={{ display: 'flex', gap: 0, alignItems: 'flex-start' }}>
                  {section.periods.map((period, periodIndex) => {
                    const aktywnyOkres =
                      selected?.section === sectionIndex &&
                      selected.period === periodIndex &&
                    ...
                    return (
                      <div
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          setSelected({ section: sectionIndex, period: periodIndex });
                        }}
                        style={{
                          width: PERIOD_WIDTH,
                          display: 'flex',
              ...
              <div style={{ display: 'flex', gap: 3 }}>
                <button
                  ...
                  onClick={() => {
                    ...
                    setSelected({ ...selected, event: selected.event! - 1 });
                  }}
                >
                  ←
                </button>
                <button
                  ...
                  style={btn}
                  title="Później"
                  disabled={selected.event === period.events.length - 1}
                  onClick={() => {
                    emit(
                      moveEvent(
                        doc,
                ...
                <button
                  ...
                  onClick={() => {
                    emit(removeEvent(doc, selected.section, selected.period!, selected.event!));
                    setSelected({ section: selected.section, period: selected.period });
                  }}
                >
                  ×
                </button>
            ...
            <>
              <label style={field('180px')}>
                ...
                <input
                  ...
                  value={period.label}
                  onChange={(e) =>
                    emit(updatePeriod(doc, selected.section, selected.period!, e.target.value))
                  }
                />
              </label>
              <div style={{ display: 'flex', gap: 3 }}>
                ...
                <button
                  type="button"
                  style={btn}
                  title="Do sekcji w prawo"
                  disabled={selected.section === timeline.sections.length - 1}
                  onClick={() => {
                    const target = selected.section + 1;
                    const index = timeline.sections[target].periods.length;
                    emit(movePeriodToSection(doc, selected.section, selected.period!, target));
                ...
                <button
                  ...
                  style={btn}
                  title="Wcześniej"
                  disabled={selected.period === 0}
                  onClick={() => {
                    emit(movePeriod(doc, selected.section, selected.period!, selected.period! - 1));
                    setSelected({ section: selected.section, period: selected.period! - 1 });
                  }}
                >
                  ←
                </button>
                <button
                  ...
                  style={btn}
                  title="Później"
                  disabled={selected.period === section.periods.length - 1}
                  onClick={() => {
                    emit(movePeriod(doc, selected.section, selected.period!, selected.period! + 1));
                    setSelected({ section: selected.section, period: selected.period! + 1 });
                  }}
                ...
                <button
                  type="button"
                  style={btn}
                  title="Usuń okres razem z wydarzeniami"
                  onClick={() => {
                    emit(removePeriod(doc, selected.section, selected.period!));
            ...
            <>
              <label style={field('180px')}>
                ...
                <input
                  ...
                  onChange={(e) =>
                    emit(updateTimelineSection(doc, selected.section, e.target.value))
                  }
                />
              </label>
              <div style={{ display: 'flex', gap: 3 }}>
                <button
                  ...
                  disabled={selected.section === 0}
                  onClick={() => {
                    emit(moveTimelineSection(doc, selected.section, selected.section - 1));
                ...
                <button
                  type="button"
                  style={btn}
                  title="Usuń sekcję razem z okresami"
                  onClick={() => {
                    emit(removeTimelineSection(doc, selected.section));
