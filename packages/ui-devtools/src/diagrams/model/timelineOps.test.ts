...
describe('sekcje', () => {
  it('dokłada, zmienia nazwę, przesuwa i usuwa', () => {
    expect(
      addTimelineSection(docOf(BASE), 'Środek', 0).timeline!.sections.map((s) => s.label)
    ).toEqual(['Początki', 'Środek', 'Rozkwit']);
    expect(updateTimelineSection(docOf(BASE), 1, 'Nowa').timeline!.sections[1].label).toBe('Nowa');
    expect(moveTimelineSection(docOf(BASE), 1, 0).timeline!.sections.map((s) => s.label)).toEqual([
      'Rozkwit',
      'Początki',
    ]);
...
describe('okresy', () => {
  it('nowy okres nie ma wydarzeń', () => {
    expect(addPeriod(docOf(BASE), 0, '2003').timeline!.sections[0].periods[2]).toEqual({
      label: '2003',
      events: [],
    });
  });
  ...
  it('wstawia okres pod wskazanym', () => {
    const labels = addPeriod(docOf(BASE), 0, '2003', 0).timeline!.sections[0].periods.map(
      (p) => p.label
    );
  ...
  it('zmienia etykietę i kolejność', () => {
    expect(updatePeriod(docOf(BASE), 0, 0, '2001').timeline!.sections[0].periods[0].label).toBe(
      '2001'
    );
    expect(
      movePeriod(docOf(BASE), 0, 1, 0).timeline!.sections[0].periods.map((p) => p.label)
    ).toEqual(['2004', '2002']);
  });
  ...
  it('przenosi okres do innej sekcji razem z wydarzeniami', () => {
    ...
    expect(next.timeline!.sections[1].periods[1]).toEqual({
      label: '2004',
  ...
  it('usuwa okres', () => {
    expect(
      removePeriod(docOf(BASE), 0, 0).timeline!.sections[0].periods.map((p) => p.label)
    ).toEqual(['2004']);
  });
...
describe('wydarzenia', () => {
  it('dokłada na koniec i pod wskazanym', () => {
    expect(addEvent(docOf(BASE), 0, 1, 'Flickr').timeline!.sections[0].periods[1].events).toEqual([
      'Facebook',
      'Google',
      'Flickr',
    ]);
    expect(
      addEvent(docOf(BASE), 0, 1, 'Flickr', 0).timeline!.sections[0].periods[1].events
    ).toEqual(['Facebook', 'Flickr', 'Google']);
  });

  it('zmienia treść, kolejność i usuwa', () => {
    expect(
      updateEvent(docOf(BASE), 0, 0, 0, 'LinkedIn (start)').timeline!.sections[0].periods[0].events
    ).toEqual(['LinkedIn (start)']);
    expect(moveEvent(docOf(BASE), 0, 1, 1, 0).timeline!.sections[0].periods[1].events).toEqual([
      'Google',
      'Facebook',
    ]);
    expect(removeEvent(docOf(BASE), 0, 1, 0).timeline!.sections[0].periods[1].events).toEqual([
      'Google',
    ]);
  });
