...
function mapSection(
  timeline: Timeline,
  index: number,
  change: (section: TimelineSection) => TimelineSection
): Timeline {
  return {
    ...timeline,
    sections: timeline.sections.map((section, i) => (i === index ? change(section) : section)),
  };
}

export function addTimelineSection(
  doc: DiagramDocument,
  label: string,
  afterIndex?: number
): DiagramDocument {
...
export function updateTimelineSection(
  ...
  return withTimeline(doc, (timeline) =>
    mapSection(timeline, index, (section) => ({ ...section, label }))
  );
}
...
export function removeTimelineSection(doc: DiagramDocument, index: number): DiagramDocument {
  return withTimeline(doc, (timeline) => ({
    ...timeline,
    sections: timeline.sections.filter((_, i) => i !== index),
  }));
}

export function moveTimelineSection(
  doc: DiagramDocument,
  from: number,
  to: number
): DiagramDocument {
...
export function addPeriod(
  ...
  return withTimeline(doc, (timeline) => {
    ...
    if (!timeline.sections.length)
      return { ...timeline, sections: [{ periods: [{ label, events: [] }] }] };
    return mapSection(timeline, sectionIndex, (section) => {
      const periods = [...section.periods];
      periods.splice(afterIndex === undefined ? periods.length : afterIndex + 1, 0, {
        label,
        events: [],
      });
...
export function updatePeriod(
  ...
  return withTimeline(doc, (timeline) =>
    mapSection(timeline, sectionIndex, (section) => ({
      ...section,
      periods: section.periods.map((period, i) =>
        i === periodIndex ? { ...period, label } : period
      ),
    }))
  );
}
  ...
  return withTimeline(doc, (timeline) => {
    ...
    return {
      ...
      sections: timeline.sections.map((section, i) => {
        if (i === fromSection)
          return { ...section, periods: section.periods.filter((_, j) => j !== periodIndex) };
...
function mapPeriod(
  ...
  return withTimeline(doc, (timeline) =>
    mapSection(timeline, sectionIndex, (section) => ({
      ...section,
      periods: section.periods.map((period, i) =>
        i === periodIndex ? { ...period, events: change(period.events) } : period
      ),
    }))
  );
}
...
export function updateEvent(
  ...
  return mapPeriod(doc, sectionIndex, periodIndex, (events) =>
    events.map((e, i) => (i === eventIndex ? text : e))
  );
}
