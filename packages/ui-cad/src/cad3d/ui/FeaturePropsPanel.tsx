...
function DatumPointProps({
  f,
  onUpdate,
}: {
  f: DatumPointFeature;
  onUpdate: (p: Partial<Feature>) => void;
}) {
  return (
    <Vec3Field label="Pozycja" value={f.position} onChange={(v) => onUpdate({ position: v })} />
  );
}
function DatumLineProps({
  f,
  onUpdate,
}: {
  f: DatumLineFeature;
  ...
  return (
    <>
      ...
      <NumField
        label="Length"
        value={f.length}
        onChange={(v) => onUpdate({ length: Math.max(0.1, v) })}
        min={0.1}
        step={5}
      />
    </>
  ...
  return (
    <>
      ...
      <TextField
        ...
        size="small"
        fullWidth
        value={presetOf(f.normal)}
        onChange={(e) => {
          const p = e.target.value;
          if (p === 'XY') setNormal([0, 0, 1]);
          else if (p === 'XZ') setNormal([0, 1, 0]);
          else if (p === 'YZ') setNormal([1, 0, 0]);
        }}
        sx={{ mb: 1.5 }}
      >
      ...
      <NumField
        label="Rozmiar"
        value={f.size}
        onChange={(v) => onUpdate({ size: Math.max(1, v) })}
        min={1}
        step={5}
      />
    </>
  ...
  return (
    <>
      {f.plane === 'face' ? (
        ...
        <>
          <TextField
            ...
            fullWidth
            value={f.plane}
            onChange={(e) => onUpdate({ plane: e.target.value as SketchFeature['plane'] })}
            sx={{ mb: 1.5 }}
          >
      ...
      <Typography variant="caption" color="text.disabled" sx={{ display: 'block', mb: 1.5 }}>
        {f.projectData
          ? `${JSON.parse(f.projectData).entities?.length ?? 0} entities`
          : 'Empty sketch'}
      </Typography>
      {onEditSketch && (
        <Button
  ...
  return (
    <>
      ...
      <TextField
        ...
        fullWidth
        value={extrudeType}
        onChange={(e) => onUpdate({ extrudeType: e.target.value as ExtrudeType })}
        sx={{ mb: 1.5 }}
      >
        ...
        <FormControlLabel
          control={
            <Switch
              size="small"
              checked={f.symmetric}
              onChange={(e) => onUpdate({ symmetric: e.target.checked })}
            />
          }
      ...
      <TextField
        ...
        fullWidth
        value={direction}
        onChange={(e) => onUpdate({ direction: e.target.value as ExtrudeDirection })}
        sx={{ mb: 1.5 }}
      >
      ...
      <Typography variant="caption" color="text.disabled">
        {f.sketchId
          ...
          : f.entityIds.length === 0
            ? 'All CAD 2D entities (dynamic)'
            : `${f.entityIds.length} pinned entities`}
      </Typography>
    </>
  );
}

function PocketProps({
  f,
  features,
  onUpdate,
}: {
  ...
  return (
    <ExtrudeProps f={f as unknown as ExtrudeFeature} features={features} onUpdate={onUpdate} />
  );
}
...
function MirrorProps({
  ...
  const currentPlaneKey =
    f.planeMode === 'datum_plane' && f.datumPlaneId
  ...
  const handleCreateFromFace = () => {
    ...
    const id = onCreateDatumPlane(
      faceDatumParams.position,
      faceDatumParams.normal,
      faceDatumParams.size
    );
    onUpdate({ planeMode: 'datum_plane', datumPlaneId: id });
  };
  ...
  return (
    <>
      ...
      <RadioGroup
        value={mode}
        onChange={(e) => onUpdate({ mode: e.target.value as MirrorMode })}
        sx={{ mb: 1 }}
      >
        <FormControlLabel
          value="content"
        ...
        <>
          ...
          >
            <MenuItem value="" disabled>
              <em>Choose a feature…</em>
            </MenuItem>
            {candidates
              .filter((c) => !featureIds.includes(c.id))
              .map((c) => (
      ...
      >
        ...
        {datumPlanes.length > 0 && [
          ...
          ...datumPlanes.map((dp) => (
            <MenuItem key={dp.id} value={`datum:${dp.id}`}>
              {dp.name}
            </MenuItem>
          )),
        ]}
  ...
  return (
    <>
          ...
          facesToRemove.map((fr, idx) => (
            ...
            >
              <Typography variant="caption" sx={{ flex: 1, mr: 1, color: 'text.primary' }}>
                Face @ ({fr.hintPoint[0].toFixed(0)}, {fr.hintPoint[1].toFixed(0)},{' '}
                {fr.hintPoint[2].toFixed(0)})
              </Typography>
              <Button
                size="small"
                variant="text"
      ...
      <TextField
        ...
        fullWidth
        value={mode}
        onChange={(e) => onUpdate({ mode: e.target.value as ShellMode })}
        sx={{ mb: 1.5 }}
      >
      ...
      <TextField
        ...
        fullWidth
        value={joinType}
        onChange={(e) => onUpdate({ joinType: e.target.value as ShellJoinType })}
        sx={{ mb: 1.5 }}
      >
      ...
      <FormControlLabel
        ...
        label={<Typography variant="body2">Intersection</Typography>}
        sx={{ mb: 0.5 }}
      />
      ...
      <FormControlLabel
        ...
        label={<Typography variant="body2">Make thickness inwards</Typography>}
        sx={{ mb: 0.5 }}
      />
      ...
      <FormControlLabel
        ...
        label={<Typography variant="body2">Recompute on change</Typography>}
        sx={{ mb: 1 }}
      />
      ...
      <Typography
        variant="caption"
        color="text.disabled"
        sx={{ display: 'block', fontSize: 11, lineHeight: 1.3 }}
      >
        Shell turns the solid into a thin-walled one, Thickness thick. The faces on the list are
        removed, so the solid is open where they were.
      </Typography>
    </>
  );
}

function FilletProps({
  f,
  onUpdate,
  edgeParams,
}: {
  ...
  return (
    <>
        ...
        >
          {edges.length === 0 ? (
            ...
            edges.map((e, idx) => (
              ...
              >
                <Typography variant="caption" sx={{ flex: 1, mr: 1, color: 'text.primary' }}>
                  Edge @ ({e.hintPoint[0].toFixed(0)}, {e.hintPoint[1].toFixed(0)},{' '}
                  {e.hintPoint[2].toFixed(0)})
                </Typography>
                <Button
                  size="small"
                  variant="text"
      ...
      <NumField
        label="Radius"
        value={f.radius}
        onChange={(v) => onUpdate({ radius: Math.max(0.01, v) })}
        min={0.01}
        step={0.5}
      />
      ...
      <FormControlLabel
        ...
        label={<Typography variant="body2">Use all edges</Typography>}
        sx={{ mb: 0.5 }}
      />
      ...
      >
        <Typography
          variant="caption"
          sx={{ fontSize: 11, lineHeight: 1.3, display: 'block', fontWeight: 600 }}
        >
        ...
        <Typography variant="caption" sx={{ fontSize: 10.5, lineHeight: 1.3, display: 'block' }}>
          ...
          Switch Select mode to Edge → click an edge → "+ Add". Untick the feature in the tree to
          see the result.
        </Typography>
      </Box>
    </>
        ...
        {featureIds.length === 0 ? (
          ...
          featureIds.map((id) => {
            ...
            return (
              ...
              >
                <Typography variant="caption">{c?.name ?? id}</Typography>
                <Button size="small" sx={{ minWidth: 24, p: 0 }} onClick={() => removeFeature(id)}>
                  ×
                </Button>
              </Box>
            );
...
function LinearPatternProps({
  ...
  return (
    <>
      <RadioGroup
        value={mode}
        onChange={(e) => onUpdate({ mode: e.target.value as PatternMode })}
        sx={{ mb: 1 }}
      >
        <FormControlLabel
          value="content"
      ...
      <Box sx={{ border: '1px solid', borderColor: 'divider', borderRadius: 1, p: 1, mb: 1.5 }}>
        <TextField
          ...
          fullWidth
          value={f.direction}
          onChange={(e) => onUpdate({ direction: e.target.value as PatternDirection })}
          sx={{ mb: 1 }}
        >
        ...
        <FormControlLabel
          control={
            <Switch
              size="small"
              checked={f.reversed ?? false}
              onChange={(e) => onUpdate({ reversed: e.target.checked })}
            />
          }
          label={<Typography variant="body2">Reversed</Typography>}
          sx={{ mb: 0.5 }}
        />
        <NumField
          label="Length"
        ...
        <NumField
          label="Occurrences"
          value={f.occurrences}
          onChange={(v) => onUpdate({ occurrences: Math.max(2, Math.round(v)) })}
          step={1}
          min={2}
        />
      </Box>
      ...
      <FormControlLabel
        ...
        label={<Typography variant="body2">Direction 2 (grid pattern)</Typography>}
        sx={{ mb: 0.5 }}
      />
      ...
      {dir2On && (
        <Box sx={{ border: '1px solid', borderColor: 'divider', borderRadius: 1, p: 1, mb: 1.5 }}>
          <TextField
            ...
            size="small"
            fullWidth
            value={f.direction2 ?? 'sketch_vertical'}
            onChange={(e) => onUpdate({ direction2: e.target.value as PatternDirection })}
            sx={{ mb: 1 }}
          >
          ...
          <NumField
            label="Length 2"
            value={f.length2 ?? 100}
            onChange={(v) => onUpdate({ length2: v })}
            step={10}
            min={0.1}
          />
          <NumField
            label="Occurrences 2"
            value={f.occurrences2 ?? 2}
            onChange={(v) => onUpdate({ occurrences2: Math.max(2, Math.round(v)) })}
            step={1}
            min={2}
          />
        </Box>
...
function PolarPatternProps({
  f,
  features,
  onUpdate,
}: {
  f: PolarPatternFeature;
  features: Feature[];
  onUpdate: (p: Partial<Feature>) => void;
}) {
  const mode = f.mode ?? 'tool_shapes';
    ...
    <>
      ...
      <Box sx={{ border: '1px solid', borderColor: 'divider', borderRadius: 1, p: 1, mb: 1.5 }}>
        <TextField
          ...
          fullWidth
          value={f.axis}
          onChange={(e) => onUpdate({ axis: e.target.value as PatternDirection })}
          sx={{ mb: 1 }}
        >
        ...
        <NumField
          label="Angle (°)"
          value={f.angle}
          onChange={(v) => onUpdate({ angle: Math.max(1, Math.min(360, v)) })}
          step={5}
          min={1}
          max={360}
        />
  ...
  return (
    <>
      ...
      <TextField
        ...
        fullWidth
        value={chamferType}
        onChange={(e) => onUpdate({ chamferType: e.target.value as ChamferType })}
        sx={{ mb: 1.5 }}
      >
      ...
      <NumField
        label="Size"
        value={f.size}
        onChange={(v) => onUpdate({ size: Math.max(0.01, v) })}
        min={0.01}
        step={0.5}
      />
      ...
      {chamferType === 'two_distances' && (
        <NumField
          label="Size 2"
          value={f.size2 ?? f.size}
          onChange={(v) => onUpdate({ size2: Math.max(0.01, v) })}
          min={0.01}
          step={0.5}
        />
      )}
      ...
      >
        ...
        <Typography variant="caption" sx={{ fontSize: 10.5, lineHeight: 1.3, display: 'block' }}>
          The scene shows the solid <b>BEFORE</b> this Chamfer, with its sharp edges. Select mode:
          Edge → click an edge → "+ Add".
        </Typography>
      </Box>
    </>
  );
...
function RevolveProps({
  ...
  const typeExt =
    f.revolveTypeExt ??
    (f.revolveType === 'through_all'
      ? 'to_last'
      : f.revolveType === 'symmetric'
        ? 'angle'
        : 'angle');
  const axisExt =
    f.axisExt ??
    (f.axis === 'X'
      ...
      : f.axis === 'Y'
        ...
        : f.axis === 'Z'
          ? 'Z'
          : f.axis === 'sketch_vertical'
            ? 'sketch_vertical'
            : f.axis === 'sketch_horizontal'
              ? 'sketch_horizontal'
              : 'Y');
  const autoRefresh = f.autoRefresh ?? true;
  ...
  const setAxisExt = (a: RevolveAxisExt, refId?: string) => {
    onUpdate({
      axisExt: a,
      axis: legacyAxis(a),
      ...(refId !== undefined ? { axisRefId: refId } : {}),
    });
  };

  const currentAxisKey =
    axisExt === 'datum_reference' && f.axisRefId ? `datum:${f.axisRefId}` : axisExt;
  ...
  return (
    <>
      ...
      <SketchSelect
        label="Sketch (profile to revolve)"
        value={f.sketchId}
        sketches={sketches}
        onChange={(id) => onUpdate({ sketchId: id })}
      />
      ...
      <Typography
        variant="caption"
        color="text.secondary"
        sx={{ display: 'block', mb: 1.5, fontSize: 11, lineHeight: 1.3 }}
      >
        The profile has to be ENTIRELY on one side of the axis. The orange line in the scene is the
        axis, shown while the Revolve is selected.
      </Typography>
      ...
      >
        ...
        <MenuItem value="to_first" disabled>
          To first
        </MenuItem>
        <MenuItem value="up_to_face" disabled>
          Up to face
        </MenuItem>
      ...
      >
        ...
        {axisRefs.length > 0 && [
          ...
          ...axisRefs.map((r) => (
            <MenuItem key={r.id} value={`datum:${r.id}`}>
              {r.name}
            </MenuItem>
          )),
        ]}
      ...
      {typeExt !== 'to_last' && typeExt !== 'to_first' && typeExt !== 'up_to_face' && (
        <NumField
          ...
          min={1}
          max={typeExt === 'two_angles' ? Math.max(1, 360 - (f.angle2 ?? 0)) : 360}
          step={5}
        />
      )}
      ...
      {typeExt === 'two_angles' && (
        <>
          ...
          <Typography
            ...
            sx={{ display: 'block', mb: 1.5, fontSize: 11, lineHeight: 1.3 }}
          >
            Suma Angle + Angle 2 = {f.angle + (f.angle2 ?? 0)}° (max 360°). The revolve runs from
            −Angle 2 to +Angle about the axis.
          </Typography>
        </>
      )}
      ...
      {typeExt === 'angle' && (
        <FormControlLabel
          control={
            <Switch
              size="small"
              checked={f.symmetric ?? false}
              onChange={(e) =>
                onUpdate({
                  symmetric: e.target.checked,
                  revolveType: e.target.checked ? 'symmetric' : 'dimension',
                })
              }
            />
          }
          label={<Typography variant="body2">Symmetric to plane</Typography>}
          sx={{ mb: 0.5 }}
        />
      )}
      ...
      <NumField
        label="Segments"
        value={f.segments}
        onChange={(v) => onUpdate({ segments: Math.max(3, Math.round(v)) })}
        min={3}
        max={128}
        step={1}
      />
  ...
  return (
    <>
      ...
      <TextField
        ...
        size="small"
        fullWidth
        value={f.cornerStyle ?? 'transformed'}
        onChange={(e) => onUpdate({ cornerStyle: e.target.value as SweepCornerStyle })}
        sx={{ mb: 1.5 }}
      >
      ...
      <TextField
        ...
        size="small"
        fullWidth
        value={f.orientationMode ?? 'standard'}
        onChange={(e) => onUpdate({ orientationMode: e.target.value as SweepOrientationMode })}
        sx={{ mb: 1.5 }}
      >
      ...
      <TextField
        ...
        size="small"
        fullWidth
        value={f.transformMode ?? 'constant'}
        onChange={(e) => onUpdate({ transformMode: e.target.value as SweepTransformMode })}
        sx={{ mb: 1.5 }}
      >
      ...
      {(!f.profileSketchId || !f.pathSketchId) && (
        <Typography variant="caption" color="warning.main" sx={{ display: 'block' }}>
          {!f.profileSketchId && !f.pathSketchId
            ? 'Profile and path sketches required.'
            : !f.profileSketchId
              ? 'Profile sketch required.'
    ...
    <>
      <FormControlLabel
        control={
          <Switch
            size="small"
            checked={f.ruled ?? false}
            onChange={(e) => onUpdate({ ruled: e.target.checked })}
          />
        }
        label="Ruled surface"
        sx={{ mb: 0.5 }}
      ...
      {available.length > 0 ? (
        <TextField
          ...
          value=""
          onChange={(e) => {
            if (e.target.value) onUpdate({ sections: [...sections, { sketchId: e.target.value }] });
          }}
          sx={{ mb: 1 }}
        >
          {available.map((s) => (
...
function GrooveProps({
  ...
  return (
    <RevolveProps f={f as unknown as RevolveFeature} features={features} onUpdate={onUpdate} />
  );
}

function LoftCutProps({
  f,
  features,
  onUpdate,
}: {
  ...
  return (
    <>
      ...
      <NumField
        label="Diameter"
        value={f.diameter ?? 6}
        onChange={(v) => onUpdate({ diameter: Math.max(0.1, v) })}
        min={0.1}
        step={0.5}
      />
      ...
      <TextField
        ...
        fullWidth
        value={depthType}
        onChange={(e) => onUpdate({ depthType: e.target.value as HoleDepthType })}
        sx={{ mb: 1.5 }}
      >
        <MenuItem value="dimension">Dimension</MenuItem>
        <MenuItem value="through_all">Through All</MenuItem>
      ...
      {depthType === 'dimension' && (
        <NumField
          label="Depth"
          value={f.depth ?? 25}
          onChange={(v) => onUpdate({ depth: Math.max(0.1, v) })}
          min={0.1}
          step={1}
        />
      )}
      ...
      <FormControlLabel
        control={
          <Switch
            size="small"
            checked={f.tapered ?? false}
            onChange={(e) => onUpdate({ tapered: e.target.checked })}
          />
        }
        label="Tapered"
        sx={{ mb: 0.5 }}
      ...
      {(f.tapered ?? false) && (
        <NumField
          label="Taper angle (°)"
          value={f.taperAngle ?? 90}
          onChange={(v) => onUpdate({ taperAngle: Math.max(1, Math.min(179, v)) })}
          min={1}
          max={179}
          step={1}
        />
      ...
      <TextField
        ...
        fullWidth
        value={drillPoint}
        onChange={(e) => onUpdate({ drillPoint: e.target.value as HoleDrillPoint })}
        sx={{ mb: 1.5 }}
      >
      ...
      {drillPoint === 'angled' && (
        <NumField
          label="Drill angle (°)"
          value={f.drillPointAngle ?? 118}
          onChange={(v) => onUpdate({ drillPointAngle: Math.max(1, Math.min(179, v)) })}
          min={1}
          max={179}
          step={1}
        />
      ...
      <TextField
        ...
        fullWidth
        value={counterType}
        onChange={(e) => onUpdate({ counterType: e.target.value as HoleCounterType })}
        sx={{ mb: 1.5 }}
      >
      ...
      {counterType !== 'none' && (
        <NumField
          ...
          value={f.counterDiameter ?? 10}
          onChange={(v) => onUpdate({ counterDiameter: Math.max((f.diameter ?? 6) + 0.1, v) })}
          min={0.1}
          step={0.5}
        />
      )}
      {counterType === 'counterbore' && (
        <NumField
          label="CB depth"
          value={f.counterDepth ?? 3}
          onChange={(v) => onUpdate({ counterDepth: Math.max(0.1, v) })}
          min={0.1}
          step={0.5}
        />
      )}
      {counterType === 'countersink' && (
        <NumField
          label="CS angle (°)"
          value={f.counterAngle ?? 90}
          onChange={(v) => onUpdate({ counterAngle: Math.max(1, Math.min(179, v)) })}
          min={1}
          max={179}
          step={1}
        />
  ...
  return (
    <>
      <TextField
        ...
        fullWidth
        value={f.axis ?? 'Y'}
        onChange={(e) => onUpdate({ axis: e.target.value as HelixAxis })}
        sx={{ mb: 1.5 }}
      >
        <MenuItem value="sketch_vertical">Vertical sketch axis</MenuItem>
        <MenuItem value="sketch_horizontal">Horizontal sketch axis</MenuItem>
      ...
      <TextField
        ...
        fullWidth
        value={mode}
        onChange={(e) => onUpdate({ mode: e.target.value as HelixMode })}
        sx={{ mb: 1.5 }}
      >
      ...
      {(mode === 'pitch_height' || mode === 'pitch_turns') && (
        <NumField
          label="Pitch"
          value={f.pitch ?? 10}
          onChange={(v) => onUpdate({ pitch: Math.max(0.1, v) })}
          min={0.1}
          step={1}
        />
      )}
      {(mode === 'pitch_height' || mode === 'turns_height') && (
        <NumField
          label="Height"
          value={f.height ?? 50}
          onChange={(v) => onUpdate({ height: Math.max(0.1, v) })}
          min={0.1}
          step={1}
        />
      )}
      {(mode === 'pitch_turns' || mode === 'turns_height') && (
        <NumField
          label="Turns"
          value={f.turns ?? 5}
          onChange={(v) => onUpdate({ turns: Math.max(0.25, v) })}
          min={0.25}
          step={0.25}
        />
      )}
      ...
      <NumField
        label="Radius"
        value={f.radius ?? 20}
        onChange={(v) => onUpdate({ radius: Math.max(0.1, v) })}
        min={0.1}
        step={1}
      />
      ...
      <FormControlLabel
        control={
          <Switch
            size="small"
            checked={f.leftHanded ?? false}
            onChange={(e) => onUpdate({ leftHanded: e.target.checked })}
          />
        }
        label="Left-handed"
        sx={{ mb: 0.5 }}
  ...
  return (
    ...
    >
      ...
      <Box sx={{ p: 1.5, overflowY: 'auto', flex: 1 }}>
        ...
        />
        {feature.type === 'sketch' && (
          <SketchProps f={feature as SketchFeature} onUpdate={up} onEditSketch={onEditSketch} />
        )}
        {feature.type === 'extrude' && (
          <ExtrudeProps f={feature as ExtrudeFeature} features={features} onUpdate={up} />
        )}
        {feature.type === 'pocket' && (
          <PocketProps f={feature as PocketFeature} features={features} onUpdate={up} />
        )}
        {feature.type === 'hole' && (
          <HoleProps f={feature as HoleFeature} features={features} onUpdate={up} />
        )}
        {feature.type === 'groove' && (
          <GrooveProps f={feature as GrooveFeature} features={features} onUpdate={up} />
        )}
        {feature.type === 'loft_cut' && (
          <LoftCutProps f={feature as LoftCutFeature} features={features} onUpdate={up} />
        )}
        {feature.type === 'mirror' && (
          <MirrorProps
            f={feature as MirrorFeature}
            features={features}
            onUpdate={up}
            onCreateDatumPlane={onCreateDatumPlane}
            faceDatumParams={faceDatumParams}
          />
        )}
        {feature.type === 'revolve' && (
          <RevolveProps f={feature as RevolveFeature} features={features} onUpdate={up} />
        )}
        {feature.type === 'shell' && (
          <ShellProps f={feature as ShellFeature} onUpdate={up} faceDatumParams={faceDatumParams} />
        )}
        {feature.type === 'fillet' && (
          <FilletProps f={feature as FilletFeature} onUpdate={up} edgeParams={edgeParams} />
        )}
        {feature.type === 'chamfer' && (
          <ChamferProps f={feature as ChamferFeature} onUpdate={up} edgeParams={edgeParams} />
        )}
        {feature.type === 'linear_pattern' && (
          <LinearPatternProps
            f={feature as LinearPatternFeature}
            features={features}
            onUpdate={up}
          />
        )}
        {feature.type === 'polar_pattern' && (
          <PolarPatternProps f={feature as PolarPatternFeature} features={features} onUpdate={up} />
        )}
        {feature.type === 'loft' && (
          <LoftProps f={feature as LoftFeature} features={features} onUpdate={up} />
        )}
        {feature.type === 'sweep' && (
          <SweepProps f={feature as SweepFeature} features={features} onUpdate={up} />
        )}
        {feature.type === 'sweep_cut' && (
          <SweepCutProps f={feature as SweepCutFeature} features={features} onUpdate={up} />
        )}
        {feature.type === 'helix' && (
          <HelixProps f={feature as HelixFeature} features={features} onUpdate={up} />
        )}
        {feature.type === 'datum_point' && (
          <DatumPointProps f={feature as DatumPointFeature} onUpdate={up} />
        )}
        {feature.type === 'datum_line' && (
          <DatumLineProps f={feature as DatumLineFeature} onUpdate={up} />
        )}
        {feature.type === 'datum_plane' && (
          <DatumPlaneProps f={feature as DatumPlaneFeature} onUpdate={up} />
        )}
        {feature.type === 'datum_cs' && (
          <DatumCsProps f={feature as DatumCsFeature} onUpdate={up} />
        )}
      </Box>
    </Box>
  );
