...
import type { SketchConstraint } from '../model/sketchConstraints';
import { constraintTypeLabel } from '../model/sketchConstraints';
    ...
    <Box sx={{ p: 1 }}>
      ...
      {filtered.length === 0 ? (
        <Typography variant="caption" color="text.disabled" sx={{ p: 1, display: 'block' }}>
          {constraints.length === 0 ? 'No constraints' : 'Nothing matches the filter'}
        </Typography>
      ) : (
          ...
          {filtered.map((c, idx) => (
            ...
            >
              <Checkbox
                ...
                onChange={(e) => {
                  e.stopPropagation();
                  onToggleVisibility(c.id, e.target.checked);
                }}
                sx={{ p: 0 }}
              />
...
/** A small red glyph for a constraint type, as FreeCAD draws them. */
function ConstraintGlyph({ type }: { type: SketchConstraint['type'] }) {
  ...
  switch (type) {
    case 'coincident':
      return (
        <svg {...commonProps}>
          ...
          <line x1="2" y1="14" x2="14" y2="2" stroke={color} strokeWidth="1.5" />
        </svg>
      );
    case 'horizontal':
      return (
    ...
    case 'vertical':
      return (
        <svg {...commonProps}>
          <line x1="8" y1="1" x2="8" y2="15" stroke={color} strokeWidth="2.5" />
        </svg>
      );
    case 'parallel':
      return (
        <svg {...commonProps}>
          <line x1="3" y1="2" x2="10" y2="14" stroke={color} strokeWidth="1.5" />
          <line x1="7" y1="2" x2="14" y2="14" stroke={color} strokeWidth="1.5" />
        </svg>
      );
    case 'perpendicular':
      return (
    ...
    case 'tangent':
      return (
        <svg {...commonProps}>
          <path d="M 2 12 Q 8 2 14 12" stroke={color} strokeWidth="1.5" fill="none" />
        </svg>
      );
    case 'equal':
      return (
      ...
      return (
        <svg {...commonProps}>
          <line
            ...
            markerStart="url(#a)"
          />
          <text x="4" y="6" fontSize="6" fill={color}>
            d
          </text>
        </svg>
      );
    case 'angle':
      return (
        <svg {...commonProps}>
          <path d="M 2 14 L 14 14 M 2 14 L 14 4" stroke={color} strokeWidth="1.5" fill="none" />
        </svg>
      );
    case 'fixed':
      return (
        <svg {...commonProps}>
          ...
          <line x1="8" y1="4" x2="8" y2="12" stroke={color} strokeWidth="1" />
          <line x1="4" y1="8" x2="12" y2="8" stroke={color} strokeWidth="1" />
        </svg>
      );
    default:
      // Symmetric, the distance dimensions, radius and diameter have no drawn
      // glyph — a dot keeps the row's layout instead of leaving a hole where an
      // icon should be. They do have FreeCAD icons, so this shows only when
      // those are not served (see `configureFreecadIcons`).
      return (
        <svg {...commonProps}>
...
export function ElementsPanel({ entities, onSelect, selectedId }: ElementsProps) {
  ...
  const filtered = entities.filter((e) => {
    if (!filter) return true;
    return (
      (e.name ?? e.type).toLowerCase().includes(filter.toLowerCase()) ||
  ...
  return (
    <Box sx={{ p: 1 }}>
      ...
      {filtered.length === 0 ? (
        <Typography variant="caption" color="text.disabled" sx={{ p: 1, display: 'block' }}>
          {entities.length === 0 ? 'The sketch is empty' : 'Nothing matches'}
        </Typography>
      ) : (
        <Box sx={{ maxHeight: 300, overflowY: 'auto' }}>
          {filtered.map((e, idx) => (
            ...
            >
              ...
              <Typography variant="caption" sx={{ flex: 1, color: 'text.primary' }}>
                {idx + 1}-{e.type}
                {e.name ? (
                  <span style={{ color: '#999', marginLeft: 4, fontSize: 10 }}>{e.name}</span>
                ) : null}
              </Typography>
            </Box>
          ))}
...
function ElementGlyph({ type }: { type: string }) {
  ...
  switch (type) {
    case 'line':
      return (
        <svg {...props}>
          ...
          <circle cx="3" cy="13" r="1.5" fill="#4caf50" />
          <circle cx="13" cy="3" r="1.5" fill="#f44336" />
        </svg>
      );
    case 'circle':
      return (
        <svg {...props}>
          ...
          <circle cx="8" cy="8" r="1" fill="#f44336" />
        </svg>
      );
    case 'rect':
      return (
        <svg {...props}>
          <rect x="2" y="4" width="12" height="8" fill="none" stroke={color} strokeWidth="1.5" />
        </svg>
      );
    case 'point':
      return (
    ...
    case 'arc':
      return (
        <svg {...props}>
          <path d="M 3 13 Q 8 2 13 13" stroke={color} strokeWidth="1.5" fill="none" />
        </svg>
      );
    default:
      return (
