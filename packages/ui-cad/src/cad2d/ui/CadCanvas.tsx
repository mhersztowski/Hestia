...
function dimensionPickDistance(
  dim: { x1: number; y1: number; x2: number; y2: number; offset: number },
  p: Point2D
): number {
  ...
  const nx = (-dy / len) * dim.offset,
    ny = (dx / len) * dim.offset;
  const a = { x: dim.x1 + nx, y: dim.y1 + ny },
    b = { x: dim.x2 + nx, y: dim.y2 + ny };
  const ex = b.x - a.x,
    ey = b.y - a.y;
...
export function CadCanvas({
  ...
  const selectDragRef = useRef<{ base: Point2D; orig: Map<string, Entity>; moved: boolean } | null>(
    null
  );
  // Sub-selection, for sketching: the picked sub-elements a constraint will apply to.
  const subSelRef = useRef<
    Array<{ ref: string; segs: Array<{ a: Point2D; b: Point2D }>; vertex?: Point2D }>
  >([]);
  // Draws the sub-selection highlight — what is picked, plus what is hovered — on the renderer's own layer.
  const renderSubHighlight = (
    hover?: { segs: Array<{ a: Point2D; b: Point2D }>; vertex?: Point2D } | null
  ) => {
  ...
  const getSnapPoint = useCallback(
    (
    ...
    ): { snapResult: SnapResult; pen: PenInput } | null => {
      const renderer = rendererRef.current;
      if (!renderer) return null;
      ...
      const snapRadius =
        pen.pointerType === 'pen' ? Math.max(20, Math.round(70 * (1 - pen.pressure * 0.65))) : 50;

      const nearby = project.entityRegistry.getInBoundingBox({
        minX: worldPt.x - snapRadius,
        minY: worldPt.y - snapRadius,
        maxX: worldPt.x + snapRadius,
        maxY: worldPt.y + snapRadius,
      });
      return {
        snapResult: project.snapEngine.snap(worldPt, nearby, renderer.getPixelToWorld()),
        pen,
      };
    },
  ...
  const handlePointerMove = useCallback(
    (e: React.PointerEvent<HTMLCanvasElement>) => {
      ...
      if (selectDragRef.current && mouseRef.current.isDown) {
        ...
        const dx = snap.point.x - d.base.x,
          dy = snap.point.y - d.base.y;
      ...
      const tool = tools[activeTool];
      tool.onPointerMove(snap.point, {
        project,
        snapResult: snap,
        pen,
        pixelToWorld: rendererRef.current?.getPixelToWorld() ?? 1,
      });
    ...
    (e: React.PointerEvent<HTMLCanvasElement>) => {
      ...
      if (activeTool === 'select' && subSelect && !is3d) {
        ...
        if (hit) {
          ...
          if (i >= 0)
            cur.splice(i, 1); // ponowny klik → odznacz
          ...
          for (const id of new Set(cur.map((s) => s.ref.split('.')[0])))
            if (!id.startsWith('#')) project.selectionManager.select(id, true);
      ...
      if (activeTool === 'select') {
        ...
        const sx = e.clientX - rect.left;
        const sy = e.clientY - rect.top;
        const picked = is3d ? renderer.pickEntity3d(sx, sy) : renderer.pickEntity(sx, sy);
        if (picked) {
    ...
    (e: React.PointerEvent<HTMLCanvasElement>) => {
        ...
        if (dd.moved && renderer) {
          ...
          if (final) {
            ...
            project.historyManager.push({
              type: 'update',
              description: 'Diameter dimension (angle and label)',
              undo: () => applyDim(orig),
              redo: () => applyDim(snapshot),
            });
      ...
      if (boxSelRef.current && renderer) {
        ...
        if (Math.hypot(up.x - bs.start.x, up.y - bs.start.y) < pxw * 4) {
          ...
          for (const p of picks)
            if (!cur.some((s) => s.ref === p.ref))
              cur.push({ ref: p.ref, segs: p.segs, vertex: p.vertex });
          project.selectionManager.clear();
          for (const id of new Set(cur.map((s) => s.ref.split('.')[0])))
            if (!id.startsWith('#')) project.selectionManager.select(id, true);
          project.eventBus.emit('selection:changed', project.selectionManager.getSelected());
      ...
      if (activeTool === 'select' && onDimensionClick && renderer) {
        ...
        const moved =
          ...
          (!!up && !!start && Math.hypot(up.x - start.x, up.y - start.y) > pxw * 4);
        if (up && start && !moved) {
          let bestId: string | null = null,
            bestD = pxw * 18; // ~18 px tolerancji
          for (const dim of project.entityRegistry.getByType('dimension')) {
            const d = dimensionPickDistance(
              dim as unknown as { x1: number; y1: number; x2: number; y2: number; offset: number },
              up
            );
            if (d < bestD) {
              bestD = d;
      ...
      if (selectDragRef.current) {
        ...
        if (d.moved && renderer) {
          ...
          for (const id of d.orig.keys()) {
            const en = project.entityRegistry.get(id);
            if (en) finals.set(id, en);
          }
  ...
  void zoomTick;
  void version;
  const committedDimLabels: DimensionLabel[] = is3d
    ? []
        ...
        .map((d) => {
          ...
          if (d.dimType === 'diameter') {
            const cx = (d.x1 + d.x2) / 2,
              cy = (d.y1 + d.y2) / 2;
            const ldx = (d.x1 - cx) / (l / 2 || 1),
              ldy = (d.y1 - cy) / (l / 2 || 1); // a unit vector towards p1
  ...
  const crosshair =
    !is3d && rendererNow && cursorActive && activeTool !== 'select'
      ? rendererNow.worldToScreen(cursorWorld.x, cursorWorld.y)
      : null;
  ...
  if (!is3d && rendererNow && previewGeom) {
    if (previewGeom.type === 'polyline') {
      ...
      const a = previewGeom.points[0],
        b = previewGeom.points[1];
      for (const c of [
        [a.x, a.y],
        [b.x, a.y],
        [b.x, b.y],
        ...
        >
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
            ...
            >
              <Box
                sx={{
                  width: `${penInput.pressure * 100}%`,
                  height: '100%',
                  bgcolor:
                    penInput.pressure > 0.7
                      ? '#4fc3f7'
                      : penInput.pressure > 0.35
                        ? '#81d4fa'
                        : '#b3e5fc',
        ...
        <>
          <Box
            sx={{
              ...
              top: crosshair.y,
              width: '100%',
              borderTop: '1px dashed rgba(120,200,255,0.4)',
              pointerEvents: 'none',
            }}
          />
          <Box
            sx={{
              ...
              left: crosshair.x,
              height: '100%',
              borderLeft: '1px dashed rgba(120,200,255,0.4)',
              pointerEvents: 'none',
            }}
          />
          <Box
...
function SnapGlyph({ kind }: { kind: 'vertex' | 'edge' | 'axis' }) {
  ...
  const style = {
    filter: 'drop-shadow(0 0 1.2px rgba(255,255,255,0.9)) drop-shadow(0 0 1px rgba(0,0,0,0.5))',
  } as const;
  if (kind === 'vertex') {
    return (
