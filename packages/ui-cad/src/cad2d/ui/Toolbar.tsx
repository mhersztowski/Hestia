...
import { Toolbar as CoreToolbar, item, separator, splitToggle, toggle, type ToolbarNode } from '@hestia/ui-core';
...
export function Toolbar({ activeTool, onToolChange, viewMode }: Props) {
  ...
  const nodes: ToolbarNode[] = useMemo(() => {
    ...
    const split = (g: Group) => {
      ...
      return splitToggle(g.key, cur.label, on, g.variants.map((v, i) => item(`${g.key}:${i}`, v.label, {
        icon: v.icon,
        onSelect: () => { setSel((s) => ({ ...s, [g.key]: i })); pick(v); },
      })), { group: 'tool', icon: cur.icon, title: cur.label, onChange: () => pick(cur) });
    };

    return [
      ...SIMPLE_TOOLS.map(plain),
      split(CIRCLE_GROUP), split(ARC_GROUP), split(RECT_GROUP),
      split(POLYGON_GROUP), split(SLOT_GROUP), split(BSPLINE_GROUP),
