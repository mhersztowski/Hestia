...
export function arcSlotOutline(
  center: Point2D,
  startPt: Point2D,
  endPt: Point2D,
  w: number
  ...
  const rOut = rc + w,
    rIn = Math.max(0.001, rc - w);
  const pt = (ang: number, r: number): Point2D => ({
...
export function rebuildConstruction(c: Construction): Point2D[] {
  if (c.kind === 'slot' && c.ctrl.length >= 2)
    return slotOutline(c.ctrl[0], c.ctrl[1], Math.max(0.01, c.radius ?? 1));
  if (c.kind === 'arcSlot' && c.ctrl.length >= 3)
    return arcSlotOutline(c.ctrl[0], c.ctrl[1], c.ctrl[2], Math.max(0.01, c.radius ?? 1));
  if (c.kind === 'bspline' && c.ctrl.length >= 2)
    return sampleSpline(c.ctrl, { interpolating: !!c.interpolating, periodic: !!c.periodic });
  return c.ctrl;
}
