import type { Entity, EntityInput, Point2D } from '../core';
import { closestPointOnSegment, offsetLineCoords, signedDistPointToLine } from '../core';
...
function buildOffsetGhostSegments(
  ...
  if (entity.type === 'line') {
    const side = signedDistPointToLine(
      cursor.x,
      cursor.y,
      entity.x1,
      entity.y1,
...
export class OffsetTool implements Tool {
  ...
  getDimensionLabels(): DimensionLabel[] {
    ...
    if (this.entity.type === 'line') {
      ...
      const dr = Math.sqrt(
        (this.cursor.x - this.entity.cx) ** 2 + (this.cursor.y - this.entity.cy) ** 2
      );
