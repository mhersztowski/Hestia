...
export function FileBrowser({
    open, mode, title, store, extension, defaultName = '',
    storageKey = DEFAULT_STORAGE_KEY, onClose, onOpen, onSave, onDone, rowActions,
}: FileBrowserProps) {
    ...
    useEffect(() => {
        try { localStorage.setItem(storageKey, dir); } catch { /* so be it */ }
    }, [dir, storageKey]);
    ...
    async function commitRename(from: string) {
        ...
        try {
            await store.rename(
                pathIn(dir, from + extension),
                pathIn(dir, to + extension),
            );
        ...
        <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
            ...
            <DialogContent sx={{ p: 0, display: 'flex', flexDirection: 'column', minHeight: 320 }}>
                ...
                {newFolder && canCreateDir && (
                    <Box sx={{
                        ...
                        />
                        <Button size="small" onClick={() => { void createFolder(); }} disabled={!folderName.trim() || busy}>
                            Create
                        </Button>
                        <Button size="small" color="inherit" onClick={() => { setNewFolder(false); setFolderName(''); }}>
                            Cancel
                        </Button>
                    </Box>
                )}
                            ...
                            {canGoUp && (
                                ...
                                >
                                    ...
                                    <ListItemText primary={
                                        <Typography variant="body2" sx={{ fontFamily: 'monospace', fontSize: 13 }}>..</Typography>
                                    } />
                                </ListItemButton>
                            )}
                                    ...
                                    <ListItemText
                                        primary={
                                            renaming === p.name ? (
                                                <TextField
                                                    ...
                                                    onBlur={() => { void commitRename(p.name); }}
                                                    onKeyDown={e => {
                                        ...
                                        secondary={
                                            <Typography variant="caption" sx={{ color: 'text.disabled' }}>
                                                {dateText(p.modified)}{p.size ? `  ·  ${sizeText(p.size)}` : ''}
                                            </Typography>
                                        }
                                    />
                                    ...
                                    <Box sx={{ display: 'flex', gap: 0.25, ml: 1, flexShrink: 0 }}>
                                        ...
                                        {canRename && (
                                            <Tooltip title="Rename">
                                                <IconButton size="small" onClick={e => startRename(p.name, e)} disabled={busy}>
                                        ...
                                        {canRemove && (
                                            <Tooltip title="Delete">
                                                <IconButton size="small" onClick={e => { void remove(p.name, e); }} disabled={busy} color="error">
                ...
                {mode === 'save' && (
                    <Box sx={{ px: 2, py: 1.5, borderTop: '1px solid', borderColor: 'divider' }}>
                        <TextField
                            ...
                            onKeyDown={e => { if (e.key === 'Enter' && canConfirm) void save(); }}
                            size="small"
                            fullWidth
                            autoFocus
                            slotProps={{
            ...
            <DialogActions sx={{ px: 2, py: 1.5 }}>
                ...
                <Button
                    ...
                    size="small"
                    disabled={!canConfirm}
                    onClick={mode === 'open' ? () => { void openFile(selected!); } : () => { void save(); }}
