...
import {
  FloatingToolbar, item, separator, submenu, toggle, type ToolbarNode,
} from '@hestia/ui-core';
...
function arrowSpine(s: NoteShape, sx: number, sy: number, curveSeg = 24): { draw: Pt[]; head: { from: Pt; tip: Pt } | null } {
  ...
  } else if (type === 'curved') { // >2 vertices → a smooth curve
    draw = catmullRom(verts, Math.max(3, Math.round(curveSeg / 2))); from = draw[draw.length - 2];
  } else { // straight (or an elbow with >2 vertices) → a polyline
...
const SHAPE_LABEL: Record<ShapeKind, string> = { rect: 'Rectangle', diamond: 'Diamond', circle: 'Circle', line: 'Line', arrow: 'Arrow' };
...
function buildShapePath(s: NoteShape, sx: number, sy: number): Path2D {
  ...
  else { // line / arrow (handles multiple vertices plus the shaft type)
...
export function SpenNotesView({ store, onFileOps, toolbarStart }: SpenNotesViewProps = {}) {
  ...
  const notesViewerUrl = notesFile && store?.viewerUrl
  ...
  const fileOps = useMemo<FileOps>(() => ({
    ...
    exportItems: [
      { label: 'Export PNG', run: () => { void exportNotes('png'); } },
      { label: 'Export SVG', run: () => { void exportNotes('svg'); } },
      { label: 'Export PDF', run: () => { void exportNotes('pdf'); } },
    ],
  ...
  const moveDragRef = useRef<{
    lastX: number; lastY: number; ids: Set<string>; moved: boolean;
  ...
  const transformRef = useRef<{
    mode: 'scale' | 'rotate';
    ox: number; oy: number;        // the opposite corner (LOCAL, unrotated) — the fixed point of the scale
    cx: number; cy: number;        // the bbox centre
    startX: number; startY: number; // the grabbed corner (LOCAL) for scaling
  ...
  const redraw = useCallback(() => {
    ...
    if (toolRef.current === 'select' && selectedRef.current.size > 0 && !marqueeRef.current && !editPointsRef.current) {
      ...
      if (gz) {
        ...
        const head = mr * 0.26;
        ctx.beginPath();
        ctx.moveTo(ctr.bx - arm, ctr.by); ctx.lineTo(ctr.bx + arm, ctr.by);
        ctx.moveTo(ctr.bx, ctr.by - arm); ctx.lineTo(ctr.bx, ctr.by + arm);
        for (const [dx, dy] of [[-1, 0], [1, 0], [0, -1], [0, 1]] as const) {
          const tipX = ctr.bx + dx * arm, tipY = ctr.by + dy * arm;
  ...
  useEffect(() => {
    ...
    const beginStroke = (e: PointerEvent) => {
      ...
      try { c.setPointerCapture(e.pointerId); } catch { /* touch may refuse — ignore */ }
      ...
      if (t === 'select') {
        ...
        if (gz) {
          ...
          if (Math.hypot(x - ctrW.x, y - ctrW.y) <= tol * 1.3) {
            moveDragRef.current = { lastX: x, lastY: y, ids: new Set(selectedRef.current), moved: false, fromCentre: true };
            return;
          }
    ...
    const endStroke = (e: PointerEvent) => {
      ...
      if (t === 'select') {
        ...
        if (moveDragRef.current) {
          const md = moveDragRef.current;
          moveDragRef.current = null;
          if (md.moved) { commitElements(applyArrowBindings(elementsRef.current)); return; }
    ...
    const onDown = (e: PointerEvent) => {
      ...
      if (e.pointerType === 'touch') {
        ...
        touchGestureRef.current = false;
        if (penIsAbout()) { startTouchPan(e); return; }
        beginStroke(e);
        return;
      }
      beginStroke(e);
    ...
    const onMove = (e: PointerEvent) => {
      ...
      if (e.pointerType === 'touch') {
        ...
        if (singleTouchPanRef.current) { doTouchPan(e); return; }
        extendStroke(e);
        return;
      }
      extendStroke(e);
    ...
    const onUp = (e: PointerEvent) => {
      ...
      if (e.pointerType === 'touch') {
        ...
        if (activePointersRef.current.size === 0) {
          touchGestureRef.current = false;
          singleTouchPanRef.current = null;
          if (wasGesture) { abortActiveDraw(); return; } // the gesture ended — do not commit the stroke
        }
        if (wasGesture) return; // fingers still on the screen after a pinch — do not draw
        if (wasPan) { singleTouchPanRef.current = null; return; } // it moved the view; there is no stroke to end
        endStroke(e);
        return;
      }
      endStroke(e);
    ...
    const onCancel = (e: PointerEvent) => {
      ...
      if (e.pointerType === 'touch') {
        ...
        if (singleTouchPanRef.current) { singleTouchPanRef.current = null; return; }
      }
  ...
  const handleCanvasClick = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (eyedropperRef.current) {
      ...
      } catch { /* getImageData may throw on a tainted canvas — ignore */ }
      setEyedropper(null);
      return;
    }
  ...
  const leftToolbar: ToolbarNode[] = useMemo(() => {
    ...
    const flyout = (id: 'color' | 'bg' | 'style' | 'text' | 'arrow', title: string, icon: ReactNode, hidden = false) =>
    ...
    const withSwatch = (icon: ReactNode, value: string, round: boolean) => (
      <Box sx={{ position: 'relative', display: 'flex' }}>{icon}{swatch(value, round)}</Box>
    );
    ...
    return [
      ...
      flyout('bg', 'Object fill colour', withSwatch(<FormatColorFillIcon sx={{ fontSize: 20 }} />, fillColor, false), !!lineStyleCtx),
      ...
      toggle('points', 'Edit points (add/move vertices)', editPointsId !== null && editPointsId === singleLinearId, {
      ...
      item('duplicate', 'Duplicate', {
        ...
        title: 'Duplicate', disabled: nothingSelected, onSelect: handleDuplicate,
      }),
      item('delete', 'Delete', {
        icon: <DeleteOutlineIcon sx={{ fontSize: 20, color: nothingSelected ? undefined : '#f44336' }} />,
        title: 'Delete', disabled: nothingSelected, onSelect: handleDeleteSelected,
      }),
      submenu('order', 'Order', [
        item('front', 'Bring to front', { icon: <FlipToFrontIcon fontSize="small" />, onSelect: () => reorderSelected('front') }),
        item('up', 'Bring forward', { icon: <ArrowUpwardIcon fontSize="small" />, onSelect: () => reorderSelected('up') }),
        item('down', 'Send backward', { icon: <ArrowDownwardIcon fontSize="small" />, onSelect: () => reorderSelected('down') }),
        item('back', 'Send to back', { icon: <FlipToBackIcon fontSize="small" />, onSelect: () => reorderSelected('back') }),
    ...
    arrowStyleCtx, color, editPointsId, fillColor, handleDeleteSelected, handleDuplicate,
    lineStyleCtx, reorderSelected, selectedIds, sidePanel, singleLinearId,
  ]);
  ...
  return (
    <Box sx={{ display: 'flex', height: '100%', overflow: 'hidden', bgcolor: 'background.default' }}>
          ...
          {ownFileMenu && (
            <>
              <Tooltip title={fileOps.currentName ?? 'File'}>
                <IconButton size="small" onClick={e => setFileAnchor(e.currentTarget)} sx={{ color: 'text.secondary' }}>
              ...
              <Menu anchorEl={fileAnchor} open={Boolean(fileAnchor)} onClose={() => setFileAnchor(null)}>
                ...
                {fileOps.store.map(item => (
                  <MenuItem key={item.label} onClick={() => { setFileAnchor(null); item.run(); }}>
                    {item.label}
                  </MenuItem>
                ))}
          ...
          */}
          ...
          {sidePanel && (
            <Box sx={{
              ...
              {sidePanel === 'bg' && (
                <ColorPickerPanel
                  title="Object fill colour" value={fillColor} allowTransparent
              ...
              {sidePanel === 'style' && imageSelected && (
                <Box sx={{ width: 232 }}>
                  ...
                  <Box sx={{ display: 'flex', gap: 0.5, mb: 1.25 }}>
                    {([
                      { rounded: false, label: 'Sharp corners',
              ...
              {sidePanel === 'style' && !imageSelected && !shapeStyleCtx && (
                <Box sx={{ width: 232 }}>
                  ...
                  <Box sx={{ display: 'flex', gap: 0.5, mb: 1.25 }}>
                    {([
                      { lvl: 'low' as const, label: 'Low — even width',
                        svg: <rect x="2" y="6" width="26" height="3" rx="1.5" fill="currentColor" /> },
                      { lvl: 'high' as const, label: 'High — width follows pressure',
                ...
                <Box sx={{ width: 232 }}>
                  ...
                  <Box sx={{ display: 'flex', gap: 0.5, mb: 1.25 }}>
                    {([
                      ...
                      { v: 'end' as ArrowHeads, label: 'Head at the end',
                  ...
                  <Box sx={{ display: 'flex', gap: 0.5, mb: 0.5 }}>
                    {([
                      ...
                      { v: 'curved' as ArrowType, label: 'Curved',
                      ...
                      { v: 'elbow' as ArrowType, label: 'Elbow',
                ...
                return (
                  <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.75 }}>
                    ...
                    {target === null && propEl.linkTo && (
                      <Typography variant="caption" sx={{ color: 'warning.main' }}>The target does not exist (deleted)</Typography>
                    )}
                  </Box>
                );
              })()}
          ...
          return (
            <Box sx={{ p: 0.75, display: 'flex', flexDirection: 'column', gap: 0.25, minWidth: 160 }}>
              ...
              <Button size="small" startIcon={<OpenInNewIcon />} sx={{ justifyContent: 'flex-start' }}
                onClick={() => { if (url) window.open(url, '_blank', 'noopener'); setLinkPopup(null); }}>Open</Button>
              <Button size="small" startIcon={<EditIcon />} sx={{ justifyContent: 'flex-start' }}
                onClick={() => { setLinkDialog({ id: linkPopup.id, url: url || '' }); setLinkPopup(null); }}>Edit</Button>
              <Button size="small" color="error" startIcon={<LinkOffIcon />} sx={{ justifyContent: 'flex-start' }}
                onClick={() => { setElementLink(linkPopup.id, ''); setLinkPopup(null); }}>Remove link</Button>
            </Box>
          );
        })()}
      </Popover>
      ...
      <Dialog open={Boolean(linkDialog)} onClose={() => setLinkDialog(null)} maxWidth="xs" fullWidth>
        ...
        <DialogActions>
          {linkDialog && (
            <Button color="error" onClick={() => { setElementLink(linkDialog.id, ''); setLinkDialog(null); }}>Remove</Button>
          )}
          <Box sx={{ flex: 1 }} />
      ...
      <Dialog open={Boolean(linkToPicker)} onClose={() => setLinkToPicker(null)} maxWidth="xs" fullWidth>
        ...
        <DialogContent dividers sx={{ p: 0 }}>
          {(() => {
            ...
            if (!els.length) return <Typography variant="body2" sx={{ p: 2, color: 'text.secondary' }}>There are no other objects on this page.</Typography>;
            return (
      ...
      <Dialog open={Boolean(infoMdDialog)} onClose={() => setInfoMdDialog(null)} maxWidth="sm" fullWidth>
        ...
        <DialogContent dividers>
          ...
          {infoMdDialog?.text.trim() && (
            <>
              <Typography variant="caption" sx={{ color: 'text.secondary', display: 'block', mt: 1.5, mb: 0.5 }}>Preview</Typography>
