...
export const mjdFieldTypeSchema = z.enum(['string', 'number', 'boolean', 'date', 'enum', 'array']);

export const mjdFieldDefSchema = z
  .object({
    name: z.string().min(1),
  ...
  .refine((f) => f.type !== 'enum' || (f.options && f.options.length > 0), {
    message: 'Enum fields must have at least one option',
  })
  .refine((f) => f.type !== 'array' || !!f.itemType, {
