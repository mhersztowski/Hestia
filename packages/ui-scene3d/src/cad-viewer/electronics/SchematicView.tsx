...
function PartBody({ part, w, h }: { part: PartDef; w: number; h: number }) {
  ...
  if (part.bodyShape === 'breadboard') {
    ...
    const oX = 1,
      oTop = 0,
      oMain = 3,
      oBot = oMain + mainH + gapH + mainH + gapH;
    return (
      <g>
        <rect
          x={0}
        ...
        {['f', 'g', 'h', 'i', 'j'].map((l, i) => (
          <text
            key={l}
            x={px(oX) - 6}
            y={px(oMain + mainH + gapH + i) + GRID / 2 + 4}
            textAnchor="middle"
            fontSize={9}
            fill="#888"
          >
    ...
    return (
      <g>
        ...
        {part.label && (
          ...
          >
            {part.label.split('\n').map((line, i, arr) => (
              <tspan key={i} x={px(w) / 2} dy={i === 0 ? -(arr.length - 1) * 6 : 12}>
                {line}
              </tspan>
            ))}
          </text>
  ...
  if (part.bodyShape === 'led') {
    ...
    return (
      <g>
        ...
        />
        <polygon
          points={`${bodyX},${GRID / 2 - 10} ${bodyX},${GRID / 2 + 10} ${bodyX + domeR},${GRID / 2}`}
          fill={color}
          stroke={part.bodyColor}
          strokeWidth={1}
        />
        <circle
          cx={bodyX + domeR}
  ...
  if (part.bodyShape === 'transistor') {
    ...
    return (
      <g>
        <path
          d={`M ${cx} ${cy - bodyR} A ${bodyR} ${bodyR} 0 1 1 ${cx} ${cy + bodyR} L ${cx} ${cy - bodyR} Z`}
          fill={part.bodyColor}
          stroke={strokeColor}
          strokeWidth={strokeWidth}
        />
        ...
        <text x={cx - 4} y={cy + 4} fontSize={7} fontFamily="monospace" fill="white">
          {part.label}
        </text>
      </g>
    );
  ...
  return (
    <g>
      ...
      {part.pins
        .filter((p) => (p.y === h - 1 || p.y === h) && p.x !== 0 && p.x !== w - 1 && p.x !== w)
        .map((p) => (
          <line
            key={p.id}
            x1={px(p.x) + GRID / 2}
      ...
      {part.pins
        .filter(
          (p) => p.x !== 0 && p.x !== w - 1 && p.x !== w && p.y !== 0 && p.y !== h - 1 && p.y !== h
        )
        .map((p) => (
          <rect
            key={p.id}
...
function contentBoundsPx(schema: ElectronicsSchema) {
  ...
  for (const c of schema.components ?? []) {
    ...
    maxX = Math.max(maxX, c.x + ext);
    maxY = Math.max(maxY, c.y + ext);
  }
...
export function SchematicView({ schema }: { schema: ElectronicsSchema }) {
  ...
  useMemo(() => {
    (schema.embeddedParts ?? []).forEach(registerEmbeddedPart);
  }, [schema]);
  ...
  return (
    <div
      ref={containerRef}
      style={{ flex: 1, minWidth: 0, minHeight: 0, overflow: 'hidden', background: '#1a1a1a' }}
    >
      <svg
        ref={svgRef}
        width={size.w}
          ...
          {(schema.components ?? []).map((comp) => {
            ...
            return (
              ...
              >
                ...
                {comp.userLabel && (
                  <ComponentLabel part={part} text={comp.userLabel} rotation={comp.rotation} />
                )}
              </g>
            );
          })}
