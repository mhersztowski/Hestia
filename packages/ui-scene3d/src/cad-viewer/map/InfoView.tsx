...
export function InfoView({ node, onClose }: Props) {
      ...
      <Dialog open fullScreen onClose={onClose}>
        ...
        >
          <Toolbar variant="dense" sx={{ gap: 1, minHeight: 44 }}>
            <Typography
              sx={{ fontSize: '0.9rem', fontWeight: 600, flex: 1, color: 'text.primary' }}
            >
              {node.name}
            </Typography>
            {isUrl && (
              <Tooltip title="Open in new tab">
                <IconButton
                  size="small"
                  onClick={() => window.open(node.info, '_blank', 'noopener')}
                >
  ...
  return (
    ...
    >
      ...
      <Box sx={{ flex: 1, overflow: 'auto', p: isUrl ? 0 : 1.5 }}>{body}</Box>
    </Paper>
  );
}
