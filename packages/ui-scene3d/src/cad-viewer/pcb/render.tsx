...
export interface CompDef {
  ...
  refPrefix: string;
  pins: number;
  draw: (preview?: boolean) => ReactNode;
}
export interface EasyEdaSym {
  shapes: string[];
...
export interface PlacedComp {
  ...
  fpEls?: FpEl[];
  pcbX?: number;
  pcbY?: number; // pozycja footprintu na PCB (niezależna od pozycji symbolu x/y na schemacie)
...
const uprightT = (x: number, y: number, rot?: number, mir?: boolean): string | undefined => {
  const r = rot || 0;
  if (!r && !mir) return undefined;
  return mir
    ? `translate(${x} ${y}) scale(-1 1) rotate(${-r}) translate(${-x} ${-y})`
    : `rotate(${-r} ${x} ${y})`;
};
  ...
  fp.shapes.forEach((s, i) => {
    ...
    switch (t[0]) {
      case 'PAD': {
        ...
        if (holeR > 0)
          out.push(<circle key={`hole${i}`} cx={px} cy={py} r={holeR} fill="#111" stroke="none" />);
        if (num)
          out.push(
            <text
              key={`num${i}`}
              ...
              transform={uprightT(px, py + 0.6, tr, flip)}
            >
              {num}
            </text>
          );
      ...
      case 'TRACK':
        out.push(
          <polyline
            ...
            fill="none"
            stroke={col(t[2])}
            strokeWidth={Math.max(0.15, +t[1] || 0.6)}
            strokeLinecap="round"
      ...
      case 'CIRCLE':
        out.push(
          <circle
            ...
            fill="none"
            stroke={col(t[5])}
            strokeWidth={Math.max(0.12, +t[4] || 0.3)}
            opacity={preview ? 0.6 : 1}
          />
        );
        break;
      case 'ARC':
        out.push(
          <path
            ...
            fill="none"
            stroke={col(t[2])}
            strokeWidth={Math.max(0.15, +t[1] || 0.5)}
            opacity={preview ? 0.6 : 1}
          />
        );
        break;
      default:
        break; // SOLIDREGION / SVGNODE / TEXT — pomijamy
    }
  });
...
export function renderPcbPart(comp: PlacedComp, key: Key, preview = false, layers?: LayerState) {
  ...
  if (comp.fp) {
    ...
    if (comp.fp.bbox) {
      hw = comp.fp.bbox.width * 2;
      hh = comp.fp.bbox.height * 2;
    } // renderFootprint skaluje ×SC(4) → półwymiar = width·2
    ...
    hh = bb.h / 2;
    body = (
      <g transform={`translate(${-(bb.x + bb.w / 2)},${-(bb.y + bb.h / 2)})`}>
        {comp.fpEls.map((el, i) => renderFpEl(el, i, preview, layers))}
      </g>
    );
  } else {
    body = (
      <g>
        <rect
          ...
          stroke="#e8c84a"
          strokeWidth={1}
          strokeDasharray={preview ? '3 2' : undefined}
        />
      </g>
    );
  }
  ...
  return (
    <g key={key} transform={tf} opacity={preview ? 0.7 : 1}>
      {body}
      {comp.showPrefix !== 'Nie' && (
        <text
          x={0}
          y={refY}
          fontSize={refFs}
          fill={bottom ? '#3a6bd6' : '#e8c84a'}
          stroke="none"
          textAnchor="middle"
          fontFamily="sans-serif"
          transform={uprightT(0, refY, rot, bottom)}
        >
          {comp.ref}
        </text>
      )}
      {comp.showName === 'Tak' && (
        <text
          ...
          textAnchor="middle"
          fontFamily="sans-serif"
          transform={uprightT(0, nameY, rot, bottom)}
        >
          {comp.label}
        </text>
      )}
...
function parsePinLine(d: string, dot: { x: number; y: number }) {
  ...
  const inner =
    Math.hypot(ex - dot.x, ey - dot.y) >= Math.hypot(sx - dot.x, sy - dot.y)
      ? { x: ex, y: ey }
      : { x: sx, y: sy };
...
export function renderEasyEdaSymbol(
  sym: EasyEdaSym,
  preview?: boolean,
  tr?: number,
  mir?: boolean
  ...
  sym.shapes.forEach((s, i) => {
    ...
    switch (t[0]) {
      case 'R':
        ...
        break;
      case 'E':
        out.push(<ellipse key={i} cx={+t[1]} cy={+t[2]} rx={+t[3]} ry={+t[4]} {...p} />);
        break;
      case 'PL':
      case 'PG': {
        const pts = t[1];
        if (t[0] === 'PG') out.push(<polygon key={i} points={pts} {...p} />);
        else out.push(<polyline key={i} points={pts} {...p} />);
        break;
      }
      case 'PT':
      case 'A':
        out.push(<path key={i} d={t[1]} {...p} />);
        break;
      case 'P': {
        const segs = s.split('^^');
        const pathD = segs[2]?.split('~')[0];
        const ds = segs[1]?.split('~');
        const dot = ds && ds.length >= 2 ? { x: +ds[0], y: +ds[1] } : null;
        if (pathD) out.push(<path key={`pp${i}`} d={pathD} {...p} />);
        if (dot)
          out.push(
            <circle key={`pd${i}`} cx={dot.x} cy={dot.y} r={1.4} fill={RED} stroke="none" />
          );
        if (dot && pathD) {
          const { inner, dir } = parsePinLine(pathD, dot);
          const nameSeg = segs[3]?.split('~'),
            numSeg = segs[4]?.split('~');
          pins.push({
            i,
            dot,
            inner,
            dir,
            name: nameSeg && nameSeg[0] === '1' && nameSeg[4] ? nameSeg[4] : '',
            nameColor: nameSeg?.[8] || '#0000FF',
            num: numSeg && numSeg[0] === '1' && numSeg[4] ? numSeg[4] : '',
            numColor: numSeg?.[8] || '#0000FF',
          });
        }
        break;
      }
  ...
  const fs = 6,
    gap = 3;
  const sideKey = (pn: PinInfo) =>
    Math.abs(pn.dir.x) >= Math.abs(pn.dir.y)
      ? pn.dir.x >= 0
        ? 'L'
        : 'R'
      : pn.dir.y >= 0
  ...
  pins.forEach((pn) => {
    const g = groups.get(sideKey(pn)) || [];
    g.push(pn);
    groups.set(sideKey(pn), g);
  });
  groups.forEach((arr, k) => {
    ...
    const nameCol = fs * (maxNameLen * 0.62 + 1.5),
      numCol = fs * 1.5,
      half = fs * 0.62;
    ...
    arr.forEach((pn) => {
      ...
      if (pn.name) {
        ...
        const off = gap + row * nameCol;
        const nx = pn.inner.x + pn.dir.x * off,
          ny = pn.inner.y + pn.dir.y * off + (horiz ? fs * 0.35 : 0);
        out.push(
          <text
            key={`pname${pn.i}`}
            x={nx}
            ...
            textAnchor={anchor}
            fontFamily="sans-serif"
            transform={uprightT(nx, ny, tr, mir)}
          >
            {pn.name}
          </text>
        );
      ...
      if (pn.num) {
        const perp = { x: -pn.dir.y, y: pn.dir.x },
          sgn = perp.y <= 0 ? 1 : -1;
        const mx = (pn.dot.x + pn.inner.x) / 2,
          my = (pn.dot.y + pn.inner.y) / 2,
          off = gap + row * numCol;
        const qx = mx + perp.x * off * sgn,
          qy = my + perp.y * off * sgn - 0.6;
        out.push(
          <text
            key={`pnum${pn.i}`}
            x={qx}
            ...
            textAnchor="middle"
            fontFamily="sans-serif"
            transform={uprightT(qx, qy, tr, mir)}
          >
            {pn.num}
          </text>
        );
...
function drawGenericIC(pins: number, label: string, preview?: boolean) {
  ...
  const pitch = 12,
    h = Math.max(40, perSide * pitch + 10),
    w = Math.max(60, label.length * 6.5);
  const p = scProps(preview);
  const nodes: ReactNode[] = [];
  for (let i = 0; i < perSide; i++) {
    const y = -h / 2 + 12 + i * pitch;
    nodes.push(<line key={`l${i}`} x1={-w / 2 - 16} y1={y} x2={-w / 2} y2={y} {...p} />);
    nodes.push(
      <text key={`ln${i}`} x={-w / 2 + 3} y={y + 3} fontSize={7} fill={RED} stroke="none">
        {i + 1}
      </text>
    );
  }
  for (let i = 0; i < pins - perSide; i++) {
    const y = -h / 2 + 12 + i * pitch;
    nodes.push(<line key={`r${i}`} x1={w / 2} y1={y} x2={w / 2 + 16} y2={y} {...p} />);
    nodes.push(
      <text
        key={`rn${i}`}
        x={w / 2 - 3}
  ...
  return (
    <g>
      {<rect x={-w / 2} y={-h / 2} width={w} height={h} {...p} />}
      {nodes}
      {txt(0, -h / 2 - 6, label)}
    </g>
  );
...
const SCHEMATIC_LIB: CompDef[] = [
  {
    ...
    draw: (pv) => {
      ...
      return (
        <g>
          {txt(0, -10, 'R?')}
          <line x1={-25} y1={0} x2={-12} y2={0} {...p} />
          <path d="M-12 0 l3 -6 l6 12 l6 -12 l6 12 l3 -6" {...p} />
          <line x1={12} y1={0} x2={25} y2={0} {...p} />
        </g>
      );
    },
  },
  {
    ...
    draw: (pv) => {
      ...
      return (
        <g>
          {txt(0, -14, 'C?')}
          <line x1={-25} y1={0} x2={-4} y2={0} {...p} />
          <line x1={-4} y1={-9} x2={-4} y2={9} {...p} />
          <line x1={4} y1={-9} x2={4} y2={9} {...p} />
          <line x1={4} y1={0} x2={25} y2={0} {...p} />
        </g>
      );
    },
  },
  {
    ...
    draw: (pv) => {
      ...
      return (
        <g>
          {txt(0, -12, 'L?')}
          <line x1={-25} y1={0} x2={-15} y2={0} {...p} />
          <path d="M-15 0 a5 5 0 0 1 10 0 a5 5 0 0 1 10 0 a5 5 0 0 1 10 0" {...p} />
          <line x1={15} y1={0} x2={25} y2={0} {...p} />
        </g>
      );
    },
  },
  {
    ...
    draw: (pv) => {
      ...
      return (
        <g>
          {txt(0, -12, 'D?')}
          <line x1={-25} y1={0} x2={-7} y2={0} {...p} />
          <path d="M-7 -8 l14 8 l-14 8 Z" {...p} />
          <line x1={7} y1={-8} x2={7} y2={8} {...p} />
          <line x1={7} y1={0} x2={25} y2={0} {...p} />
        </g>
      );
    },
  },
    ...
    draw: (pv) => {
      ...
      return (
        <g>
          ...
          <line x1={7} y1={-8} x2={7} y2={8} {...p} />
          <line x1={7} y1={0} x2={25} y2={0} {...p} />
          <path d="M6 -10 l6 -6 m-2 -4 l4 0 0 4" {...p} />
        </g>
      );
    },
  },
  {
    ...
    draw: (pv) => {
      ...
      return (
        <g>
          {txt(14, -12, 'Q?')}
          <circle cx={0} cy={0} r={14} {...p} />
          <line x1={-20} y1={0} x2={-5} y2={0} {...p} />
          <line x1={-5} y1={-9} x2={-5} y2={9} {...p} />
          <line x1={-5} y1={-5} x2={8} y2={-12} {...p} />
          <line x1={-5} y1={5} x2={8} y2={12} {...p} />
          <line x1={8} y1={-12} x2={8} y2={-22} {...p} />
          <line x1={8} y1={12} x2={8} y2={22} {...p} />
        </g>
      );
    },
  },
  {
    ...
    draw: (pv) => {
      ...
      return (
        <g>
          <line x1={0} y1={-14} x2={0} y2={0} {...p} />
          <line x1={-11} y1={0} x2={11} y2={0} {...p} />
          <line x1={-7} y1={5} x2={7} y2={5} {...p} />
          <line x1={-3} y1={10} x2={3} y2={10} {...p} />
        </g>
      );
    },
  },
  ...
  },
  {
    ...
    draw: (pv) => {
      ...
      for (let i = 0; i < 4; i++) {
        const y = -21 + i * 14;
        n.push(<line key={i} x1={12} y1={y} x2={26} y2={y} {...p} />);
        n.push(<circle key={`c${i}`} cx={9} cy={y} r={2} fill={RED} stroke="none" />);
      }
      return (
        <g>
          {txt(0, -30, 'J?')}
          <rect x={-12} y={-28} width={24} height={56} {...p} />
          {n}
        </g>
      );
    },
...
export function renderPlaced(comp: PlacedComp, key: Key, preview = false) {
  ...
  const rot = comp.rotation || 0,
    bmir = comp.layer === 'Dolna warstwa'; // teksty utrzymywane „prosto" mimo rotacji/odbicia
  const body = comp.savedEls ? (
    <g>{comp.savedEls.map((el, i) => renderEl(el, i, preview, { rot, mir: bmir }))}</g>
  ) : comp.easyeda ? (
    renderEasyEdaSymbol(comp.easyeda, preview, rot, bmir)
  ) : comp.octopart || !def ? (
    drawGenericIC(comp.pins, comp.label, preview)
  ) : (
    def.draw(preview)
  );
  ...
  return (
    <g key={key} transform={`translate(${comp.x},${comp.y})`} opacity={preview ? 0.6 : 1}>
      <g transform={`rotate(${comp.rotation || 0})${mir}`}>{body}</g>
      {caption && (
        <text
          x={0}
          y={labelY}
...
export function placedBBox(comp: PlacedComp): { x: number; y: number; w: number; h: number } {
  if (comp.savedEls && comp.savedEls.length) {
    const bs = comp.savedEls.map(elBBox);
    const x0 = Math.min(...bs.map((b) => b.x)),
      y0 = Math.min(...bs.map((b) => b.y)),
      x1 = Math.max(...bs.map((b) => b.x + b.w)),
      y1 = Math.max(...bs.map((b) => b.y + b.h));
    return { x: comp.x + x0, y: comp.y + y0, w: x1 - x0, h: y1 - y0 };
  }
  if (comp.easyeda?.bbox)
    return {
      x: comp.x - comp.easyeda.bbox.width / 2,
      y: comp.y - comp.easyeda.bbox.height / 2,
      w: comp.easyeda.bbox.width,
      h: comp.easyeda.bbox.height,
    };
...
export function pcbPartBBox(comp: PlacedComp): { x: number; y: number; w: number; h: number } {
  const px = comp.pcbX ?? comp.x,
    py = comp.pcbY ?? comp.y; // pozycja footprintu na PCB (niezależna od symbolu)
  const b = comp.fp?.bbox;
  if (b) return { x: px - b.width * 2, y: py - b.height * 2, w: b.width * 4, h: b.height * 4 };
  if (comp.fpEls && comp.fpEls.length) {
    const bb = unionBB(comp.fpEls, elBBoxFp);
    return { x: px - bb.w / 2, y: py - bb.h / 2, w: bb.w, h: bb.h };
  }
...
const sa = (e: ShapeStyle, preview?: boolean) =>
  ({
    stroke: e.stroke || '#000000',
    strokeWidth: e.strokeWidth || 1,
    fill: e.fill && e.fill !== 'none' ? e.fill : 'none',
    strokeDasharray: preview ? '4 3' : dashOf(e.strokeStyle),
  }) as const;
...
const pinEnd = (p: PinEl) => {
  const r = ((p.rotation || 0) * Math.PI) / 180;
  return { x: p.x + Math.cos(r) * p.length, y: p.y + Math.sin(r) * p.length };
};
...
export function renderEl(el: El, key: Key, preview = false, upr?: { rot: number; mir: boolean }) {
  ...
  const selfRot = (x: number, y: number): string | undefined => {
    const r = (el as { rot?: number }).rot || 0,
      f = (el as { flip?: boolean }).flip;
  ...
  switch (el.t) {
    case 'pin': {
      const e = pinEnd(el),
        fs = parseInt(el.fontSize) || 7,
        out = Math.cos(((el.rotation || 0) * Math.PI) / 180) >= 0,
        sw = preview ? 1 : 1.4,
        dash = preview ? '4 3' : undefined;
      const nx = e.x + (out ? 4 : -4),
        ny = e.y + fs / 3,
        mx = (el.x + e.x) / 2,
        my = el.y - 3;
      return (
        <g key={key} opacity={preview ? 0.6 : el.show === false ? 0.35 : 1}>
          <line
            x1={el.x}
            y1={el.y}
            x2={e.x}
            y2={e.y}
            stroke={el.pinColor || SYM_COLOR}
            strokeWidth={sw}
          ...
          {el.dot ? (
            ...
            <circle cx={el.x} cy={el.y} r={1.4} fill={el.pinColor || SYM_COLOR} />
          )}
          {el.clock && (
            <path
              d={`M${e.x},${e.y - 4} L${e.x + (out ? 5 : -5)},${e.y} L${e.x},${e.y + 4}`}
              fill="none"
              stroke={el.pinColor || SYM_COLOR}
              strokeWidth={1}
            />
    ...
    case 'line':
      return (
        <polyline
          key={key}
          points={el.pts.map((p) => `${p.x},${p.y}`).join(' ')}
          {...sa(el, preview)}
        />
      );
    case 'polygon':
    ...
    case 'bezier': {
      ...
      return (
        <path
          key={key}
          d={c ? `M${a.x},${a.y} Q${b.x},${b.y} ${c.x},${c.y}` : `M${a.x},${a.y} L${b.x},${b.y}`}
          {...sa(el, preview)}
        />
      );
    }
    ...
    case 'arc':
      return (
        <path
          key={key}
          d={`M${el.cx - el.rx},${el.cy} A${Math.abs(el.rx)},${Math.abs(el.ry)} 0 0 1 ${el.cx + el.rx},${el.cy}`}
          {...sa(el, preview)}
        />
      );
    case 'pie': {
      const a0 = (-50 * Math.PI) / 180,
        a1 = (230 * Math.PI) / 180;
      const p0 = { x: el.cx + el.rx * Math.cos(a0), y: el.cy + el.ry * Math.sin(a0) },
        p1 = { x: el.cx + el.rx * Math.cos(a1), y: el.cy + el.ry * Math.sin(a1) };
      return (
        <path
          key={key}
          d={`M${el.cx},${el.cy} L${p0.x},${p0.y} A${Math.abs(el.rx)},${Math.abs(el.ry)} 0 1 1 ${p1.x},${p1.y} Z`}
          {...sa(el, preview)}
        />
      );
    }
    ...
    case 'text':
      return (
        <text
          ...
          x={el.x}
          y={el.y}
          fontSize={parseInt(el.fontSize) || 9}
          fill={el.color || '#0000FF'}
          fontFamily={el.fontFamily || 'sans-serif'}
          textAnchor={ANCHOR[el.anchor] || 'start'}
          fontWeight={el.fontWeight === 'bold' ? 'bold' : undefined}
          fontStyle={el.fontStyle === 'italic' ? 'italic' : undefined}
          stroke="none"
          opacity={preview ? 0.6 : 1}
          transform={upr ? ut(el.x, el.y) : selfRot(el.x, el.y)}
        >
          {el.text}
        </text>
      );
          ...
          {el.url ? (
            ...
            <>
              <rect
                ...
                strokeDasharray="4 3"
              />
              <circle cx={el.x + el.w * 0.28} cy={el.y + el.h * 0.32} r={4} fill="#888" />
            </>
          )}
        </g>
      );
    ...
    case 'netlabel':
      return (
        <text
          ...
          x={el.x}
          y={el.y}
          fontSize={parseInt(el.fontSize) || 7}
          fill={el.color || '#0000ff'}
          fontFamily={el.fontFamily || 'serif'}
          stroke="none"
          opacity={preview ? 0.6 : 1}
          transform={selfRot(el.x, el.y)}
        >
          {el.name}
        </text>
      );
    ...
    case 'netport':
    case 'probe': {
      const lp = netLabelPos(el),
        fs = parseInt(el.fontSize) || 9;
      const label = el.show !== false && (
        <text
          x={lp.x}
          y={lp.y}
          fontSize={fs}
          ...
          transform={uprightT(
            lp.x,
            lp.y,
            (el as { rot?: number }).rot || 0,
            (el as { flip?: boolean }).flip
          )}
        >
          {el.name}
        </text>
      ...
      let glyph: ReactNode;
      if (el.t === 'netflag' && el.flagType === 'gnd')
        glyph = (
          <>
            <line x1={el.x} y1={el.y} x2={el.x} y2={el.y + 12} />
            <line x1={el.x - 8} y1={el.y + 12} x2={el.x + 8} y2={el.y + 12} />
            <line x1={el.x - 5} y1={el.y + 16} x2={el.x + 5} y2={el.y + 16} />
            <line x1={el.x - 2} y1={el.y + 20} x2={el.x + 2} y2={el.y + 20} />
          </>
        );
      else if (el.t === 'netflag')
        glyph = (
          <>
            <line x1={el.x} y1={el.y} x2={el.x} y2={el.y - 12} />
            <line x1={el.x - 8} y1={el.y - 12} x2={el.x + 8} y2={el.y - 12} />
            <path
              d={`M${el.x - 5},${el.y - 12} L${el.x},${el.y - 19} L${el.x + 5},${el.y - 12} Z`}
              fill={gc}
            />
          </>
        );
      else if (el.t === 'netport')
        glyph = (
          <polygon
            points={`${el.x},${el.y} ${el.x + 8},${el.y - 7} ${el.x + 46},${el.y - 7} ${el.x + 46},${el.y + 7} ${el.x + 8},${el.y + 7}`}
            fill="none"
          />
        );
      else
        glyph = (
          <>
            <circle cx={el.x} cy={el.y} r={5} fill="none" />
            <line x1={el.x + 5} y1={el.y} x2={el.x + 11} y2={el.y} />
            <path
              d={`M${el.x - 2},${el.y + 2} L${el.x},${el.y - 3} L${el.x + 2},${el.y + 2}`}
              fill="none"
            />
          </>
        );
    ...
    case 'noconnect': {
      ...
      return (
        <g
          key={key}
          stroke={el.stroke || '#33cc33'}
          strokeWidth={1.4}
          opacity={preview ? 0.6 : 1}
          transform={selfRot(el.x, el.y)}
        >
          <line x1={el.x - s} y1={el.y - s} x2={el.x + s} y2={el.y + s} />
          <line x1={el.x - s} y1={el.y + s} x2={el.x + s} y2={el.y - s} />
        </g>
      );
    }
    case 'group':
      return (
        <g key={key} opacity={preview ? 0.6 : 1}>
          {el.children.map((c, i) => renderEl(c, i, preview, upr))}
        </g>
      );
    default:
      return null;
...
export function elBBox(el: El): { x: number; y: number; w: number; h: number } {
  switch (el.t) {
    ...
    case 'arrow':
      return { x: el.x - el.size, y: el.y - el.size, w: el.size * 2, h: el.size * 2 };
    case 'text': {
      const fs = parseInt(el.fontSize) || 9;
      return { x: el.x, y: el.y - fs, w: Math.max(20, el.text.length * fs * 0.6), h: fs * 1.4 };
    }
    case 'image':
    ...
    case 'netlabel': {
      const fs = parseInt(el.fontSize) || 7;
      return { x: el.x, y: el.y - fs, w: Math.max(20, el.name.length * fs * 0.6), h: fs * 1.4 };
    }
    case 'netflag':
    case 'netport':
    case 'probe': {
      const glyph =
        el.t === 'netport'
          ? { x: el.x, y: el.y - 7, w: 46, h: 14 }
          : el.t === 'probe'
            ? { x: el.x - 5, y: el.y - 5, w: 16, h: 10 }
            : el.flagType === 'gnd'
              ? { x: el.x - 8, y: el.y, w: 16, h: 20 }
              : { x: el.x - 8, y: el.y - 19, w: 16, h: 19 };
      const tb = elTextBBox(el);
      if (!tb) return glyph;
      const x0 = Math.min(glyph.x, tb.x),
        y0 = Math.min(glyph.y, tb.y);
      return {
        x: x0,
        y: y0,
        w: Math.max(glyph.x + glyph.w, tb.x + tb.w) - x0,
        h: Math.max(glyph.y + glyph.h, tb.y + tb.h) - y0,
      };
    }
    case 'noconnect':
      return { x: el.x - 6, y: el.y - 6, w: 12, h: 12 };
    case 'group': {
      const bs = el.children.map(elBBox);
...
export function renderFpEl(el: FpEl, key: Key, preview = false, layers?: LayerState) {
  ...
  switch (el.t) {
    ...
    case 'pad': {
      const pc = st ? st.color : fpLayer(el.layer);
      const pad =
        el.shape === 'Prostokąt' ? (
          <rect x={-el.w / 2} y={-el.h / 2} width={el.w} height={el.h} fill={pc} />
        ) : (
          <ellipse cx={0} cy={0} rx={el.w / 2} ry={el.h / 2} fill={pc} />
        );
      return (
        <g key={key} transform={`translate(${el.x},${el.y}) rotate(${el.rot})`} opacity={op}>
          {pad}
          {el.hole > 0 && <circle r={el.hole / 2} fill="#111" />}
          <text y={3} fontSize={7} fill="#1b1f24" textAnchor="middle" fontFamily="sans-serif">
            {el.num}
          </text>
        </g>
      );
    ...
    case 'via':
      return (
        <g key={key} opacity={op}>
          <circle cx={el.x} cy={el.y} r={el.dia / 2} fill="#c9a227" />
          <circle cx={el.x} cy={el.y} r={el.holeW / 2} fill="#111" />
        </g>
      );
    case 'ftext':
      return (
    ...
    case 'arc': {
      ...
      const p0 = { x: el.cx + el.r * Math.cos(rad(el.a0)), y: el.cy + el.r * Math.sin(rad(el.a0)) },
        p1 = { x: el.cx + el.r * Math.cos(rad(el.a1)), y: el.cy + el.r * Math.sin(rad(el.a1)) };
      const large = Math.abs(el.a1 - el.a0) % 360 > 180 ? 1 : 0,
        sweep = el.dir === 'Anti-Clockwise' ? 0 : 1;
      return (
        <path
          key={key}
          d={`M${p0.x},${p0.y} A${el.r},${el.r} 0 ${large} ${sweep} ${p1.x},${p1.y}`}
          fill="none"
          stroke={col}
          strokeWidth={el.width}
          opacity={op}
    ...
    case 'copper':
      return (
        <polygon
          ...
          points={el.pts.map((p) => `${p.x},${p.y}`).join(' ')}
          fill={col}
          fillOpacity={el.fillStyle === 'Brak' ? 0 : 0.28}
          stroke={col}
          strokeWidth={2}
          strokeDasharray={preview ? '4 3' : undefined}
          opacity={op}
    ...
    case 'dimension': {
      ...
      return (
        <g key={key} opacity={op}>
          <line x1={el.x1} y1={el.y1} x2={el.x2} y2={el.y2} stroke={col} strokeWidth={el.width} />
          <text
            x={(el.x1 + el.x2) / 2}
            y={(el.y1 + el.y2) / 2 - 4}
            fontSize={28}
            ...
            textAnchor="middle"
          >
            {(len * W2MM).toFixed(el.precision)}mm
          </text>
        </g>
      );
    }
    ...
    case 'group':
      return (
        <g key={key} opacity={op}>
          {el.children.map((c, i) => renderFpEl(c, `${key}-${i}`, preview, layers))}
        </g>
      );
    default:
      return null;
...
export function elBBoxFp(el: FpEl): { x: number; y: number; w: number; h: number } {
  if (el.t === 'group') {
    if (!el.children.length) return { x: 0, y: 0, w: 0, h: 0 };
    const bs = el.children.map(elBBoxFp);
    const x0 = Math.min(...bs.map((b) => b.x)),
      y0 = Math.min(...bs.map((b) => b.y));
    return {
      x: x0,
      y: y0,
      w: Math.max(...bs.map((b) => b.x + b.w)) - x0,
      h: Math.max(...bs.map((b) => b.y + b.h)) - y0,
    };
  }
  switch (el.t) {
    case 'track':
    ...
    case 'via':
      return { x: el.x - el.dia / 2, y: el.y - el.dia / 2, w: el.dia, h: el.dia };
    case 'ftext':
      return {
        x: el.x,
        y: el.y - el.height,
        w: Math.max(20, el.text.length * el.height * 0.6),
        h: el.height * 1.3,
      };
    case 'arc':
    case 'fcircle':
      return {
        x: el.cx - Math.abs(el.r),
        y: el.cy - Math.abs(el.r),
        w: 2 * Math.abs(el.r),
        h: 2 * Math.abs(el.r),
      };
    case 'hole':
      return { x: el.x - el.hole / 2, y: el.y - el.hole / 2, w: el.hole, h: el.hole };
    case 'dimension': {
...
export const fitVB = (b: { x: number; y: number; w: number; h: number }, pad = 8) =>
  `${b.x - pad} ${b.y - pad} ${Math.max(1, b.w) + 2 * pad} ${Math.max(1, b.h) + 2 * pad}`;
export const unionBB = <T,>(
  arr: T[],
  bb: (e: T) => { x: number; y: number; w: number; h: number }
) => {
