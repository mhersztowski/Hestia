...
function symbolPane(els: El[]): Pane {
  return {
    bg: '#ffffff',
    viewBox: fitVB(unionBB(els, elBBox)),
    svg: els.map((e, i) => renderEl(e, i)),
  };
}
function footprintPane(els: FpEl[]): Pane {
  ...
  return {
    bg: '#111',
    viewBox: els.length ? fitVB(unionBB(els, elBBoxFp), 30) : FALLBACK_VB,
    svg: els.map((e, i) => renderFpEl(e, i, false, layers)),
  };
}
function sheetPane(project: PcbProject, sheetId?: string): Pane {
  const sheets = project.sheets ?? [];
  const sheet =
    (sheetId ? sheets.find((s) => s.id === sheetId) : undefined) ??
  ...
  return {
    ...
    svg: [
      ...els.map((e, i) => renderEl(e, `e${i}`)),
      ...placed.map((c, i) => renderPlaced(c, `p${i}`)),
    ],
  };
}
...
export function PcbViewerPage({ vfsPath }: Props) {
  ...
  const { dir, name, view, itemId } = useMemo(() => {
    ...
    if (parts.length >= 2 && VIEWS.includes(parts[parts.length - 2] as View)) {
      v = parts[parts.length - 2] as View;
      item = parts[parts.length - 1];
      fileSegs = parts.slice(0, -2);
    } else if (parts.length >= 1 && VIEWS.includes(parts[parts.length - 1] as View)) {
      v = parts[parts.length - 1] as View;
      fileSegs = parts.slice(0, -1);
    }
    return {
      dir: '/' + fileSegs.slice(0, -1).join('/'),
      name: fileSegs[fileSegs.length - 1] ?? '',
      view: v,
      itemId: item,
    };
  }, [vfsPath]);
  ...
  const allPlaced = useMemo(
    () => (project?.sheets ?? []).flatMap((s) => s.placed ?? []),
    [project]
  );
    ...
    >
      {!itemMode && project && (
        <Tabs
          ...
          sx={{
            ...
            '& .MuiTab-root': { minHeight: 34, py: 0, fontSize: 12, color: '#aeb4bb' },
      ...
      >
        ...
        {!error && !itemMode && project && tab === '3d' && (
          <Pcb3DView
            pcbEls={project.pcb?.elements ?? []}
            placed={allPlaced}
            layers={defaultLayers()}
          />
        )}
        {!error && !itemMode && projectPane && <SvgPane pane={projectPane} />}
      </Box>
    </Box>
  );
}
