...
function findPath(nodes: MapNode[], id: string, trail: string[] = []): string[] | null {
  for (const n of nodes) {
    ...
    if (n.children) {
      const r = findPath(n.children, id, [...trail, n.id]);
      if (r) return r;
    }
  }
  return null;
...
function allGroupIds(nodes: MapNode[], acc: string[] = []): string[] {
  for (const n of nodes) {
    if (n.type === 'group') {
      acc.push(n.id);
      if (n.children) allGroupIds(n.children, acc);
    }
  }
  return acc;
}
...
function nodeIcon(type: MapNodeType) {
  ...
  switch (type) {
    case 'tile-layer':
      return <LayersIcon sx={{ ...sx, color: '#ffb74d' }} />;
    case 'marker':
      return <PlaceIcon sx={{ ...sx, color: '#ef5350' }} />;
    case 'polygon':
      return <CropSquareIcon sx={{ ...sx, color: '#4fc3f7' }} />;
    case 'polyline':
      return <TimelineIcon sx={{ ...sx, color: '#66bb6a' }} />;
    case 'circle':
      return <RadioButtonUncheckedIcon sx={{ ...sx, color: '#ce93d8' }} />;
    case 'group':
      return <FolderIcon sx={{ ...sx, color: '#78909c' }} />;
    case 'route':
      return <RouteIcon sx={{ ...sx, color: '#42a5f5' }} />;
  }
}
  ...
  return (
    <>
      ...
      >
        ...
        />
        {node.info?.trim() && (
          <InfoOutlinedIcon sx={{ fontSize: 12, color: '#4fc3f7', flexShrink: 0 }} />
        )}
      </ListItemButton>
      {hasChildren && (
...
export function MapViewerPage({ vfsPath }: Props) {
  ...
  useEffect(() => {
    if (nodes) setExpandedIds(new Set(allGroupIds(nodes)));
  }, [nodes]);
  ...
  const flyTo = (node: MapNode) => {
    ...
    if (pts.length === 1) {
      map.flyTo(pts[0], Math.max(map.getZoom(), 15), { duration: 0.5 });
      return;
    }
    const b = L.latLngBounds(pts);
  ...
  const handleMapClick = (node: MapNode) => {
    setSelectedId(node.id);
    setInfoId(node.id);
    revealInHierarchy(node.id);
  };
        ...
        <Box sx={{ flex: 1, display: 'flex', minHeight: 0 }}>
          ...
          <Box sx={{ flex: 1, position: 'relative', minWidth: 0 }}>
            ...
            >
              ...
              />
              {nodes.map((n) =>
                renderMapNode(n, { onClick: handleMapClick, skipTileLayers: true })
              )}
            ...
            >
              ...
              >
                {BASEMAPS.map((b) => (
                  <MenuItem key={b.key} value={b.key} sx={{ fontSize: '0.75rem' }}>
                    {b.label}
                  </MenuItem>
                ))}
              </Select>
              ...
              {described.length === 0 ? (
                ...
                <List dense disablePadding sx={{ flex: 1, overflow: 'auto' }}>
                  {described.map((n) => (
                    ...
                    >
                      <InfoOutlinedIcon
                        sx={{ fontSize: 14, color: '#4fc3f7', mr: 1, flexShrink: 0 }}
                      />
                      <ListItemText
                        primary={n.name}
