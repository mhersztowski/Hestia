...
describe('statyczny — pozycje wprost', () => {
  ...
  it('nie podaje stopni swobody — tu nie ma o co pytać', () => {
    const r = solveLayout(
      DOK({ shapes: [{ id: 'a', x: lit(0), y: lit(0), w: lit(1), h: lit(1) }] })
    );
  ...
  it('kolumna układa w pionie', () => {
    const r = solveLayout(
      DOK({
        mode: 'flow',
        shapes: [
          { ...kontener, container: { direction: 'column', gap: 4, padding: 0 } },
  ...
  it('„stretch" rozciąga w poprzek kierunku układania', () => {
    const r = solveLayout(
      DOK({
        mode: 'flow',
        shapes: [
          { ...kontener, container: { direction: 'row', gap: 0, padding: 10, align: 'stretch' } },
          { id: 'a', parent: 'box', x: lit(0), y: lit(0), w: lit(50), h: lit(20) },
        ],
      })
    );
